...
class WarrantyClaim(models.Model):
    ...
    available_so_ids = fields.Many2many('sale.order', compute='_compute_available_so_ids', string='Available SO',
                                        help="Technical field to filter sale order based on selected product", groups='sales_team.group_sale_salesman')
    sale_order_id = fields.Many2one('sale.order', string='Sale Order', readonly=False,
                                    ...
                                    domain="[('id', 'in', available_so_ids)]", groups='sales_team.group_sale_salesman')
    ...
    def _compute_warranty_claim_policy_ids(self):
        ...
        for r in self:
            ...
            if r.type == 'customer' and r.sale_order_id:
                warranty_policies = r.sale_order_id.order_line.filtered(lambda line: line.product_id == self.product_id).mapped('warranty_policy_ids')
    ...
    def action_investigation(self):
        for r in self:
            if r.type == 'customer' and r.product_id and r.sale_order_id:
