...
class TestZoomAPIV2(SavepointCase):
    ...
    def test_generate_JWT_key_with_correct_params(self, mock_jwt, mock_fields):
        ...
        mock_fields.Datetime.now().timestamp().__int__.return_value = exp
        result = self.env['zoom.apiv2'].generate_JWT_key()

        encode_jwt = mock_jwt.encode(
            {
        ...
        self.assertEqual(
            result,
            encode_jwt,
            "to_zoom_calendar: wrong result of generate_JWT_key",
        )
    ...
    def test_generate_JWT_key(self, mock_fields):
        ...
        self.env.company.zoom_api_key = self.zoom_api_key
        self.env.company.zoom_secret_key = self.zoom_secret_key
        exp = int(fields.Datetime.to_datetime("3000-01-01 08:00:00").timestamp()) + 60
        mock_fields.Datetime.now().timestamp().__int__.return_value = exp

        self.assertEqual(
            self.env['zoom.apiv2'].generate_JWT_key(),
            'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJ0ZXN0LWtleSIsImV4cCI6MzI1MDM3MDg5MjB9.JjCeO20GC-sbqJueO4YMw1l80YDD9ijTQmsltILLPSA'
        )
