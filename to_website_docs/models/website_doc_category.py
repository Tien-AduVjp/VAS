...
class WebsiteDocCategory(models.Model):
    ...
    def action_view_childs(self):
        ...
        if self.type == 'subject':
            res = self.env['ir.actions.act_window']._for_xml_id('to_website_docs.section_action')
            res['context'] = {'default_type': 'section'}

        # if type == 'section', children's type will be category
        elif self.type == 'section':
            res = self.env['ir.actions.act_window']._for_xml_id('to_website_docs.category_action')
            res['context'] = {'default_type': 'category'}

        # @TODO: consider nested cateogries here
        else:
            res = self.env['ir.actions.act_window']._for_xml_id('to_website_docs.document_action')
    ...
    def get_perms(self):
        ...
        _perm_docs.load_category(self, self.env.user)
        return _perm_docs
