...
class WebsiteDocument(models.Model):
    ...
    def action_view_contents(self):
        ...
        action = self.env.ref('to_website_docs.document_content_action')
        ...
        result['context'] = {'default_document_id': self.id}
        if total_records != 1:
            result['domain'] = "[('document_id', 'in', %s)]" % str(self.ids)
        elif total_records == 1:
            res = self.env.ref('to_website_docs.document_content_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = content_ids.id
        return result
    ...
    def get_perms(self, category=None):
        ...
        if self._perm_docs is None:
            ...
            self._perm_docs.load_document(self, self.env.user)
