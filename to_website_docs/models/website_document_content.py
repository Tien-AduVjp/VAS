...
class WebsiteDocumentContent(models.Model):
    ...
    def get_perms(self, category=None):
        ...
        """
        if self._perm_docs_content is None:
            self._perm_docs_content = document_permission.DocumentPermission()
            self._perm_docs_content.load_document_content(self, self.env.user)
        return self._perm_docs_content
