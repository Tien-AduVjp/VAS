...
String.prototype.toASCII = function() {
	if (FE.UNICODE === undefined) {
		FE.UNICODE = [ "﻿a", "á", "à", "ả", "ã", "ạ", "ă", "ắ", "ằ", "ẳ", "ẵ", "ặ", "â", "ấ", "ầ", "ẩ", "ẫ", "ậ", "o",
				"ó", "ò", "ỏ", "õ", "ọ", "ô", "ố", "ồ", "ổ", "ỗ", "ộ", "ơ", "ớ", "ờ", "ở", "ỡ", "ợ", "e", "é", "è",
				"ẻ", "ẽ", "ẹ", "ê", "ế", "ề", "ể", "ễ", "ệ", "i", "í", "ì", "ỉ", "ĩ", "ị", "u", "ú", "ù", "ủ", "ũ",
				"ụ", "ư", "ứ", "ừ", "ử", "ữ", "ự", "y", "ý", "ỳ", "ỷ", "ỹ", "ỵ", "đ", "A", "Á", "À", "Ả", "Ã", "Ạ",
				"Ă", "Ắ", "Ằ", "Ẳ", "Ẵ", "Ặ", "Â", "Ấ", "Ầ", "Ẩ", "Ẫ", "Ậ", "O", "Ó", "Ò", "Ỏ", "Õ", "Ọ", "Ô", "Ố",
				"Ồ", "Ổ", "Ỗ", "Ộ", "Ơ", "Ớ", "Ờ", "Ở", "Ỡ", "Ợ", "E", "É", "È", "Ẻ", "Ẽ", "Ẹ", "Ê", "Ế", "Ề", "Ể",
				"Ễ", "Ệ", "I", "Í", "Ì", "Ỉ", "Ĩ", "Ị", "U", "Ú", "Ù", "Ủ", "Ũ", "Ụ", "Ư", "Ứ", "Ừ", "Ử", "Ữ", "Ự",
				"Y", "Ý", "Ỳ", "Ỷ", "Ỹ", "Ỵ", "Đ" ];

		FE.ACSII = [ "﻿a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "o",
				"o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "e", "e", "e",
				"e", "e", "e", "e", "e", "e", "e", "e", "e", "i", "i", "i", "i", "i", "i", "u", "u", "u", "u", "u",
				"u", "u", "u", "u", "u", "u", "u", "y", "y", "y", "y", "y", "y", "d", "A", "A", "A", "A", "A", "A",
				"A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "O", "O", "O", "O", "O", "O", "O", "O",
				"O", "O", "O", "O", "O", "O", "O", "O", "O", "O", "E", "E", "E", "E", "E", "E", "E", "E", "E", "E",
				"E", "E", "I", "I", "I", "I", "I", "I", "U", "U", "U", "U", "U", "U", "U", "U", "U", "U", "U", "U",
				"Y", "Y", "Y", "Y", "Y", "Y", "D" ];
	}
	...
	for (var i = 0; i < this.length; i++) {
		var c = this.charAt(i);
		for (var j = 0; j < FE.UNICODE.length; j++) {
			if (FE.UNICODE[j] === c) {
				c = FE.ACSII[j];
				break;
			}
...
Number.prototype.toFixed = function(precision) {
	...
	var power = Math.pow(10, precision || 0), value = String(Math.round(number * power) / power);
...
function getUrlParam(paramName, str) {
	if (str === undefined) {
		str = window.top.location.search;
	}
...
function setCookie(a, b) {
	if (window.widget) {
		...
		document.cookie = a + "=" + encodeURIComponent(b) + "; expires=" + (new Date(time)).toGMTString() + "; path=/";
	}
}
...
function setSession(a, b) {
	if (new RegExp(a + '=([^;]*);', '').test(window.name + ';')) {
		window.name = window.name.replace(new RegExp(a + '=([^;]*);', ''));
		if (b != null) {
			window.name + '=' + encodeURIComponent(b) + ';';
		}
	} else {
...
function addScript(src, onload) {
	...
	if (FE.HEAD === undefined) {
		FE.HEAD = document.getElementsByTagName('head')[0];
	}
	FE.HEAD.appendChild(script);
}
...
window.busy = function(open, cls) {
	if (busyMask == null) {
		busyMask = new LoadingMask();
	}
	if (open === true) {
		busyMask.open(cls);
	} else {
...
function LoadingMask() {
	this.count = 0;
	this.element = $('<div id="mask" class="noprint" style="z-index: 2001;"/>');

	this.element.appendTo(FE.body);
	...
	this.showLoading = function() {
		...
		var me = this;
		if (!me.warp) {
			me.warp = $('<div class="loading-warp">' + FE.LOADING + '</div>').appendTo(me.element);
		}
		me._pos();
	};
	...
	this._size = function() {
		var doc = FE.document;
		this.element.height(doc.height()).width(doc.width());
	};

	this._pos = function() {
		var w = FE.window, top = (w.height() - this.warp.height()) / 2 + w.scrollTop(), left = (w.width() - this.warp
				.width())
				/ 2 + w.scrollLeft();
		this.warp.css({
			position : 'absolute',
	...
	try {
		...
		if (params != null) {
			if (!(params instanceof Array)) {
				val = val.replace('{0}', $.trim(params));
			} else {
				$.each(params, function(index, value) {
					val = val.replace('{' + index + '}', $.trim(value));
				});
			}
		}
		return val;
...
function getLabelDefault(key, def) {
	var val = message[key];
	if (!val) {
		val = def;
	}
...
(function($, FE) {
	...
	$.fn.scrollTo = function(interval, fn) {
		var off = this.offset();
		if (off) {
			$.scrollTo(off.top - 50, interval, fn);
		}
	};
