...
class HrEmployeeBase(models.AbstractModel):
    ...
    #TODO 15:change field name "qualifying_skill_report_ids" to "qualified_skill_report_ids"
    qualifying_skill_report_ids = fields.One2many('hr.employee.skill.report', 'employee_id',
                                             domain="[('expectation','=','required'),('reach_progress','>=',100.0)]",
                                             string='Qualified Skills', help="Show required skills that the employee is currently qualify.")
    #TODO 15:change field name "qualifying_skills_report_entries_count" to "qualified_skills_report_entries_count"
    qualifying_skills_report_entries_count = fields.Integer(string='Qualified Skills Count', compute='_compute_qualifying_skills_report_entries_count')
    ...
    @api.depends('qualifying_skill_report_ids')
    def _compute_qualifying_skills_report_entries_count(self):
        report_data = self.env['hr.employee.skill.report'].read_group([
            ('expectation', '=', 'required'),
            ('reach_progress', '>=', 100.0),
            ], ['employee_id'], ['employee_id'])
        mapped_data = dict([(dict_data['employee_id'][0], dict_data['employee_id_count']) for dict_data in report_data])
        for r in self:
            r.qualifying_skills_report_entries_count = mapped_data.get(r.id, 0)
    ...
    def action_view_required_skill_descriptions(self):
        ...
        action['context'] = {}
        action['domain'] = "[('id','in',%s)]" % self.skill_description_ids.ids
        return action

    def action_view_preferred_skill_descriptions(self):
        action = self.env['ir.actions.act_window']._for_xml_id('viin_hr_skill_framework.action_skill_description')
        action['context'] = {}
        action['domain'] = "[('id','in',%s)]" % self.preferred_skill_description_ids.ids
        return action

    def action_view_skill_reports(self):
        action = self.env['ir.actions.act_window']._for_xml_id('viin_hr_skill_framework.action_hr_employee_skill_report_report')
        action['context'] = {}
    ...
    def action_view_unmet_skill_report(self):
        action = self.env['ir.actions.act_window']._for_xml_id('viin_hr_skill_framework.action_hr_employee_skill_report_report')
        action['context'] = {}
        action['domain'] = "[('employee_id','in',%s),('expectation','=','required'),('reach_progress','<',100.0)]" % self.ids
        return action

    def action_view_qualifying_skill_report(self):
        result = self.env['ir.actions.act_window']._for_xml_id('viin_hr_skill_framework.action_hr_employee_skill_report_report')
        result['context'] = {}
