...
class TestHrPayrollMealAccount(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        # Meal type
        cls.meal_type = cls.env.ref('to_hr_meal.hr_meal_type_lunch_for_everyone')
    ...
    def test_01_hr_payroll_meal_account(self):
        ...
        không được thiết lập là '1388 Phải thu khác', '334 Phải trả người lao động'
        """
    ...
    def test_02_hr_payroll_meal_account(self):
        ...
        - Output: Tài khoản ghi nợ và Tài khoản ghi có của quy tắc lương "Tổng khẩu trừ suất ăn" (Mã: MODED)
        thiết lập lần lượt là '1388 Phải thu khác', '334 Phải trả người lao động'
        """
        ...
        meal_salary_rule = self.env['hr.salary.rule'].search([('code','=', 'MODED'), ('company_id', '=', self.company_demo.id)])
        account334 = self.env['account.account'].search([('code', '=', '334'), ('company_id', '=', self.company_demo.id)], limit=1)
        account1388 = self.env['account.account'].search([('code', '=', '1388'), ('company_id', '=', self.company_demo.id)], limit=1)
        self.assertRecordValues(
            meal_salary_rule,
            [{'account_debit': account1388.id, 'account_credit': account334.id}]
        )
    ...
    def test_03_hr_payroll_meal_account(self):
        ...
        - Output:
            - Sinh ra bút toán Có 1388 30000/ Nợ 334 30000
        ...
        journal_entry_account_334 = payslip_employee.move_id.line_ids.filtered(
            lambda a: a.salary_rule_id.code == 'MODED' and a.account_id.code == '334'
        )
        self.assertRecordValues(
            (journal_entry_account_1388 | journal_entry_account_334),
            [
                {
                    'debit': 0,
                    'credit': 30000,
