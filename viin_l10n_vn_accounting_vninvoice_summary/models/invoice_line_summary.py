...
class AccountInvoiceLineGroup(models.Model):
    ...
    def _prepare_vninvoice_line_summary_data(self, sequence=None):
        ...
        if taxes_count == 1:
            ...
            if self.invoice_line_tax_ids.tax_group_id == self.env.ref('l10n_vn_common.account_tax_group_exemption'):
                vatPercent = -1
        ...
        if self.invoice_id.invoice_line_summary_group_mode in ['product_unit_price_tax_discount','product_tax_discount']:
            if self.invoice_id.einvoice_api_version == 'v1':
                data.update({
        ...
        data.update({
            ...
            'vatPercentDisplay': self.invoice_line_tax_ids.with_context(lang=self.env.ref('base.lang_vi_VN').code).name,
    ...
    def _prepare_einvoice_line_summary_data(self, sequence=None):
        ...
        if self.invoice_id.einvoice_provider == 'vninvoice':
