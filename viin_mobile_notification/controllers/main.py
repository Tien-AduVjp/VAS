...
class ControllerNotifications(http.Controller):
    ...
    def add_token(self, token, platform, package_name):
        ...
        device_tokens = request.env['mobile.device.token'].sudo().search([('token', '=', token)])
        res = {'status': 200}
        if (
            device_tokens.partner_id.ids == partner.ids \
            and any(tk.platform == platform for tk in device_tokens) \
            and any(tk.package_name == package_name for tk in device_tokens)
            ):
            res['message'] = "Token already exists"
        elif device_tokens:
            device_tokens.write({
            ...
            request.env['mobile.device.token'].sudo().create({
                'token': token,
                'partner_id': partner.id,
                'platform': platform,
                'package_name': package_name
    ...
    def logout(self, token):
        ...
        It will check if token exists in table 'mobile.device.token' or not?
        If exists, will unlink this token in table 'mobile.device.token' then return message "Delete mobile token successfully."
        ...
        res = {'status': 200, 'message': "Token does not exist"}
        if device_token:
            device_token.sudo().unlink()
            res['message'] = "Delete mobile token successfully."
        return res
    ...
    def _check_mobile_channel_seen(self, partner_id, channels, mobile_channel_seens):
        ...
        for channel in channels:
            if channel.id in channel_partners:
                ...
                if partner_last_seen_id in all_message.ids:
                    partner_last_seen_index = all_message.ids.index(partner_last_seen_id)
                else:
                    partner_last_seen_index = 0
        ...
        if notifications:
            notify_groups_channel = notifications._get_group_notification()
    ...
    def _check_mobile_last_inbox(self, partner_id, mobile_inbox_date):
        ...
        if all_mail_notification:
            ...
            notify_inbox_groups = mail_notifications._get_group_notification()
    ...
    def _get_number_badge(self, partner_id, channels):
        ...
        @param {list} channels: List of channels where current partner is a member
        """
        # This function return number of badges
        unread_counters = sum(channels.mapped('message_unread_counter'))
