...
class AccountAssetDepreciationLine(models.Model):
    ...
    move_check = fields.Boolean(compute='_get_move_check', string='Depreciation Entry Status', tracking=True, store=True,
    ...
    def _prepare_move_grouped(self):
        ...
        analytic_account_id = asset_id.sudo().analytic_account_id
        analytic_tag_ids = asset_id.sudo().analytic_tag_ids
        ...
        move_line_2 = {
            'name': name,
            'account_id': asset_id.depreciation_expense_account_id.id or category_id.depreciation_expense_account_id.id,
            'credit': 0.0,
            'debit': amount,
            'journal_id': category_id.journal_id.id,
            'analytic_account_id': analytic_account_id.id if category_id.type == 'purchase' else False,
        ...
        for line in self:
            ...
            if not asset.depreciation_line_ids.filtered(lambda line: line.move_id.state == 'draft') \
               and asset.currency_id.is_zero(asset.value_residual):
                asset.create_account_move(post_move=False)
