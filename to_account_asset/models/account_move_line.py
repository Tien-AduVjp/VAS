...
class AccountMoveLine(models.Model):
    ...
    asset_start_date = fields.Date(string='Asset Start Date', compute='_compute_asset_date', readonly=True, store=True)
    asset_end_date = fields.Date(string='Asset End Date', compute='_compute_asset_date', readonly=True, store=True)
    asset_mrr = fields.Float(string='Monthly Recurring Revenue', compute='_compute_asset_date', readonly=True, digits='Account', store=True)
    ...
    def _prepare_asset_vals_list(self):
        # Only create a asset that have not been link to asset
        lines_with_depr_assets = self.env['account.asset.asset'].search([
            ('invoice_line_id', 'in', self.ids),
            ]).invoice_line_id

        vals_list = []
        prec = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        for r in self.filtered(lambda r: r not in lines_with_depr_assets):
    ...
    def _onchange_product_id(self):
        ...
        for r in self:
            if r.product_id:
                ...
                elif r.move_id.move_type == 'in_invoice':
    ...
    def _set_additional_fields(self, invoice):
        if not self.asset_category_id:
            ...
            self._onchange_asset_category_id()
        return super(AccountMoveLine, self)._set_additional_fields(invoice)
