...
class AccountAssetAsset(models.Model):
    ...
    entry_count = fields.Integer(compute='_compute_entry_count', string='# Asset Entries')
    posted_entry_count = fields.Integer(compute='_compute_entry_count', help='A technical field to show/hide Set to Draft button.')
    ...
    original_move_line_ids = fields.Many2many('account.move.line', 'asset_purchase_move_line_rel', 'asset_id', 'move_line_id',
                                              ...
                                              compute='_compute_original_move_lines', inverse='_inverse_original_move_lines', store=True,
    ...
    depreciation_expense_account_id = fields.Many2one('account.account', string='Expense Account',
    ...
    revaluation_decrease_account_id = fields.Many2one('account.account', string='Decrease Asset Account',
                                                      readonly=True, states={'draft': [('readonly', False)], 'open': [('readonly', False)]},
                                                      domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                                      ...
                                                      "When no account is set here, the Revaluation Entries: Decrease Asset Account "
                                                       "on Asset Category will be used instead.")
    revaluation_increase_account_id = fields.Many2one('account.account', string='Increase Asset Account',
                                                      readonly=True, states={'draft': [('readonly', False)], 'open': [('readonly', False)]},
                                                      domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
    ...
    value_residual = fields.Float(compute='_compute_amount_residual', readonly=True, digits=0, string='Residual Value')
    ...
    revaluation_value = fields.Float(string='Revaluation Value', compute='_compute_amount_residual', digits=0, readonly=True, help="It is the amount you plan to have that you cannot depreciate.", tracking=True, copy=False)
    ...
    def _compute_original_move_lines(self):
        for r in self:
    ...
    def _inverse_original_move_lines(self):
        for r in self.with_context(do_not_recompute_original_move_lines=True):
            invoice_lines = r.original_move_line_ids.filtered(lambda l: l.move_id.move_type in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund'))
            if invoice_lines:
    ...
    def _compute_depreciation_board(self):
        ...
        if not self.currency_id.is_zero(self.value_residual):
            value = self.value
        ...
        if self.state == 'open':
            self.depreciation_line_ids.filtered(lambda l: not l.move_check).create_move()
        return True
    ...
    def validate(self):
        ...
        # Automatic create depreciation entries when validating the asset
        self.depreciation_line_ids.filtered(lambda l: not l.move_check).create_move()
        ...
        for r in self:
            ...
            if posted_depreciation_line_ids:
                ...
                if posted_depreciation_line_ids and date < posted_depreciation_line_ids[-1].depreciation_date:
                    raise UserError(_("You cannot perform this action on the asset '%(name)s' that contains posted depreciation lines before %(date)s date.", name=r.name, date=date))
            ...
            if unposted_depreciation_line_ids:
                ...
                if date > first_unposted_depreciation_line.depreciation_date:
                    raise UserError(_("You have not posted all depreciation lines on the asset '%(name)s' before %(date)s date.", name=r.name, date=date))
    ...
    def create_account_move(self, post_move=True):
        ...
        account_move_obj = self.env['account.move'].with_context(default_move_type='entry')
    ...
    def _compute_entry_count(self):
        for r in self:
            r.entry_count = len(
                                ...
                                | r.move_ids.filtered(lambda move: move.move_type != 'out_invoice') \
    ...
    def action_view_sales_invoice(self):
        self.ensure_one()
        move_ids = self.mapped('move_ids').filtered(lambda move: move.move_type == 'out_invoice')
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('account.view_invoice_tree')
        action['domain'] = [('account_asset_id', 'in', self.ids)]
        return action
    ...
    def open_entries(self):
        moves = self.mapped('depreciation_line_ids.move_id')\
                | self.mapped('move_ids').filtered(lambda move: move.move_type != 'out_invoice')\
                | self.mapped('revaluation_line_ids.move_id')
        return {
