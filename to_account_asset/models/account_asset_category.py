...
class AccountAssetCategory(models.Model):
    ...
    depreciation_account_id = fields.Many2one('account.account', string='Depreciation Account', required=True,
                                              domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                              tracking=True,
                                              help="The account that is used in the depreciation entries, to decrease the asset value.")
    depreciation_expense_account_id = fields.Many2one('account.account', string='Expense Account', required=True,
                                                      domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                                      tracking=True,
                                                      help="The account that is used in the periodical entries, to record a part of the asset as expense.")
    disposal_expense_account_id = fields.Many2one('account.account', string='Disposal Account',
                                                       domain=[('internal_type', '=', 'other'), ('deprecated', '=', False)],
                                                       tracking=True,
                                                       default=lambda self: self._get_default_disposal_expense_account_id(),
    ...
    account_asset_ids = fields.One2many('account.asset.asset', 'category_id', string='Assets', groups='account.group_account_user')
    account_assets_count = fields.Integer(string='Assets Count', compute='_compute_account_assets_count', compute_sudo=True)
    ...
    def action_view_account_assets(self):
        ...
        if len(account_asset_ids) > 1:
            action['domain'] = "[('category_id', 'in', %s)]" % str(self.ids)
        else:
