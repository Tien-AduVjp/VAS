...
class AccountAssetAssetAddWizard(models.TransientModel):
    _name = 'account.asset.asset.add.wizard'
    _description = 'Add Value to Asset'
    ...
    def action_add_value(self):
        ...
        if len(asset_ids) > 1:
            raise UserError(_('You may only add values to one asset at a time'))
        
        asset_id = self.env['account.asset.asset'].browse(asset_ids)
        if asset_id and asset_id.state not in ['draft', 'open']:
            raise UserError(_('You may only add values to one asset that in draft / open state'))
