...
class TestAssetFlow(asset_common.AssetCommon):
    ...
    def test_01_asset_depreciation_complete(self):
        ...
        self.assertEqual('posted', asset.move_ids.state)
        self.assertEqual(asset.value, asset.move_ids.amount_total)

    @patch('odoo.fields.Date.context_today', lambda self: date(2020, 8, 10))
    def test_02_asset_sale(self):
        ...
        with patch('odoo.fields.Date.context_today', lambda self: date(2020, 11, 25)):
            depreciation_lines.move_id._post(soft=False)
            for state in depreciation_lines.move_id.mapped('state'):
                self.assertEqual('posted', state)
    ...
    def test_04_asset_dispose(self):
        ...
        with patch('odoo.fields.Date.context_today', lambda self: date(2021, 4, 18)):
            depreciation_lines.move_id._post(soft=False)
    ...
    @patch('odoo.fields.Date.context_today', lambda self: date(2021, 1, 18))
    def test_05_asset_increase(self):
        ...
        with patch('odoo.fields.Date.context_today', lambda self: date(2021, 1, 19)):
            asset.revaluation_line_ids.move_id._post(soft=False)
        self.assertEqual(asset.value, 100000000)
        ...
        asset.revaluation_line_ids.move_id.button_draft()
        self.assertEqual(asset.value, 100000000)
        self.assertEqual(asset.value_residual, 50000002)
        self.assertEqual(asset.revaluation_value, 0)
    ...
    @patch('odoo.fields.Date.context_today', lambda self: date(2021, 1, 17))
    def test_06_asset_decrease(self):
        ...
        # has been posted revaluation entry
        with patch('odoo.fields.Date.context_today', lambda self: date(2021, 1, 18)):
            # With today as 18/01/2021, depreciation entry that have accounting date on 18/01/2021 will auto posted
            asset.revaluation_line_ids.move_id._post(soft=False)
        self.assertEqual(asset.value, 100000000)
        self.assertEqual(asset.value_residual, 41428573)
    ...
    @patch('odoo.fields.Date.context_today', lambda self: date(2020, 8, 18))
    ...
    def _modify_asset(self, asset, method_number, method_period):
        modify_wizard = self.env['asset.modify'].create({
            'name': str(method_number) + '/' + str(method_period),
    ...
    def _revaluation_asset(self, asset, method, value, date):
        revaluation_wizard = self.env['asset.revaluation.wizard'].create({
            'name': method + str(value),
