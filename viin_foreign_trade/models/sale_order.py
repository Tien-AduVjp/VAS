...
class SaleOrder(models.Model):
    ...
    custom_declaration_ids = fields.Many2many('custom.declaration.export', string="Custom Clearance Requests", readonly=True,
        groups="stock.group_stock_user,viin_foreign_trade.group_foreign_trade_user", copy=False)
    custom_dec_count = fields.Integer(string="# of Custom Clearance Request", compute='_compute_custom_dec_count', compute_sudo=True)
    ...
    def _compute_custom_dec_count(self):
        for r in self:
            r.custom_dec_count = len(r.custom_declaration_ids)
    ...
    def action_view_custom_declarations(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('viin_foreign_trade.custom_declaration_export_action')
        # override the context to get rid of the default filtering
        result['context'] = {}
        # choose the view_mode accordingly
        if len(custom_declaration_ids) != 1:
            result['domain'] = [('sale_order_ids', 'in', [self.id])]
        elif len(custom_declaration_ids) == 1:
            res = self.env.ref('viin_foreign_trade.custom_declaration_export_form', False)
