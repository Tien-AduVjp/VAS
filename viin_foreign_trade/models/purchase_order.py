...
class PurchaseOrder(models.Model):
    ...
    custom_declaration_ids = fields.Many2many('custom.declaration.import', string="Custom Clearance Requests", readonly=True, copy=False)
    custom_dec_count = fields.Integer(string="# of Custom Declaration Documents", compute='_compute_custom_dec_count', compute_sudo=True)
    ...
    def action_view_custom_declarations(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('viin_foreign_trade.custom_declaration_import_action')
        # override the context to get rid of the default filtering
        result['context'] = {}
        # choose the view_mode accordingly
        if len(custom_declaration_ids) != 1:
            result['domain'] = [('purchase_order_ids', 'in', [self.id])]

        elif len(custom_declaration_ids) == 1:
            res = self.env.ref('viin_foreign_trade.custom_declaration_import_form', False)
