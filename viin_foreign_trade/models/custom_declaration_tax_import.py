...
class CustomDeclarationTaxImport(models.Model):
    _name = "custom.declaration.tax.import"
    _inherit = 'abstract.custom.declaration.tax'
    _description = "Custom Declaration Import Tax"
#     _order = 'sequence'
    ...
    custom_dec_tax_group_id = fields.Many2one('custom.declaration.tax.import.group', string='Custom Declaration Tax Group', readonly=True)
    ...
    def _prepare_account_move_lines(self):
        ...
        if self.is_vat:
            ...
            debit_account = self.tax_repartition_line_id.vat_ctp_account_id
        else:
            if not self.tax_id.import_tax_cost_product_id:
                raise ValidationError(_("Please specify import tax product for the tax '%s'.") % self.tax_id.name)
            accounts_data = self.tax_id.import_tax_cost_product_id.product_tmpl_id.sudo().get_product_accounts()
            debit_account = accounts_data['stock_input']
            if not debit_account:
                raise ValidationError(_("Cannot find stock valuation account specify for the product '%s'.")
                                % self.tax_id.import_tax_cost_product_id.name)
    ...
    def _get_custom_dec_tax_group_model(self):
        return 'custom.declaration.tax.import.group'
