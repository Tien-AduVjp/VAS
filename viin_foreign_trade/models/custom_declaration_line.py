...
class CustomDeclarationLine(models.Model):
    ...
    extra_expense_currency = fields.Monetary(string='Expense Taxable Value', currency_field='currency_id')
    ...
    @api.depends('currency_id', 'currency_rate', 'total_cost_currency', 'extra_expense_currency')
    def _compute_total_costs(self):
        for r in self:
            r.total_cost = r.currency_rate * (r.total_cost_currency + r.extra_expense_currency)
    ...
    def _prepare_tax_line_vals(self, tax):
        ...
        if self.custom_declaration_import_id:
            ...
            vals['payment_term_id'] = self.custom_declaration_import_id.company_id.import_tax_payment_term_id
        if self.custom_declaration_export_id:
            vals['custom_declaration_id'] = self.custom_declaration_export_id.id
            vals['payment_term_id'] = self.custom_declaration_export_id.company_id.export_tax_payment_term_id
    ...
    def _build_key_for_declaration_line(self):
        self.ensure_one()
        return (self.product_id.id, self.stock_move_id.id)

    def _find_reference_uom(self):
        uoms = self.product_uom
        ref_uom = self.env['uom.uom']
        if len(uoms.category_id) != 1:
            return ref_uom

        ref_uom = self.env['uom.uom'].search([('uom_type', '=', 'reference'),
                                              ('active', '=', True), ('category_id', '=', uoms.category_id.id)], limit=1)
        return ref_uom

    def _split_expense_by_equal(self, expense):
        dec_line_dic = {}
        if not self:
            return dec_line_dic

        expense_value = expense.expense_value / len(self)
        for r in self:
            dec_line_dic[r._build_key_for_declaration_line()] = expense_value
        return dec_line_dic

    def _split_expense_by_quantity(self, expense):
        dec_line_dic = {}
        uoms = self.product_uom
        if len(uoms) == 1:
            ...
            if total_qty > 0:
                for r in self:
                    qty_rate = r.qty / total_qty
                    expense_value = expense.expense_value * qty_rate
                    dec_line_dic[r._build_key_for_declaration_line()] = expense_value
        else:
            ref_uom = self._find_reference_uom()
            if ref_uom:
                total_qty = 0.0
                qty_dic = {}
                # convert qty to ref_uom qty and then calculate to total qty
                for r in self:
                    ref_uom_qty = r.product_uom._compute_quantity(r.qty, ref_uom, round=False)
                    qty_dic[r.id] = ref_uom_qty
                    total_qty += ref_uom_qty

                # split expense based on ref_uom qty
                for r in self:
                    qty_rate = qty_dic[r.id] / total_qty
                    expense_value = expense.expense_value * qty_rate
                    dec_line_dic[r._build_key_for_declaration_line()] = expense_value
        return dec_line_dic

    def _split_expense_by_cost(self, expense):
        dec_line_dic = {}
        total_cost = sum(self.mapped('total_cost_currency'))
        if total_cost > 0:
            for r in self:
                price_rate = r.total_cost_currency / total_cost
                expense_value = expense.expense_value * price_rate
                dec_line_dic[r._build_key_for_declaration_line()] = expense_value
        return dec_line_dic

    def _split_expense(self, expense):
        dec_line_dic = {}

        if expense.split_method == 'equal':
            dec_line_dic = self._split_expense_by_equal(expense)

        elif expense.split_method == 'by_quantity':
            dec_line_dic = self._split_expense_by_quantity(expense)

        elif expense.split_method == 'by_cost':
            dec_line_dic = self._split_expense_by_cost(expense)
