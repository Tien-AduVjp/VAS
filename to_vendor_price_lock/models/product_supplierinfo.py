...
class SuppliferInfo(models.Model):
    ...
    state = fields.Selection(STATES, string='Status', default='confirmed', index=True,
                    readonly=True, copy=False, required=True, tracking=True,
                    help="Confirmed: The pricelist is confirmed and active. "
                    "The price will be loaded into purchase order lines but still can be changed on the lines\n"
                    "Locked: the pricelist is locked and cannot be changed on purchase order lines "
                    "if the user is not a purchase manager")
    ...
    def unlink(self):
        for r in self:
            if r.state == 'locked':
                raise UserError(
                    _("You are not allowed to delete a pricelist which is in locked state."
        ...
        if not self.env.user.has_group('purchase.group_purchase_manager'):
            if vals['name'] and vals['product_tmpl_id']:
                ...
                if existing_locked_pricelist:
                    ...
                    raise UserError(
                        _("Creating a new vendor pricelist for product \'%s\' from the vendor \'%s\' is"
