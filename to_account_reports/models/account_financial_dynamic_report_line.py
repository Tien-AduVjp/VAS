...
#    report_code = fields.Char(string='Report Code', help='Code of an item witch will be shown in reports (if defined)')
    ...
    def _get_with_statement(self, financial_report):
        ...
        if financial_report == self.env.ref('to_account_reports.af_dynamic_report_cashsummary0'):
            ...
            # replace_columns = {
            #     'date': 'ref.date',
            #     'debit_cash_basis': 'CASE WHEN \"account_move_line\".debit > 0 THEN \"account_move_line\".debit_cash_basis ELSE 0 END AS debit_cash_basis',
            #     'credit_cash_basis': 'CASE WHEN \"account_move_line\".credit > 0 THEN \"account_move_line\".credit_cash_basis ELSE 0 END AS credit_cash_basis',
            #     'balance_cash_basis': '\"account_move_line\".balance_cash_basis AS balance_cash_basis'
            # }
            # columns = []
            # columns_2 = []
            ...
            replace_columns = {
                'date': 'ref.date',
                'debit_cash_basis': 'CASE WHEN aml.debit > 0 THEN aml.debit_cash_basis ELSE 0 END AS debit_cash_basis',
                'credit_cash_basis': 'CASE WHEN aml.credit > 0 THEN aml.credit_cash_basis ELSE 0 END AS credit_cash_basis',
                'balance_cash_basis': 'aml.balance_cash_basis AS balance_cash_basis'
            }
