...
class AccountFinancialDynamicReport(models.Model):
    ...
    def get_options(self, previous_options=None):
        cls = type(self)
        if self.date_range:
            cls.filter_date = {'date_from': '', 'date_to': '', 'filter': 'this_year'}
            if self.comparison:
                cls.filter_comparison = {'date_from': '', 'date_to': '', 'filter': 'no_comparison', 'number_period': 1}
        else:
            cls.filter_date = {'date': '', 'filter': 'today'}
            if self.comparison:
                cls.filter_comparison = {'date': '', 'filter': 'no_comparison', 'number_period': 1}
        if self.unfold_all_filter:
            cls.filter_unfold_all = False
        if self.show_journal_filter:
            cls.filter_journals = True
        cls.filter_all_entries = False
        cls.filter_analytic = True if self.analytic else None
        cls.filter_hierarchy = True if self.hierarchy_option else None
        ...
        for report in self:
            if not report.generated_menu_id:
                ...
                menu_vals = {
                    ...
                    'chart_template_id': report.chart_template_id.id,
                }
    ...
    def get_lines(self, options, line_id=None):
        ...
        res = line_obj.get_lines(self, currency_table, options, linesDicts)
        return res
