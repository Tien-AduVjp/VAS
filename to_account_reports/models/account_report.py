...
class AccountReport(models.AbstractModel):
    ...
    filter_account_all = None
    filter_partner_all = None
    ...
    def _build_options(self, previous_options=None):
        ...
        if self.env.user.id in group_multi_company.users.ids:
            ...
            allow_companies = self.env['res.company'].browse(self.env.context.get('allowed_company_ids', []))
            options['multi_company'] = [{'id': c.id, 'name': c.name, 'selected': True if c.id == self.env.company.id else False} for c in allow_companies]
    ...
    def get_options(self, previous_options=None):
        ...
        cls = type(self)
        if cls.filter_analytic:
            cls.filter_analytic = self.env.user.id in self.env.ref('analytic.group_analytic_accounting').users.ids and True or None
            cls.filter_analytic_tags = [] if self.filter_analytic else None
            cls.filter_analytic_accounts = [] if self.filter_analytic else None
        cls.filter_accounts = [] if cls.filter_account_all else None
        cls.filter_partners = [] if cls.filter_partner_all else None
    ...
    def open_tax(self, options, params=None):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('to_account_reports.action_move_line_select_tax_audit')
    ...
    def open_document(self, options, params=None):
        ...
        if aml_id:
            ...
            if document == 'account.move' and aml.move_id.id:
                res_id = aml.move_id.id
                if aml.move_id.move_type in ('in_refund', 'in_invoice'):
                    view_name = 'view_move_form'
                    ctx['journal_type'] = 'purchase'
                elif aml.move_id.move_type in ('out_refund', 'out_invoice'):
                    view_name = 'view_move_form'
                    ctx['journal_type'] = 'sale'
                ctx['type'] = aml.move_id.move_type
                ctx['default_type'] = aml.move_id.move_type
    ...
    def view_too_many(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        action = self.env['ir.actions.act_window']._for_xml_id('account.action_move_line_select')
        ctx['active_id'] = int(active_id)
        if model == 'account':
            ctx.update({
    ...
    def open_journal_items(self, options, params):
        ...
        if options:
            domain = expression.normalize_domain(safe_eval(action.get('domain', '[]') or '[]'))
            ...
            if options.get('accounts'):
                account_ids = [int(r) for r in options['accounts']]
                domain = expression.AND([domain, [('account_id', 'in', account_ids)]])
            if options.get('partners'):
                partner_ids = [int(r) for r in options['partners']]
            ...
            if params and 'account_id' in params:
                domain = expression.AND([domain, [('account_id', '=', params['account_id'])]])
            action['domain'] = domain
        if params and 'line_id' in params:
            move_lines = self.env['account.move.line'].search(action['domain'])
            report_line = self.env['account.financial.dynamic.report.line'].browse(params['line_id'])
            line_domain_str = report_line.domain
            if line_domain_str and report_line.groupby == 'account_id':
                line_domain = ast.literal_eval(line_domain_str)
                move_lines = move_lines.filtered_domain(line_domain)
                if move_lines:
                    action['domain'] = "[('id', 'in', %s)]" % str(move_lines.move_id.line_ids.ids)
        return action
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env['ir.actions.client']._for_xml_id('to_account_reports.action_af_report_general_ledger')
    ...
    def set_context(self, options):
        ...
        ctx['company_id'] = len(company_ids) > 0 and company_ids[0] or self.env.company.id
        if options.get('analytic_accounts'):
        ...
        if options.get('partners'):
            ctx['partner_ids'] = self.env['res.partner'].browse([int(p) for p in options['partners']])
        return ctx
    ...
    def get_rp_informations(self, options):
        ...
        if options.get('accounts') is not None:
            searchview_dict['accounts'] = [(a.id, a.code) for a in self.env['account.account'].search(self.get_account_domains())]
            options['selected_account_names'] = [self.env['account.account'].browse(int(account)).code for account in options['accounts']]
        if options.get('partners') is not None:
            partner_domain = []
            if options.get('multi_company', []):
                company_ids = [c.get('id') for c in options.get('multi_company', []) if c.get('selected')]
                partner_domain += ['|' ,('company_id', '=', False), ('company_id', 'in', company_ids)]
            searchview_dict['partners'] = [(p.id, p.name) for p in self.env['res.partner'].search(partner_domain)]
            options['selected_partner_names'] = [self.env['res.partner'].browse(int(partner)).name for partner in options['partners']]
        ...
        info = {'options': options,
                ...
                'searchview_html': self.env['ir.ui.view']._render_template(self.get_templates().get('search_template', 'to_account_reports.search_template'), values=searchview_dict),
                }
        return info
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        if minimal_layout:
            header = self.env['ir.actions.report']._render_template(self.get_templates().get('header_layout', 'web.internal_layout'), values=rcontext)
            footer = ''
            if self.get_templates().get('footer_layout', False):
                footer = self.env['ir.actions.report']._render_template(self.get_templates().get('footer_layout'), values=rcontext)
...
#         Todo in master: Try to put this logic elsewhere
        ...
        for super_col in super_columns.get('columns', []):
            ...
            cell_merge = super_col.get('merge', 1)
            if x_merge > 1:
                sheet.merge_range(start_row, x, start_row, x + (x_merge - 1), cell_content, super_col_style)
                x += x_merge
            else:
                if cell_merge > 1:
                    sheet.merge_range(start_row, x, start_row, x + 1, cell_content, super_col_style)
                    x += 2
                else:
        ...
        for y in range(0, len(lines)):
            ...
            if 'total' in lines[y].get('class', '') and (num_table_excel_col - len(lines[y]['columns'])) > 1:
                ...
                if isinstance(lines[y]['name'], date):
