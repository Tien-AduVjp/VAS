            ...
            if 'partner_' + str(partner.id) in options.get('unfolded_lines') or unfold_all:
                ...
                for line in amls:
                    ...
                    domain_lines.append({
                        ...
                        'name': line.date,
                        'columns': [{'name': v} for v in [line.journal_id.code, line.account_id.code, ', '.join(line.mapped('ctp_account_ids.code')), name, line.full_reconcile_id.name, self.format_value(progress_before),
