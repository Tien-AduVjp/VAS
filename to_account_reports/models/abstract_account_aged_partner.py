...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def get_columns_name(self, options):
        columns = [{}]
        date = format_date(self.env, options['date']['date'])
        if options['date']['filter'] == 'custom' and options['date'].get('date_to', False):
            date = format_date(self.env, options['date']['date_to'])
        columns += [{'name': v, 'class': 'number', 'style': 'white-space:nowrap;'} for v in [
            _("Not&nbsp;due&nbsp;on %s").replace('&nbsp;', ' ') % date,
            _("0&nbsp;-&nbsp;30").replace('&nbsp;', ' '),
            _("30&nbsp;-&nbsp;60").replace('&nbsp;', ' '),
            _("60&nbsp;-&nbsp;90").replace('&nbsp;', ' '),
            _("90&nbsp;-&nbsp;120").replace('&nbsp;', ' '),
    ...
    def get_templates(self):
        templates = super(ReportAccountAgedPartner, self).get_templates()
        templates['main_template'] = 'to_account_reports.template_aged_partner_balance_report'
        try:
            self.env['ir.ui.view'].get_view_id('to_account_reports.template_aged_partner_balance_line_report')
            templates['line_template'] = 'to_account_reports.template_aged_partner_balance_line_report'
        except ValueError:
            pass
        return templates
