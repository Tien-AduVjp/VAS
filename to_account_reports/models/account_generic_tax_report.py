...
class GenericTaxReport(models.AbstractModel):
    ...
    def _get_with_statement(self, user_types, domain=None):
        ...
        if self.env.context.get('cash_basis'):
            ...
            sql = """WITH account_move_line AS (
              SELECT \"account_move_line\".id, \"account_move_line\".date, \"account_move_line\".name, \"account_move_line\".debit_cash_basis, \"account_move_line\".credit_cash_basis, \"account_move_line\".move_id, \"account_move_line\".account_id, \"account_move_line\".journal_id, \"account_move_line\".balance_cash_basis, \"account_move_line\".amount_residual, \"account_move_line\".partner_id, \"account_move_line\".reconciled, \"account_move_line\".company_id, \"account_move_line\".company_currency_id, \"account_move_line\".amount_currency, \"account_move_line\".balance, \"account_account\".user_type_id, \"account_move_line\".tax_line_id, \"account_move_line\".tax_exigible, \"account_move_line\".display_type
               FROM \"account_account\",""" + tables + """
               WHERE (\"account_move_line\".journal_id IN (SELECT id FROM account_journal WHERE type in ('cash', 'bank'))
                 OR \"account_move_line\".move_id NOT IN (SELECT DISTINCT move_id FROM account_move_line LEFT JOIN account_account ON account_move_line.account_id = account_account.id WHERE account_account.user_type_id IN %s))
                 AND \"account_move_line\".account_id = \"account_account\".id AND """ + where_clause + """
              UNION ALL
              (
               WITH payment_table AS (
                 SELECT aml.move_id, \"account_move_line\".date, CASE WHEN aml.balance = 0 THEN 0 ELSE part.amount / ABS(am.amount_total_signed) END as matched_percentage
                   FROM account_partial_reconcile part LEFT JOIN account_move_line aml ON aml.id = part.debit_move_id LEFT JOIN account_move am ON aml.move_id = am.id,\"account_account\", """ + tables + """
                   WHERE part.credit_move_id = "account_move_line".id
                    AND "account_move_line".account_id = "account_account".id
                    AND "account_account".user_type_id IN %s
                    AND """ + where_clause + """
                 UNION ALL
                 SELECT aml.move_id, \"account_move_line\".date, CASE WHEN aml.balance = 0 THEN 0 ELSE part.amount / ABS(am.amount_total_signed) END as matched_percentage
                   FROM account_partial_reconcile part LEFT JOIN account_move_line aml ON aml.id = part.credit_move_id LEFT JOIN account_move am ON aml.move_id = am.id,\"account_account\", """ + tables + """
               ...
               SELECT aml.id, ref.date, aml.name,
                 CASE WHEN aml.debit > 0 THEN aml.debit_cash_basis ELSE 0 END AS debit_cash_basis,
                 CASE WHEN aml.credit > 0 THEN aml.credit_cash_basis ELSE 0 END AS credit_cash_basis,
                 aml.move_id, aml.account_id, aml.journal_id,
                 aml.balance_cash_basis AS balance_cash_basis,
                 aml.amount_residual, aml.partner_id, aml.reconciled, aml.company_id, aml.company_currency_id, aml.amount_currency, aml.balance, acc.user_type_id, aml.tax_line_id, aml.tax_exigible, aml.display_type
                FROM account_move_line aml
                RIGHT JOIN payment_table ref ON aml.move_id = ref.move_id LEFT JOIN account_account acc on aml.account_id = acc.id
                WHERE journal_id NOT IN (SELECT id FROM account_journal WHERE type in ('cash', 'bank'))
                  AND aml.move_id IN (SELECT DISTINCT move_id FROM account_move_line LEFT JOIN account_account ON account_move_line.account_id = account_account.id WHERE account_account.user_type_id IN %s)
              )
            ) """
    ...
    def set_context(self, options):
        ctx = super(GenericTaxReport, self).set_context(options)
