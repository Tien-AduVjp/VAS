...
class IrModuleModule(models.Model):
    ...
    def _update_translations(self, filter_lang=None, overwrite=False):
        ...
        """
        res = super(IrModuleModule, self)._update_translations(filter_lang, overwrite)
        ...
        self.env.cr.execute("""
           ...
           SELECT l.code, 'model', 'ir.actions.client,name', a.id, t.src, t.value, t.module, t.state
             FROM account_financial_dynamic_report r
             JOIN ir_act_client a ON (r.name = a.name)
             JOIN ir_translation t ON (t.res_id = r.id AND t.name = 'account.financial.dynamic.report,name')
