...
class ReportAccountFollowupReport(models.AbstractModel):
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        return super(ReportAccountFollowupReport, self).get_html(options, line_id=line_id, additional_context=additional_context)

    def get_pdf(self, options, minimal_layout=True):
        partner = self.env['res.partner'].browse(options.get('partner_id'))
        lang = partner.lang or self.env.user.lang or 'en_US'
        return super(ReportAccountFollowupReport, self.with_context(keep_summary=True, lang=lang)).get_pdf(options, minimal_layout=False)
    ...
    def get_report_value(self, partner, options):
        ...
        return {'name': self.get_report_name(), 'summary': self.get_report_manager(options).summary, 'company_name': self.env.company.name}

    def get_templates(self):
        templates = super(ReportAccountFollowupReport, self).get_templates()
        templates['main_template'] = 'to_account_reports.template_followup_report'
        templates['line_template'] = 'to_account_reports.line_template_followup_report'
        if self.env.context.get('print_mode') and not self.env.context.get('mail'):
            templates['main_template'] = 'to_account_reports.report_followup_letter'
        return templates

    def get_history(self, partner):
        return self.env['mail.message'].search([('subtype_id', '=', self.env.ref('to_account_reports.followup_logged_action').id), ('id', 'in', partner.message_ids.ids)], limit=5)
    ...
    def change_next_action(self, partner_id, date, note):
        ...
        partner.message_post(body=msg, subtype='to_account_reports.followup_logged_action')
        return True
    ...
    def send_email(self, options):
        ...
        if email and email.strip():
            ...
            msg_id = partner.message_post(body=msg, subtype='to_account_reports.followup_logged_action')
...
class AccountReportFollowupAll(models.AbstractModel):
    ...
    def get_templates(self):
        ...
        if self.env.context.get('print_mode') and not self.env.context.get('mail'):
            ...
            templates['main_template'] = 'to_account_reports.report_followup_all'

        templates['line_template'] = 'to_account_reports.line_template_followup_report'
        templates['search_template'] = 'to_account_reports.followup_search_template'
        return templates

    def get_lines(self, options, line_id=None):
        lines = []
        if self.env.context.get('print_mode'):
            return super(AccountReportFollowupAll, self).get_lines(options, line_id=line_id)
        for partner_id in options.get('partners_to_show'):
            options['partner_id'] = partner_id
            lines.append(super(AccountReportFollowupAll, self).get_lines(options, line_id=line_id))
        return lines
    ...
    def get_rp_informations(self, options):
        informations = super(AccountReportFollowupAll, self).get_rp_informations(options)
