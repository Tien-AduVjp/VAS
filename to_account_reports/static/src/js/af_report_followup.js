odoo.define('to_account_reports.af_report_followup_generic', function (require) {
'use strict';
...
var account_report = require('to_account_reports.af_report_generic');

var QWeb = core.qweb;

var af_report_followup_generic = account_report.extend({
    events: _.defaults({
        ...
        'click .o_af_reports_followup_skip': 'skip_partner',
        'click .o_af_reports_followup_done': 'done_partner',
        'click .o_af_reports_followup-auto': 'enable_auto',
        "change *[name='blocked']": 'on_change_blocked',
        'click .o_af_reports_followup-set-next-action': 'set_next_action',
    ...
    render: function() {
        if (this.rp_options.partners_to_show){
            this.renderPager();
    ...
    renderPager: function() {
        var self = this;
        var pager = new Pager(this, this.rp_options.total_pager, this.rp_options.pager, 1);
        pager.appendTo($('<div>')); // render the pager
        this.$pager = pager.$el;
        pager.on('pager_changed', this, function (state) {
            self.rp_options.pager = state.current_min;
            self.reload_report();
        });
        return this.$pager;
    },
    render_searchview: function() {
        this.$searchview = $(QWeb.render("toAccountReports.followupProgressbar", {options: this.rp_options}));
    },
    ...
    display_done: function(e) {
        $(e.target).parents('.o_af_reports_body').find("div.o_af_reports_page").find(".alert.alert-info.alert-dismissible").remove();
        $(e.target).parents('.o_af_reports_body').find('#action-buttons').addClass('o_af_reports_followup_clicked');
    ...
    send_followup_email: function(e) {
        ...
        this.rp_options['partner_id'] = partner_id;
        return this._rpc({
                model: this.rp_model,
                method: 'send_email',
                args: [this.rp_options],
            })
            .then(function (result) { // send the email server side
                self.display_done(e);
                $(e.target).parents("div.o_af_reports_page").prepend(QWeb.render("emailSent")); // If all went well, notify the user
            });
    },
    ...
    done_partner: function(e) {
        ...
        return this._rpc({
            ...
            .then(function () { // Update in db and restart report
                if (self.rp_options.progressbar) {
                    self.rp_options.progressbar[0] += 1;
                }
                self.reload_report();
            });
    },
    ...
    set_next_action: function(e) {
        ...
        nextActionDatePicker.appendTo($content.find('div.o_af_reports_next_action_date_picker'));
        ...
        $content.find('.o_af_reports_followup_next_action_date_button').bind('click', changeDate);

        var save = function () {
            var note = $content.find(".o_af_reports_next_action_note").val().replace(/\r?\n/g, '<br />').replace(/\s+/g, ' ');
            var date = nextActionDatePicker.getValue();
            var target_id = $content.find("#target_id").val();
            if (self.$el.find('.o_af_reports_followup-manual').hasClass('btn-default')){
    ...
    enable_auto: function(e) {
        ...
        if ($(e.target).parents('.o_af_reports_body').find('.o_af_reports_followup-auto:last').hasClass('btn-default')){
    ...
    toggle_auto_manual: function(e) {
        $(e.target).parents('.o_af_reports_body').find('.o_af_reports_followup-manual').toggleClass('btn-default btn-info'); // Change the highlighted buttons
        $(e.target).parents('.o_af_reports_body').find('.o_af_reports_followup-auto:last').toggleClass('btn-default btn-info'); // Change the highlighted buttons
    },
    ...
    skip_partner: function(e) {
        var partner_id = $(e.target).data('id');
        this.rp_options.skipped_partners.push(partner_id);
        this.reload_report();
    },
});
core.action_registry.add("af_report_followup_generic", af_report_followup_generic);
return af_report_followup_generic;
});