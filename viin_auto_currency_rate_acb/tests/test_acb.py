...
class ACBTest(RateTest):
    ...
    def _prepare_domain_get_currency_rate(self, date, currency, company):
        return [('name', '=', date), ('currency_id', '=', currency.id), ('company_id', '=', company.id)]

    def _get_currency_rate(self, date, currency, company=False):
        return self.env['res.currency.rate'].search(self._prepare_domain_get_currency_rate(date, currency, company or self.env.company), limit=1)
    ...
    def test_80_cron_curency_rate_multi_banks(self):
        ...
        try:
            self.bank_acb._cron_currency_rate_sync()
            rates_usd = self.env['res.currency.rate'].search(self._prepare_domain_get_currency_rate(date, self.currency_usd, self.company_vn))
