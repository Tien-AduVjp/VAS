...
class SaleSubscriptionLine(models.Model):
    ...
    uom_id = fields.Many2one('uom.uom', string='Unit of Measure', required=True, domain="[('category_id', '=', product_id.uom_id.category_id.id)]")
    ...
    def _onchange_uom_id(self):
        if not self.uom_id:
            ...
            self._onchange_product_quantity()
    ...
    def _amount_line_tax(self):
        ...
        for tax in product_tmp.taxes_id.filtered(lambda t: t.company_id.id == company_id):
            partner = self.subscription_id.partner_id
            fiscal_position_id = Position.with_company(company_id).get_fiscal_position(partner.id)
