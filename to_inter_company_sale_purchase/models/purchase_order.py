...
class PurchaseOrder(models.Model):
    ...
    def button_approve(self, force=False):
        ...
        for order in self:
            ...
            if (
                company_rec
                and company_rec.so_po_auto_validation == 'validated'
                and company_rec.intercompany_user_id
                and order.inter_comp_sale_order_id
                and not order.auto_generated
            ):
                order.inter_comp_sale_order_id.with_company(company_rec).with_user(company_rec.intercompany_user_id.id).action_confirm()
        return res
    ...
    def _generate_inter_comp_sale_order(self):
        ...
        company_partner = self.with_company(company).company_id.partner_id.with_user(inter_comp_user.id)
        if self.currency_id.id != company_partner.property_product_pricelist.currency_id.id:
            raise UserError(_("Could not generate sales order for the company of the vendor while the currency of vendor's sales pricelist differs from the purchase order's currency."
        ...
        SaleOrderLine = self.env['sale.order.line'].with_context(context)
