...
class WalletAdjust(models.TransientModel):
    ...
    account_id = fields.Many2one('account.account', string='Adjustment Account', required=True, domain="[('deprecated','=',False)]")

    def action_adjust(self):
        wallet = self.wallet_id
        if self.date:
            wallet = wallet.with_context(force_period_date=self.date)
        move = wallet._adjust(self.amount, self.journal_id, self.account_id)
