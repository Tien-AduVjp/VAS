...
class SaleOrder(models.Model):
    ...
    def _update_odoo_module_dependency_lines(self):
        ...
        last_sequence = lines._get_biggest_sequence()
        if missing:
            for product_id in missing:
                ...
                new_line = lines.new(ol_vals)
                new_line.product_id_change()
                lines += new_line

            self.with_context(no_update_module_dependencies=True).order_line = lines

        product_existed_in_order = self.order_line.filtered(lambda l: not float_is_zero(l.product_uom_qty, precision_digits=uom_precision)).sudo().product_id
        for line in self.order_line:
            if line.odoo_module_version_id:
                depended_product = line.odoo_module_version_id.depended_module_version_ids.mapped('product_id')
                if not (depended_product & product_existed_in_order) and line.odoo_app_dependency:
                    line.odoo_app_dependency = False
