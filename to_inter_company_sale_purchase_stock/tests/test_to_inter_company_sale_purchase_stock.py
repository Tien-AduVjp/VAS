...
class TestToInterCompanySalePurchaseStock(Common):
    ...
    def setUp(self):
        ...
        self.vendor_company_warehouse = self.env['stock.warehouse'].with_user(self.sale_manager).create({
            'name':'Vendor Inter Company Warehouse',
            'code':'OUT'           
        })
        self.customer_company_warehouse = self.env['stock.warehouse'].with_user(self.purchase_manager).create({
