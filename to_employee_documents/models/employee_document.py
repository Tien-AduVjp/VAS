...
class EmployeeDocument(models.Model):
    ...
    kept_by = fields.Selection([
        ('employee', 'Employee'),
        ('company', 'The Company')], required=True, default='employee', string='Kept by', tracking=True)
    ...
    def action_return_to_owner(self):
        for r in self:
            if r.kept_by != 'company':
                ...
                raise UserError(_("The document %s cannot be set to be kept by Employee as there is no need return back to the employee") % (r.name,))
            return self.write({'kept_by': 'employee'})
