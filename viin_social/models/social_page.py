...
class SocialPage(models.Model):
    ...
    _sql_constraints = [
        ('unique_social_social_page_id_social_page_url',
         'UNIQUE(social_page_id, social_page_url)',
         "You can not have two page with the same Page Id on Social and URL of Social!"),
    ]
    ...
    def write(self, vals):
        ...
        if 'active' in vals:
            if not vals['active']:
                ...
                self.env['mail.channel'].with_context(active_test=False).search([
                    ('channel_type', '=', 'social_chat'),
                    ('social_page_id.social_page_id', 'in', self.mapped('social_page_id'))
                ]).channel_last_seen_partner_ids.write({
                    'is_pinned': False
                })
