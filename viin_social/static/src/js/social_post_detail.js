...
var SocialPostDetail = Dialog.extend(ViinSocialEmojisMixin, {
	...
	events: {
		...
		'click .button_hide_comment': '_hideCommnet',
		'click .button_unhide_comment': '_unhideComment',
	...
	init: function(parent, options){
		...
		this.emojis = emojis;
		this.post_engagement = options.post_engagement
	},
	...
	synchronizePost: function() {
		...
		var div_modal_social_post_message = $('.modal_social_post_message');

		div_post_comment_count.text(this.post_engagement['comments_count']);
		div_post_like_count.text(this.post_engagement['likes_count']);
		div_post_share_count.text(this.post_engagement['shares_count']);
		div_modal_social_post_message.text(this.post_engagement['message']);
	},
	...
	_hideCommnet: function(ev) {
		...
		this._rpc({
			...
			if (data.success) {
				...
				current_comment.find('.social_comment_engagement').append(`
					<a class="button_unhide_comment text-primary font-weight-bold" href="#">Unhide</a>
				`);
			} else {
