...
#         ('cancel','Canceled')
    ...
    job_id = fields.Many2one('hr.job', string="Job Position", compute='_compute_job_id', store=True)
    ...
    amount = fields.Monetary(string="Total Amount", compute='_get_total_amount',
                          store=True, currency_field='currency_id')
    balance = fields.Monetary(string="Balance", compute='_get_balance', currency_field='currency_id', store=True, copy=False)
    move_id = fields.Many2one('account.move', string="Journal Entry", readonly=True, copy=False,
                              groups='account.group_account_user')
    move_lines = fields.One2many('account.move.line', related='move_id.line_ids', string="Journal Items", readonly=True, copy=False,
                                 groups='account.group_account_user')
    ...
    company_id = fields.Many2one('res.company', required=True, default=lambda self: self.env.company, readonly=False,
    ...
    journal_id = fields.Many2one('account.journal', string="Employee Advance Journal",
                             ...
                             context="{'default_is_advance_journal': True, 'default_type': 'cash', 'default_company_id': company_id}",
                             readonly=False,
                             states={'confirm': [('readonly', True)],
                                     'refuse': [('readonly', True)],
                                     'validate1': [('readonly', True)],
    ...
    def _onchange_employee_id(self):
        if self.employee_id and not self.employee_id.sudo().address_home_id:
            ...
            raise ValidationError(_("The employee '%s' you've selected has no Private Address specified. Please specify one for this employee first.")
                                  % (employee_id.name,))
        res = {}
        if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_approver'):
            res['domain'] = {'employee_id': [('user_id', '=', self.env.user.id)]}
        return res
    ...
    def _get_balance(self):
        for r in self:
            r.balance = r.amount - sum(x.amount for x in r.payment_ids.filtered(lambda line: line.state in ['posted', 'reconciled']))
    ...
    def _double_validation_required(self):
        ...
        if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_manager') and self.currency_id.compare_amounts(self.amount, self.company_id.currency_id._convert(self.company_id.amount_double_validation, self.currency_id, self.company_id, date)) >= 0:
            return True
        else:
            return False
    ...
    def action_approve(self):
        ...
        for r in self:
            if not is_manager:
                if not is_approver or not r.check_is_parent(r.employee_id):
    ...
    def action_validate(self):
        ...
        for r in self:
            ...
            if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_manager'):
                if r.state == 'validate1':
                    ...
                    if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_approver') or \
                        not self.check_is_parent(r.employee_id):
                        raise UserError(_("You must be an Employee Advance Approver and manager of employee %s.") % r.employee_id.name)
            ...
            if r.company_id.use_employee_advance_pass_through_account:
                AccountMove = self.env['account.move']
                move_lines = []
    
                different_currency = r.currency_id != r.company_id.currency_id
                total_credit = r.amount
                vals = {
                    'name': r.name,
                    'partner_id': r.employee_id.sudo().address_home_id.id,
                    'account_id': r.employee_id.sudo().address_home_id.property_account_payable_id.id,
                    'credit': total_credit,
                }
                if different_currency:
                    total_credit = r.currency_id._convert(r.amount, r.company_id.currency_id, r.company_id, r.date)
                    vals.update({
                        'credit': total_credit,
                ...
                for item in r.lines:
                    ...
                    if different_currency:
                        debit = r.currency_id._convert(item.amount, r.company_id.currency_id, r.company_id, r.date)
                        total_debit += debit
    
                        vals.update({
                            'debit': debit,
                            'amount_currency': item.amount,
                            'currency_id': r.currency_id.id,
                            })
                    move_lines.append((0, 0, vals))
                ...
                if different_currency:
                    ...
                    move_lines[-1][2].update({'debit': move_lines[-1][2].get('debit', 0) + diff})
    ...
    def action_refuse(self):
        ...
        for r in self:
            if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_manager'):
                if not self.env.user.has_group('to_hr_employee_advance.group_employee_advance_approver') or \
                    not r.check_is_parent(r.employee_id):
                    raise UserError(_("You must be an Employee Advance Approver and manager of employee %s.") % r.employee_id.name)

            if self.state not in ['confirm', 'validate', 'validate1', 'spent']:
                raise UserError(_('Advance request must be confirmed or validated in order to refuse it.'))

            if r.move_id:
                if r.move_id.state == 'draft':
                    r.move_id.unlink()
                elif r.move_id.state == 'posted':
    ...
    def check_is_parent(self, employee_id):
