...
class OdooVersion(models.Model):
    ...
    def action_view_git_branches(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('to_git.git_branch_action_window')

        # choose the view_mode accordingly
        branches_count = len(git_branch_ids)
        if branches_count != 1:
            action['domain'] = "[('odoo_version_id', 'in', " + str(self.ids) + ")]"
        elif branches_count == 1:
            res = self.env.ref('to_git_odoo_version.git_branch_form_view', False)
            action['views'] = [(res and res.id or False, 'form')]
            action['res_id'] = git_branch_ids.id
        return action
