...
class AccountJournal(models.Model):
    ...
    @api.model
    def _get_unearn_revenue_account(self, company_id):
        """
        :param company_id: integer
        :return: account record of an unearn revenue account
        :rtype: account.account
        """
        company_id = self.env['res.company'].browse(company_id)
        if company_id.property_unearn_revenue_account_id:
            return company_id.property_unearn_revenue_account_id
        return False

    @api.model
    def create(self, vals):
        unearn_revenue_acc = False
        if vals.get('voucher_payment', False):
            default_account = vals.get('default_debit_account_id') or vals.get('default_credit_account_id')
            if not default_account:
                unearn_revenue_acc = self._get_unearn_revenue_account(vals['company_id'])
                if unearn_revenue_acc:
                    vals['default_debit_account_id'] = unearn_revenue_acc.id
                    vals['default_credit_account_id'] = unearn_revenue_acc.id
