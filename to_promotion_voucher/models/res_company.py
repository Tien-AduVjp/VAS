...
class Company(models.Model):
    ...
    def _update_promotion_voucher_journal_if_exists(self):
        """ This function to find promotion_voucher journals and update their type to 'cash'. Because they has created with type is 'general' before. """
        self.ensure_one()
        Journal = self.env['account.journal'].sudo()
        journals = Journal.search([('name', '=', _('Promotion Voucher')),('code', '=', 'PVJ'), ('voucher_payment', '=', True), ('company_id', '=', self.id)])
        if not journals:
            journals = Journal.search([('code', '=', 'PVJ'), ('company_id', '=', self.id)])
            if journals:
                return journals.write({'voucher_payment': True, 'type': 'cash'})
            return False
        else:
