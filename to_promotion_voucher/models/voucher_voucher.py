...
class VoucherVoucher(models.Model):
    ...
    activated_date = fields.Date(string='Activated Date', readonly=True)
    issue_date = fields.Date(string='Issued Date', readonly=True)
    expiry_date = fields.Date(string='Expiry Date', readonly=True, compute='_compute_expiry_date', store=True)
    ...
    valid_duration = fields.Integer(string='Valid Duration', index=True, readonly=True, related='issue_order_id.valid_duration', store=True,
