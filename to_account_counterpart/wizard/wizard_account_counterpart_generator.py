...
class WizardAccountCounterpartGenerator(models.TransientModel):
    ...
    recreate_for_the_partially = fields.Boolean(string='Recreate for Partially Countered Entries',
                                                compute='_compute_recreate_for_the_partially', readonly=False, store=True)
    ...
    @api.depends('type')
    def _compute_recreate_for_the_partially(self):
        for r in self:
            r.recreate_for_the_partially = True if r.type == 'missing_only' else False
