...
class AccountMoveLineCTP(models.Model):
    ...
    dr_aml_id = fields.Many2one('account.move.line', string='Debit Move Line', required=True, ondelete='cascade',
                                tracking=True, domain=_get_dr_aml_domain)
    ...
    cr_aml_id = fields.Many2one('account.move.line', string='Credit Move Line', required=True, ondelete='cascade',
                                tracking=True, domain=_get_cr_aml_domain)
    ...
    countered_amt = fields.Monetary(string='Countered Amount', currency_field='company_currency_id', tracking=True,
                                   help="The amount in company's currency that matches the two lines in a counterpart operation.")

    countered_amt_currency = fields.Monetary(string='Countered Amount Currency', currency_field='currency_id', tracking=True,
    ...
    def _check_countered_amt(self):
        for r in self:
            ...
            if float_compare(r.countered_amt, max_countered_amt, precision_rounding=r.company_currency_id.rounding) == 1:
    ...
    def _compute_company_currency(self):
        for r in self:
            r.company_currency_id = r.dr_aml_id.company_currency_id or r.cr_aml_id.company_currency_id or self.env.company.currency_id
    ...
    def _check_cr_aml_id(self):
        for r in self:
            if float_is_zero(r.cr_aml_id.credit, precision_rounding=r.company_currency_id.rounding):
