...
class AccountMove(models.Model):
    ...
    def action_smart_create_counterpart(self):
        ...
        for r in self.filtered(lambda m: m.countered_status != 'fully'):
            ...
            # multiple debit lines and multiple credit lines, experimental
                ...
                for dr_line_id in dr_line_ids:
                    ...
                    dr_line_rounding = dr_line_id.company_currency_id.rounding

                    # try to match the next credit line which has same amount
                    if cr_line_ids and (cr_line_ids[0].id == dr_line_id.id - 1 or cr_line_ids[0].id == dr_line_id.id + 1) and float_compare(dr_line_id.debit, cr_line_ids[0].credit, precision_rounding=dr_line_rounding) == 0:
                        match = cr_line_ids[0]
                    ...
                    # try to match lines with same partner for the first
                        ...
                        match = cr_line_ids.filtered(lambda l: l.partner_id.id == dr_line_id.partner_id.id \
                                                     and float_compare(dr_line_id.debit, l.credit, precision_rounding=dr_line_rounding) == 0)
