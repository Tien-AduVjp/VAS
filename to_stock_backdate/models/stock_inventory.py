...
class StockInventory(models.Model):
    ...
    def action_validate(self):
        ...
        if self._context.get('open_stock_inventory_backdate_wizard', False)\
            and not self._context.get('manual_validate_date_time', False)\
            and self.env.user.has_group('to_stock_picking_backdate.group_stock_backdate') and not len(self) > 1:
        ...
        elif self.env.user.has_group('to_stock_picking_backdate.group_stock_backdate') and not len(self) > 1:
