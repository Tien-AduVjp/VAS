...
# Test functional
    ...
    def test_constrains_department_1(self):
        """
        Case 1: Both Employee and Grade have Department
            1. Adding grade and department for 1 employee
            2. Change another department
            Expect: Raise ValidationError
        """
        self.expert.write({'department_id': self.sales_dept.id})    
        with self.assertRaises(ValidationError):
            self.employeea.write({'grade_id': self.expert.id,'department_id': self.admin_dept.id})

    def test_constrains_department_2(self):
        """
        Case 2: Grade have Department, Employee don't have
            Adding department for Grade
            Expect: Error not raised
        """
        self.expert.write({'department_id': self.sales_dept.id})
        self.employeea.write({'grade_id': self.expert.id})

    def test_constrains_department_3(self):
        """
        Case 3: Employee have Department, Grade don't have
            Adding department for Employee
            Expect: Error not raised
        """
        self.employeea.write({'grade_id': self.expert.id, 'department_id': self.sales_dept.id})

    def test_constrains_department_4(self):
        """
        Case 4: Neither Employee nor Grade don't have Department
            Adding department for Employee and Grade
            Expect: Error not raised
        """
        self.employeea.write({'grade_id': self.expert.id})
