...
class MrpWorkorder(models.Model):
    ...
    def button_finish(self):
        ...
        if manual_validate_date_time:
            ...
            # after call button finish of super then state of self is done
            # so, we have to use SQL to write because write api do not write if state of self is done
            # this will not overide button_finish()
            self.env['mrp.workorder'].check_access_rights('write')
            self.env['mrp.workorder'].check_access_rule('write')
            
            # force orm call sql in origin button_finish func
            self.flush(['date_finished'], self)
            
            self.env.cr.execute("""
                UPDATE %s
                SET date_finished=%s, date_planned_finished=%s
                WHERE id in %s
                """, (
                    AsIs(self._table),
