...
class ResPartner(models.Model):
    ...
    property_hbd_email_template_id = fields.Many2one('mail.template', company_dependent=True, string='Happy Birthday Email Template', domain=[('model', '=', 'res.partner')])
    ...
    def cron_send_happy_birthday_email(self):
        ...
        for company in companies:
            partners_to_send = partners.with_company(company).filtered(
                lambda p: p.property_send_hbd_email \
                and p.property_hbd_email_template_id \
                and (not p.company_id or p.company_id == company) \
                and (not p.property_last_hbd_email_sent or p.property_last_hbd_email_sent != fields.Date.today())
                )
            partners_to_send.with_company(company)._send_happy_birthday_email()
