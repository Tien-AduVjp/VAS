...
class TestHrTimesheet(CommonTimesheetApproval):
    ...
    # Employees whose contracts are open and unmarked 'timesheet approve' (date_start < date < date_end)
        self.contract_1.write({'timesheet_approval': False})
    ...
    # User have no employees. create timesheet
        ...
        self.project.message_subscribe(partner_ids=self.user_employee_2.partner_id.ids)
        timesheet = self.env['account.analytic.line'].with_context(user_id=self.user_employee_2.id).create({
            'name': 'Timesheet Demo',
            'project_id': self.project.id,
            'unit_amount': 1,
        })
