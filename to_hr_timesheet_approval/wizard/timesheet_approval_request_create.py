...
class TimesheetApprovalRequestCreate(models.TransientModel):
    ...
    timesheet_line_ids = fields.Many2many('account.analytic.line', string='Timesheet Records', groups='hr_timesheet.group_hr_timesheet_user')
    ...
    def _check_timesheet_line_ids(self):
        for r in self:
            ...
            if employee_ids != self.env.user.employee_id and not self.user_has_groups('hr_timesheet.group_timesheet_manager'):
                child_of_user = self.env.user.employee_id.subordinate_ids
    ...
    def _prepare_approval_request_vals(self):
        ...
        self_sudo = self.sudo()
        return {
            'title': self_sudo.title,
            'employee_id': self_sudo.env.user.employee_id.id or self_sudo.employee_id.id,
            'timesheet_line_ids': [(6, 0, self_sudo.timesheet_line_ids.ids)],
            'approval_type_id': self_sudo.approval_type_id.id,
            'date': fields.Date.today(),
            'company_id': self_sudo.company_id.id,
            'currency_id': self_sudo.company_id.currency_id.id,
            'deadline': self_sudo.approval_type_id._get_deadline()
            }
