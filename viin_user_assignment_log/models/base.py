...
class Base(models.AbstractModel):
    ...
    def unlink(self):
        domain = self._get_user_assignments_domain()
        res = super(Base, self).unlink()
        if not self._context.get('no_assigment_log_unlink', False):
            self._delete_user_assignment_logs(domain)
        return res

    @after_commit
    @api.model
    def _delete_user_assignment_logs(self, domain):
        to_delete = self.env['user.assignment'].sudo().search(domain)
        if to_delete:
            to_delete.with_context(no_assigment_log_unlink=True).unlink()
    ...
    def _get_last_user_assignment_dict(self):
        ...
        for r in self:
            assignment_log_records = all_assignment_log_records.filtered(lambda l: l.res_id == r.id)
    ...
    def action_view_assignment_logs(self):
        ...
        action['domain'] = "[('id', 'in', %s)]" % str(assignments.ids)
        return action
