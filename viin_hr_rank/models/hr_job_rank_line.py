...
class HrJobRankLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        records._generate_ranks_if_not_exists()
        return records

    def _generate_ranks_if_not_exists(self):
        vals_list = self.filtered(lambda r: not r.rank_id)._prepare_hr_rank_vals_list()
        return self.env['hr.rank'].create(vals_list)

    def _prepare_hr_rank_vals_list(self):
        """Hook method for others to extend"""
        return [{
            'role_id': r.role_id.id,
            'grade_id': r.grade_id.id
            } for r in self]
