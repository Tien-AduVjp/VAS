...
class MaintenanceEquipment(models.Model):
    ...
    serial_no = fields.Char(compute='_get_serial_no', inverse='_set_serial_no', store=True, tracking=True)

    current_stock_location_id = fields.Many2one('stock.location', string='Current Stock Location', tracking=True,
                                                compute='compute_current_stock_location', store=True)
    ...
    def _set_serial_no(self):
        pass

    @api.depends('lot_id', 'lot_id.quant_ids.quantity')
    def compute_current_stock_location(self):
        for r in self:
            if r.lot_id:
