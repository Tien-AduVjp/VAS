...
class StockMoveLine(models.Model):
    ...
    def _prepare_equipment_data(self):
        ...
        return{
            ...
            'maintenance_team_id': maintenance_team_id.id if maintenance_team_id else False,
    ...
    def create_equipment(self):
        ...
        equipment_ids = self.env['maintenance.equipment'] if can_create else self.env['maintenance.equipment'].sudo()
        for line in self:
    ...
    def _action_done(self):
        ...
        for r in self.exists():
            if r.can_create_equipment:
                picking_type = r.picking_id.picking_type_id
                if picking_type.code == 'incoming' and not picking_type.use_create_lots:
                    raise UserError(_("You cannot create equipment from this picking since "
                                      "its operation '%s' does not allow Create New Lots/Serial Numbers!")
                                      % picking_type.name)
                if r.lot_id:
                    r.create_equipment()
        return res
