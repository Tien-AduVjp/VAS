...
class TestSurveyRecomputeResults(SavepointCase):
    ...
    def setUp(self):
        ...
        self.survey_userinput1 = self.env['survey.user_input'].create({
            'survey_id': self.survey1.id,
            'question_ids': [(6, 0, self.question1.ids)],
            'user_input_line_ids': [
                (0, 0, {
                    'question_id': self.question1.id,
                    'answer_type': 'suggestion',
                    'value_suggested': self.question1.labels_ids[1].id
                })
            ],
            'state': 'done'
        })
    ...
    def test_recompute_results(self):
        # case 1:
        self.assertEqual(self.survey_userinput1.quizz_score, 0)
        
        self.question1.labels_ids[2].write({
            'is_correct': False,
            'answer_score': 0
        })
        self.question1.labels_ids[1].write({
            'is_correct': True,
            'answer_score': 1
        })
    
        self.survey1.action_recompute_answer_score()
        self.assertEqual(self.survey_userinput1.quizz_score, 100)
