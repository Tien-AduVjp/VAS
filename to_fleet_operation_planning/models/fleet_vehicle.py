...
class FleetVehicle(models.Model):
    ...
    def _compute_access_partner_ids(self):
        ...
        for r in self:
            ...
            if active_trips:
                r.access_partner_ids = active_trips.mapped('driver_id.id')
            else:
                r.access_partner_ids = False

    def action_view_scheduled_trips(self):
        self.ensure_one()
        action = self.env.ref('to_fleet_operation_planning.fleet_confirmed_trip_action')
        ...
        result['context'] = {'default_vehicle_id':self.id}
        if self.driver_id:
            result['context']['driver_id'] = self.driver_id.id

        trip_ids = self.trip_ids.filtered(lambda t: t.state == 'confirmed')
        # choose the view_mode accordingly
        if len(trip_ids) > 1:
            result['domain'] = [('id','in', trip_ids.ids)]
        elif len(trip_ids) == 1:
            res = self.env.ref('to_fleet_operation_planning.fleet_trip_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = trip_ids[0].id
