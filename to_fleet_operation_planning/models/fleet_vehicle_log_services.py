...
class FleetVehicleLogServices(models.Model):
    ...
    trip_section_id = fields.Many2one('fleet.vehicle.trip.section', string='Trip Section',
                                      ...
                                      help="The section of the trip's route for which the service was logged.")
    ...
    trip_waypoint_id = fields.Many2one('fleet.vehicle.trip.waypoint', string='Trip Waypoint',
                                       domain="[('trip_id','=',trip_id)]",
                                       help="The waypoint of the trip's route for which the service was logged.")
    ...
    trip_id = fields.Many2one('fleet.vehicle.trip', string='Trip', compute='_compute_trip', readonly=False, store=True, index=True,
                              help="The trip on which the service was logged. This field is automatically computed base on the Trip Section and the Trip Waypoint")

    route_id = fields.Many2one('route.route', string="Route", related='trip_id.route_id', store=True, index=True, readonly=True,
                               help="The route on which the service was logged, computed based on the route of the corresponding trip.")
    ...
    def _check_constrains_trip_section_id_trip_waypoint_id(self):
        for r in self:
            if r.trip_section_id and r.trip_waypoint_id:
                raise ValidationError(_("You may register a service log for either a Trip Section or a Trip Waypoint. Not for both!"))
