...
class FleetVehicleTrip(models.Model):
    ...
    vehicle_id = fields.Many2one('fleet.vehicle', string='Vehicle', index=True, readonly=True, required=True, ondelete='restrict',
                                 states={'draft': [('readonly', False)]}, tracking=True,
                                 help="The vehicle, which can be changed upon starting the trip")
    driver_id = fields.Many2one('res.partner', string='Driver', readonly=True, ondelete='restrict', required=True,
                                states={'draft': [('readonly', False)]}, tracking=True,
    ...
    assistant_ids = fields.Many2many('res.partner', 'assistant_trip_rel', 'trip_id', 'assistant_id', string="Assistants",
                                     readonly=True, states={'draft': [('readonly', False)]},
                                     domain=[('is_company', '=', False)])
    route_id = fields.Many2one('route.route', string="Route", readonly=True, states={'draft': [('readonly', False)]},
    ...
    odometer_unit = fields.Selection(related='vehicle_id.odometer_unit', readonly=True, store=True)
    ...
    log_services_ids = fields.One2many('fleet.vehicle.log.services', 'trip_id', string='Services Logs')
    log_services_count = fields.Integer(string='Services Logs Count', compute='_compute_log_services_count', store=True)
    ...
    @api.depends('log_services_ids')
    def _compute_log_services_count(self):
        costs = self.env['fleet.vehicle.log.services'].read_group([('trip_id', 'in', self.ids)], ['trip_id'], ['trip_id'])
        mapped_data = dict([(c['trip_id'][0], c['trip_id_count']) for c in costs])
        for r in self:
            r.log_services_count = mapped_data.get(r.id, 0)
    ...
    def action_view_log_services(self):
