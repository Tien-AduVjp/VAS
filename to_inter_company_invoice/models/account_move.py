...
class AccountMove(models.Model):
    ...
    def _generate_inter_comp_invoice(self):
        ...
        invoice_vals = self.with_context(context).sudo()._prepare_inter_comp_invoice_data()
        invoice = self.with_context(context).with_user(inter_comp_user.id).create([invoice_vals])
    ...
    def _get_inter_comp_invoice_type(self):
        ...
        return types_map.get(self.type)
    ...
    def _get_inter_comp_journal_type(self):
        if self.type in ('out_invoice', 'out_refund'):
            ...
            raise Warning(_("Could not find a proper journal type for inter-company invoice generation from the invoice in type of %s")
                          % self.type)
        return journal_type
