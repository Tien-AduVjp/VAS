odoo.define('viin_account_reconciliation.ReconciliationModel', function(require) {
    "use strict";
    ...
    var StatementModel = BasicModel.extend({
        ...
        closeStatement: function () {
            ...
            return this._rpc({
                ...
                method: 'button_validate', // it was button_confirm_bank in Odoo 13
        ...
        _loadReconciliationModel: function(params) {
        	// TODO: rewrite this return Promise
            const def = $.Deferred();
            this._rpc({
                ...
                const rec_ids = _.map(reconcileModels, 'id');
                // Clean
                ...
                this._rpc({
                    ...
                    method: 'search_read',
                    domain: [['model_id', 'in', rec_ids]],
                }).then((reconcileModelLines) => {
                    let analyticTagIds = [];
                    // TODO: rewrite this to avoid similar variable names
                    ...
                    this._readAnalyticTags({res_ids: analyticTagIds}).then(() => {
                        for (const reconcileModel of reconcileModels) {
                            var analyticTagData = [];
                            var line_ids = reconcileModel.line_ids || [];
                            for (const line of line_ids) {
                                for (const tagId of line.analytic_tag_ids)
                                    analyticTagData.push([
                                        tagId,
                                        this.analyticTags[tagId].display_name,
                                    ]);
                                line.analytic_tag_ids = analyticTagData
                            }
                        }
        ...
        quickCreateProposition: function(handle, reconcileModelId) {
            ...
            for (const reconcileModelLineIndex in reconcileModel.line_ids) {
                ...
                if (reconcileModelLineIndex === "0") {
                    focus = self._formatQuickCreate(line, _.pick(reconcileModelLine, fields), reconcileModel);
                    focus.reconcileModelId = reconcileModelId;
                    line.reconciliation_proposition.push(focus);
                } else {
                    defs.push(
                        self._computeLine(line).then(() => {
                            const second_focus = self._formatQuickCreate(
                                line,
        ...
        updateProposition: function(handle, values) {
            ...
            // TODO: this if block may not be needed in Odoo 14
            // Recheck and remove it if possible
            /**
                        ...
                        var values_dict = {
                            ...
                            'counterpart_aml_dicts': _.map(
                                _.filter(props, function(prop) {
                                	// prop.already_paid now become prop.is_liquidity_line in Odoo 14
        ...
        _formatQuickCreate: function(line, values, reconcileModel) {
            ...
            switch (values.amount_type) {
                case 'percentage':
                    ...
                    break;
                case 'regex':
                    var matching = line.st_line.name.match(new RegExp(values.amount_string));
                    if (matching && matching.length == 2) {
                        matching = matching[1].replace(new RegExp('\\D' + reconcileModel.decimal_separator, 'g'), '');
                        matching = matching.replace(reconcileModel.decimal_separator, '.');
                ...
                switch (context.mode) {
                    ...
                    default:
                        ...
                        return self._rpc({
                            ...
                            .then(function(result) {
                                // Flatten the result
                                self.manualLines = [].concat(result.accounts, result.customers, result.suppliers, result.partners);
                                self.valuenow = 0;
                                self.valuemax = Object.keys(self.manualLines).length;
                                var lines = self.manualLines.slice(0, self.defaultDisplayQty);
                                self.pagerIndex = lines.length;
