...
class TestLoanL10nVn(TestLoanCommon):
    ...
    def setUpClass(cls, company=None):
        ...
        cls.env.user.groups_id = [(4, cls.env.ref('analytic.group_analytic_accounting').id), (4, cls.env.ref('analytic.group_analytic_tags').id)]
        cls.company = cls.env['res.company'].create({'name': 'Cty VietNam'})

        chart_template_vn = cls.env.ref('l10n_vn.vn_template', raise_if_not_found=False)
        chart_template_vn.try_loading(company=cls.company)
        cls.env.companies = [(4, cls.company.id)]
        ...
        cls.lending_loan_tag = cls.env.ref('l10n_vn_common.account_analytic_tag_interests_dividends_distributed_profits')
        cls.borrowing_loan_tag = cls.env.ref('l10n_vn_common.account_analytic_tag_borrowing_loan')
    ...
    def init_invoice(self, move_type):
        move_form = Form(self.env['account.move'].with_context(default_move_type=move_type,default_company_id=self.company.id))
    ...
    def test_check_journal_loan_borrow(self):
        self.assertRecordValues(self.journal_loan_borrow, [
            {
                'default_account_id': self.account_3411.id,
            }
        ])
    ...
    def test_check_journal_loan_lend(self):
        self.assertRecordValues(self.journal_loan_lend, [
            {
                'default_account_id': self.account_1283.id,
            }
        ])
    ...
    def test_auto_add_analytic_tag_borrowing_loan(self):
        order = self.create_loan_order('loan.borrowing.order', 100000000, self.interest_fixed_rate_type_month)
        self.assertEqual(order.analytic_tag_ids, self.borrowing_loan_tag)

    def test_auto_add_analytic_tag_lending_loan(self):
        order = self.create_loan_order('loan.lending.order', 100000000, self.interest_fixed_rate_type_month)
