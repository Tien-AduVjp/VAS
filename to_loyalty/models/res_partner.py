...
class ResPartner(models.Model):
    ...
    level_id = fields.Many2one('customer.level', string='Level', compute='_compute_customer_level', store=True, index=True,
                               tracking=True)
    ...
    def _compute_customer_level(self):
        ...
        CustomerLevel = self.env['customer.level']
        for r in self:
            level_id = CustomerLevel.search([('points', '<=', r.loyalty_points)], order='points desc', limit=1)
            if level_id:
                data = {
