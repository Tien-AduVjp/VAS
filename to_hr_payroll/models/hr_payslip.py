...
class HrPayslip(models.Model):
    ...
    # TODO: make this salary_cycle_id required in master/14+
    ...
    # TODO: remove contract_id in master/14 since we already have contract_ids
    ...
    def default_get(self, fields_list):
        ...
        if result.get('date_to', False):
            ...
            default_date_to = default_date_from + relativedelta(months=+1, day=1, days=-1)
        result.update({
    ...
    def _compute_company(self):
        for r in self:
            company_id = r.employee_id.company_id.id or r._context.get('force_company', r._context.get('default_company_id', r.env.company.id))
            r.company_id = company_id or r.env.company.id
    ...
    def _get_dependent_deduction(self):
        for r in self:
            r.dependent_deduction = r.personal_tax_rule_id and r.personal_tax_rule_id.dependent_tax_base_ded * r.employee_id.total_dependant or 0.0
            r.personal_tax_base_deduction = r.personal_tax_rule_id and r.personal_tax_rule_id.personal_tax_base_ded or 0.0
    ...
    @api.depends('contract_ids', 'thirteen_month_pay', 'thirteen_month_pay_year')
    ...
    @api.depends('contract_ids', 'date_from', 'thirteen_month_pay', 'thirteen_month_pay_year')
    ...
    def _check_thirteen_month_pay_year(self):
        for r in self.filtered(lambda rec: rec.thirteen_month_pay):
            self.env['to.base'].validate_year(r.thirteen_month_pay_year)
    ...
    def refund_sheet(self):
        for payslip in self:
            ...
            number = copied_payslip.number or self.env['ir.sequence'].with_context(force_company=copied_payslip.company_id.id).next_by_code('salary.slip')
    ...
    def compute_sheet(self):
        ...
        for payslip in self:
            if not payslip.number:
                number = self.env['ir.sequence'].with_context(force_company=payslip.company_id.id).next_by_code('salary.slip')
    ...
    def action_view_refunds(self):
        ...
        if len(refund_ids) != 1:
            result['domain'] = "[('refund_for_payslip_id', 'in', %s)]" % str(self.ids)
        elif len(refund_ids) == 1:
            res = self.env.ref('to_hr_payroll.view_hr_payslip_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = refund_ids.id
        return result

    def action_view_hr_working_month_calendar_lines(self):
        action = self.env.ref('to_hr_payroll.act_hr_working_month_calendar_line_view_list')
        result = action.read()[0]
        result['context'] = {}
        result['domain'] = "[('working_month_calendar_id.payslip_id','in',%s)]" % str(self.ids)
        return result
