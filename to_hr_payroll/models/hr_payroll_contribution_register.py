...
class HRPayrollContributionRegister(models.Model):
    ...
    computation_method = fields.Selection([
        ...
        ], string='Computation Method', readonly=False,
        ...
        help="- Half-up Duration Rounding: If worked duration of employee in month is greater or equal to a half of total days in month, contribution will be calculated.\n"
             "- Unpaid Days Limitation: If employee has total unpaid days less than the following limit in a month, contribution will be calculated.")  # TODO: Mark as required in master/14.0
    ...
    def action_change_base(self):
        if self._context.get('call_wizard'):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_change_base_action')
            result = action.read()[0]
            result['context'] = {'default_payroll_contribution_reg_ids': self.ids}
            return result
    ...
    def action_suspend(self):
        if self._context.get('call_wizard'):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_suspend_action')
            result = action.read()[0]
            result['context'] = {'default_payroll_contribution_reg_ids': self.ids}
            return result
        for r in self:
            ...
            r.current_history_id.write({
                'date_to': r.date_suspended
                })
            new_history = r._create_history(r.date_suspended + relativedelta(days=1), 'suspended')
            r.write({
                'current_history_id': new_history.id,
                'state': 'suspended',
                })
    ...
    def action_resume(self):
        if self._context.get('call_wizard'):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_resume_action')
            result = action.read()[0]
            result['context'] = {'default_payroll_contribution_reg_ids': self.ids}
            return result
        for r in self:
    ...
    def action_done(self):
        if self._context.get('call_wizard'):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_done_action')
            result = action.read()[0]
            result['context'] = {'default_payroll_contribution_reg_ids': self.ids}
            return result
