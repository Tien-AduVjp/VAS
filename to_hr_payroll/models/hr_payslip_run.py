...
class HrPayslipRun(models.Model):
    ...
    def action_view_contracts(self):
        contracts = self.mapped('slip_ids.contract_ids')
        action = self.env.ref('hr_contract.action_hr_contract')
        result = action.read()[0]
        result['context'] = {'search_default_group_by_state': 1}
        result['domain'] = "[('id', 'in', %s)]" % str(contracts.ids)
        return result

    def action_view_employees(self):
        employees = self.mapped('slip_ids.employee_id')
        action = self.env.ref('hr.open_view_employee_list_my')
        result = action.read()[0]
        result['context'] = {}
        result['domain'] = "[('id', 'in', %s)]" % str(employees.ids)
        return result
    ...
    def action_view_payslips(self):
        ...
        if self.payslips_count != 1:
            result['domain'] = "[('payslip_run_id', '=', %s)]" % str(self.id)
        elif self.payslips_count == 1:
            res = self.env.ref('to_hr_payroll.view_hr_payslip_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = self.slip_ids.id
        return result
    
    def action_view_payslip_lines(self):
        self.ensure_one()
        action = self.env.ref('to_hr_payroll.action_windows_hr_payslip_line')
        result = action.read()[0]
        # choose the view_mode accordingly
