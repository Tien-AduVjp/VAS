...
class HRPayrollContributionHistory(models.Model):
    ...
    def _prepare_hr_payslip_contribution_line_data(self, payslip, date_from, date_to):
        """
        Prepare data to create contribution history line on payslip
        TODO: remove the unnecessary 'payslip' param on master/14.0+
        """
    ...
    def action_edit_date_end(self):
        ...
        if self._context.get('call_wizard', False):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_edit_date_end_action')
            result = action.read()[0]
            return result
    
    # TODO: imp code for 14.0
    def action_edit_date_start(self):
        self.ensure_one()
        if self._context.get('call_wizard', False):
            action = self.env.ref('to_hr_payroll.hr_payroll_contrib_action_edit_date_start_action')
            result = action.read()[0]
            return result
