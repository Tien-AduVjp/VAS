...
class WebsiteSale(main.WebsiteSale):
    ...
    # Themes menu
    @http.route(['/apps/themes'], type='http', auth="public", website=True)
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        if request.website.dedicate_apps_store and odoo_module_versions:
            odoo_module_version = odoo_module_versions[0].sudo()
            return request.redirect('/apps/app/%s/%s' % (odoo_module_version.odoo_version_id.name, odoo_module_version.technical_name))
    ...
    @http.route(['/apps/upload'], type='http', auth='public', website=True)
    def upload(self):
        # not yet developed
        if not request.website.dedicate_apps_store:
            pass
