...
class TestAppStore(HttpCase):
    ...
    def test_button_visible_on_page_details(self):
        ...
        self.register_payment(order.invoice_ids[:1], 10000000)
        with MockRequest(self.env, website=self.website, context={'lang': 'en_US'}):
            # bought apps => show button download
            self.authenticate('portalapps1@abc.com', 'portalapps1')
            ...
            button = root.xpath("//a[@id='btn_module_download']")
            self.assertEqual(len(button), 1, 'Show button add to cart')
            self.assertEqual(button[0].attrib.get('href'), '/my/apps/download/%s' % self.module_5_v12.id, 'Show button add to cart')
            self.logout()
            ...
            self.assertEqual(res.status_code, 200)
            root = etree.fromstring(res.content.decode(), etree.HTMLParser())
            button = root.xpath("//button[@id='btn_module_add_to_cart']")
    ...
    def prepare_rating_values(self, module_version_id, rating_value):
        return {
    ...
    def rating_module(self, data):
        resp = self.opener.post(self.base_url + '/mail/chatter_post', data=data)
        return resp

    def test_rating_module(self):
        with MockRequest(self.env, website=self.website, context={'lang': 'en_US'}):
            # did not buy
            data = self.prepare_rating_values(self.module_2_v12.id, 10)
            rating_result = self.rating_module(data)
        ...
        # user bought app
        order = self.create_sale_order(self.partner_1, [self.module_2_v12])
        self.register_payment(order.invoice_ids[:1], 10000000)
        with MockRequest(self.env, website=self.website, context={'lang': 'en_US'}):
            data = self.prepare_rating_values(self.module_2_v12.id, 10)
            self.authenticate('portalapps1@abc.com', 'portalapps1')
