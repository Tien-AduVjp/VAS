...
class TestViinProjectKanBanStateNotification(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.project_1.kanban_state_notification = True
    ...
    def test_01_message_post(self, message_post):
        # case 1:
        self.task_1.with_user(self.user_demo).kanban_state = 'blocked'
        self.task_1.flush()
        self.task_1.env.cr.precommit.run()
    ...
    def test_02_message_post(self, message_post):
        # case 2:
        self.task_1.with_user(self.user_admin).kanban_state = 'blocked'
        self.task_1.flush()
        self.task_1.env.cr.precommit.run()
        message_post.assert_called()
    ...
    @patch('odoo.addons.mail.models.mail_thread.MailThread.message_post')
    def test_03_message_post(self, message_post):
        # case 3:
        with self.assertRaises(AssertionError):
            self.task_1.with_user(self.user_demo).kanban_state = 'normal'
            self.task_1.flush()
            self.task_1.env.cr.precommit.run()
            message_post.assert_called()

    @patch('odoo.addons.mail.models.mail_thread.MailThread.message_post')
    def test_04_message_post(self, message_post):
        # case 3:
        with self.assertRaises(AssertionError):
            self.task_1.with_user(self.user_demo).kanban_state = 'normal'
            self.task_1.with_user(self.user_demo).project_id.kanban_state_notification = False
            self.task_1.flush()
            self.task_1.env.cr.precommit.run()
            message_post.assert_called()
