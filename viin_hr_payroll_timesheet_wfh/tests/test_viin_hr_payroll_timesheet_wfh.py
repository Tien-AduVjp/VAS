...
class TestHrPayrollTimesheetWFH(Common):
    ...
    def test_31_payslips(self):
        ...
        self.assertRecordValues(payslip,
            [{
                'pow_timesheet_ids': pow_ts.ids,
                'pow_required_hours': 18.0,
                'pow_timesheet_hours': 9.0,
                'missing_pow_hours': 9.0
            }])
    ...
    def test_34_payslips(self):
        # prepare data
        self.worktrip_time_off_type = self.wfh_time_off_type.copy()
        self.worktrip_time_off_type.name = 'Work Trip'
        self.worktrip_time_off_type.project_required = True

        # Avoid overlap with other time off
        start_of_next_moth = fields.Date.start_of(self.today + relativedelta(months=3), 'month')
        end_of_next_moth = fields.Date.end_of(start_of_next_moth, 'month')
        start_week_in_next_month = fields.Date.start_of(start_of_next_moth, 'week')
        tz_name = self.user.employee_id.resource_calendar_id.tz
        worktrip_time_off_1 = self.hr_leave_obj.create({
            'name': 'Work Trip 1',
            'department_id': self.user.employee_id.department_id.id,
            'employee_id': self.user.employee_id.id,
            'holiday_status_id': self.worktrip_time_off_type.id,
            'date_from': self._local_to_utc(datetime.combine(start_week_in_next_month, time(8, 0, 0)), tz_name),
            'date_to': self._local_to_utc(datetime.combine(start_week_in_next_month, time(17, 0, 0)), tz_name),
            'timesheet_project_id': self.env.ref('project.project_project_2').id,
            'timesheet_task_id': self.env.ref('project.project_task_12').id,
        })
        worktrip_time_off_1.action_validate()
        ...
        payslip = self.env['hr.payslip'].create({
            ...
            'contract_id': self.user.employee_id.contract_ids[0].id,
            'company_id': self.env.company.id,
            'date_from': start_of_next_moth,
            'date_to': end_of_next_moth,
            'salary_cycle_id': self.env.company.salary_cycle_id.id,
        })
        ...
        self.assertRecordValues(
            ...
            [{
                ...
                'pow_timesheet_hours': 0.0,
                'missing_pow_hours': 0.0
            }])
