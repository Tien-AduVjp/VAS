...
def _compute_timesheet_for_pow(env):
    ...
    if pow_timeoff:
        ...
        finished = 0
        for candidates in env['to.base'].splittor(pow_timesheet_candidates, PREFETCH_MAX):
            to_rec_number = finished + PREFETCH_MAX
...
def _compute_hr_working_month_calendar_lines(env):
    ...
    finished = 0
    for lines in env['to.base'].splittor(hr_working_month_calendar_lines, PREFETCH_MAX):
        _logger.info(
