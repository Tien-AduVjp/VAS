...
class Common(SavepointCase):
    ...
    def create_invoices(cls, type_invoice, currency, value, date=date.today()):
        ...
        invoice_form = Form(cls.env['account.move'].with_context(default_type=type_invoice))
    ...
    @classmethod
    def create_payment(cls, type_payment, currency, amount, journal=False, dest_journal=False, date=date.today()):
        ...
        if type_payment == 'transfer':
            payment_form.destination_journal_id = dest_journal or cls.default_journal_bank
        else:
            payment_form.partner_id = cls.env.ref('base.res_partner_1')
        if not payment_form.hide_payment_method:
            payment_form.payment_method_id = cls.env.ref('account.account_payment_method_manual_out')
            if type_payment == 'inbound':
                payment_form.payment_method_id = cls.env.ref('account.account_payment_method_manual_in')
        payment_form.currency_id = currency
        payment_form.payment_date = date
        payment_form.amount = amount
        payment = payment_form.save()
        payment.post()
        if type_payment == 'transfer' and currency==cls.company_currency:
                return payment, payment.move_line_ids.filtered(lambda r: r.amount_currency > 0)
        return payment, payment.move_line_ids[-1:]
