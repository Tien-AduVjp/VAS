...
class Report_c200_s07dn(models.AbstractModel):
    _name = 'report.viin_l10n_vn_account_cash_book.report_c200_s07dn'
    _inherit = 'report.l10n_vn.common'
    _description = 'VietNam C200 S07dn Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        res = super(Report_c200_s07dn, self)._get_report_values(docids, data)
        if 'account_ids' in data['form']:
            res['account_ids'] = (', '.join(self.env['account.account'].browse(data['form']['account_ids']).mapped('code'))) or ''
        else:
    ...
    def group_by_account_id(self):
        ...
        for account_id, result in results.items():
            ...
            for aml in amls:
                line = aml
                line.sum_debit = sum_data[line.id][1]
                line.sum_credit = sum_data[line.id][2]
                line.sum_name = sum_data[line.id][3]
                accounts[account]['lines'].append(line)
        return accounts
    ...
    def _get_lines(self, data):
        ...
        for account in sorted_accounts:
            ...
            progress_credit = initial_credit
            for line in amls:
                progress_debit = progress_debit + line.sum_debit
                progress_credit = progress_credit + line.sum_credit
                name = line.sum_name
