...
class ReplenishmentRequest(models.Model):
    ...
    def _default_approval_user(self):
        return self.env.ref('to_procurement_requests.group_manager').users[0] or False

    def _default_approval_user_domain(self):
        return [('id', 'in', self.env.ref('to_procurement_requests.group_manager').users.ids)]

    name = fields.Char(string='Name', required=True, index=True, copy=False, default='New')
    warehouse_id = fields.Many2one('stock.warehouse', string='Warehouse', tracking=True, default=_default_warehouse_id,
                                   required=True, readonly=False, states={'waiting': [('readonly', True)],
                                                                          'approved': [('readonly', True)],
                                                                          'refused': [('readonly', True)],
                                                                          'done': [('readonly', True)],
                                                                          'cancelled': [('readonly', True)]},
                                   help="The warehouse for which the replenishment is.")
    line_ids = fields.One2many('replenishment.request.line', 'request_id', string='Replenishment Request Lines',
    ...
    procurement_group_ids = fields.One2many('procurement.group', 'replenishment_request_id', string='Procurement Group', store=True)
    procurement_groups_count = fields.Integer(string='Procurement Groups Count', compute='_compute_procurement_groups_count')

    picking_ids = fields.Many2many('stock.picking', 'replenishment_request_stock_picking_rel', 'request_id', 'picking_id', string='Transfers',
    ...
    def _compute_procurement_groups_count(self):
        data = self.env['procurement.group'].read_group(
            [('replenishment_request_id', 'in', self.ids)], ['replenishment_request_id'], ['replenishment_request_id'])
        mapped_data = dict([(dict_data['replenishment_request_id'][0], dict_data['replenishment_request_id_count']) for dict_data in data])
        for r in self:
    ...
    def _compute_picking_ids(self):
        ...
        for r in self:
            ...
            if r.state in ('approved', 'done') and picking_ids:
    ...
    def _check_responsible_id(self):
        approval_group_id = self.env.ref('to_procurement_requests.group_manager')
        for r in self:
            if r.responsible_id and not r.responsible_id.has_group('to_procurement_requests.group_manager'):
                raise ValidationError(_("%s is not the user who has proper access rights to approve the replenishment request %s."
    ...
    def _notify_manager(self):
        ...
        for r in self.filtered(lambda record: record.responsible_id and record.responsible_id != current_user):
            email_template_id = self.env.ref('to_procurement_requests.email_template_submit_notification')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('replenishment.request') or '/'
        return super(ReplenishmentRequest, self).create(vals_list)
    ...
    def action_draft(self):
        for r in self:
            if r.state not in ('refused', 'cancelled'):
                raise ValidationError(_("You cannot set the replenishment request %s to Draft state while it is neither Refused nor Cancelled!")
                                      % r.name)
    ...
    def action_confirm(self):
        for r in self:
            if r.state != 'draft':
                raise ValidationError(_("You cannot submit a replenishment request that is not in Draft state."))
    ...
    def action_approve(self):
        ...
        for r in self:
            if r.state not in ('refused', 'waiting'):
                raise ValidationError(_("You cannot approve a Replenishment Request whose state is neither Refused nor Waiting Approval"))
    ...
    def action_refuse(self):
        for r in self:
            if r.state != 'waiting':
                raise ValidationError(_("You cannot refuse a Replenishment Request that is not in Waiting Approval state."))
        return self.write({'state':'refused'})

    def action_done(self):
        for r in self:
            if r.state != 'approved':
                raise ValidationError(_("You cannot set the replenishment request %s as Done while its state is not Approved!")
                                      % r.name)
    ...
    def action_cancel(self):
        for r in self:
            if r.state not in ('refused', 'waiting'):
                raise ValidationError(_("You cannot cancel a Replenishment Request whose state is neither Refused nor Waiting Approval!"))
        return self.write({'state':'cancelled'})

    def action_view_procurement_groups(self):
        self.ensure_one()

        action = self.env.ref('to_procurement_requests.action_procurement_group_view')
        ...
        if self.procurement_groups_count > 1:
            result['domain'] = "[('replenishment_request_id', '=', %s)]" % self.id
    ...
    def unlink(self):
        for r in self:
            if r.state != 'draft':
                raise ValidationError(_("You cannot delete the replenishment request %s while it is not in Draft state.") % r.name)

            if self.env.user != r.create_uid and not self.env.user.has_group('to_procurement_requests.group_manager'):
                raise ValidationError(_("You are not allowed to delete the replenishment request %s."
                                        " You must be either the user who has created the request"
                                        " or the one who has %s access rights to do so")
                                        % (r.name, self.env.ref('to_procurement_requests.group_manager').display_name))
        return super(ReplenishmentRequest, self).unlink()
