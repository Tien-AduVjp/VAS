...
class TestTransitTransferWithRoute(TestTransitTransferCommon):
    ...
    def _create_purchase_order(self, product, quantity, unit_cost):
        ...
        self.env[res_dict['res_model']].browse(res_dict['res_id']).process()
        # Form(self.env[wiz_act['res_model']].browse(wiz_act['res_id'])).save().process()
    ...
    def test_10(self):
        ...
        resupply_route = sub_warehouse.resupply_route_ids.filtered(
            lambda sr: sr.name == 'My Sub Warehouse (San Francisco): Supply Product from YourCompany'
        )
        ...
        self.assertEqual(
            resupply_route.name,
            'My Sub Warehouse (San Francisco): Supply Product from YourCompany',
        )
        self.assertRecordValues(resupply_route, [{
            'name': 'My Sub Warehouse (San Francisco): Supply Product from YourCompany',
        ...
        self.env[second_result['res_model']].browse(second_result['res_id']).process()
