...
class AccountMoveLine(models.Model):
    ...
    def _prepare_vninvoice_line_data(self, sequence=None):
        """
        ...
        if self.display_type:
            if self.company_id.einvoice_api_version == 'v1':
                return False
            else:
                data.update({
                    'productType': 4
        ...
        if self.move_id.type == 'out_refund' and not self.reversed_move_line_id:
        ...
        if taxes_count == 1:
            ...
            if self.tax_ids.tax_group_id == self.env.ref('l10n_vn_c200.tax_group_exemption'):
                vatPercent = -1
        ...
        if self.move_id.type == 'out_refund':
            ...
            if self.company_id.einvoice_api_version == 'v1':
                price_unit = float_round(reversed_price_unit - price_unit, precision_digits=prec)
                quantity = float_round(
                    self.reversed_move_line_id.quantity - quantity,
                    precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure')
                    )
                price_subtotal = self.move_id.currency_id.round(self.reversed_move_line_id.price_subtotal - price_subtotal)
                price_total = self.move_id.currency_id.round(self.reversed_move_line_id.price_total - price_total)
    ...
    def _prepare_einvoice_line_data(self, sequence=None):
        ...
        if self.move_id.company_einvoice_provider == 'vninvoice':
            res = self._prepare_vninvoice_line_data(sequence)
        return res
    
    def _prepare_einvoice_line_name(self, einvoice_item_name=False):
        if self.move_id.company_einvoice_provider == 'vninvoice':
            einvoice_item_name = 'invoice_line_product'
        return super(AccountMoveLine, self)._prepare_einvoice_line_name(einvoice_item_name)
