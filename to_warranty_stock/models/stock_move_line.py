...
class StockMoveLine(models.Model):
    ...
    def _action_done(self):
        ...
        for ml in self.exists():
            if ml.lot_id:
                warranty_policy_ids = ml._get_warranty_policy_ids()
                for warranty_policy_id in warranty_policy_ids:
                    existed_warranty_claim_policy = ml.lot_id.warranty_claim_policy_ids.filtered(lambda x: x.product_milestone_id == warranty_policy_id.product_milestone_id \
                                                                                                 and x.apply_to == warranty_policy_id.apply_to)
                    if existed_warranty_claim_policy:
                        continue

                    vals = warranty_policy_id._prepare_warranty_claim_policy_data()
