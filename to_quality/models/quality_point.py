...
class QualityPoint(models.Model):
    ...
    team_id = fields.Many2one(
        ...
        default=lambda self: self._get_default_team_id(), required=True)
    ...
    def action_see_quality_checks(self):
        self.ensure_one()
        action = self.env.ref('to_quality.quality_check_action_main').read()[0]
    ...
    def action_see_spc_control(self):
        self.ensure_one()
        action = self.env.ref('to_quality.quality_check_action_spc').read()[0]
        if self.test_type == 'measure':
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        domain = []
        if name:
            domain = ['|', ('name', '=ilike', name + '%'), '|', ('name', operator, name), ('product_id.name', operator, name)]
