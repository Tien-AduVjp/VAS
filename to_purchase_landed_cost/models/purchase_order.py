...
class PurchaseOrder(models.Model):
    ...
    def _prepare_purchase_landed_cost_data(self, product_id):
        data = {
            ...
            'product_uom_qty': 1.0,
            'product_uom': product_id.uom_po_id and product_id.uom_po_id.id or product_id.uom_id.id,
            }
        return data
    ...
    def action_view_landed_cost_po(self):
        action = self.env.ref('to_purchase_landed_cost.landed_cost_purchase_order_action')
        ...
        if len(landed_cost_po_ids) > 1:
            result['domain'] = "[('id','in',[" + ','.join(map(str, landed_cost_po_ids)) + "])]"
        elif len(landed_cost_po_ids) == 1:
            res = self.env.ref('purchase.purchase_order_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = landed_cost_po_ids and landed_cost_po_ids[0] or False
        return result

    def action_view_landed_costs(self):
        action = self.env.ref('to_purchase_landed_cost.po_landed_cost_action')
        result = action.read()[0]
        ...
        if len(stock_landed_cost_ids) > 1:
            result['domain'] = "[('id','in',[" + ','.join(map(str, stock_landed_cost_ids)) + "])]"
        elif len(stock_landed_cost_ids) == 1:
            res = self.env.ref('stock_landed_cost.view_stock_landed_cost_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = stock_landed_cost_ids and stock_landed_cost_ids[0] or False
        return result
