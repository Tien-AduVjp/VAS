odoo.define('to_mrp_plm.set_cover_kanban', function (require) {
'use strict';
...
KanbanRecord.include({
    ...
    _onKanbanActionClicked: function (ev) {
        ...
        if (this.modelName === 'mrp.eco' && $(ev.currentTarget).data('type') === 'set_cover') {
            ...
            var domain = [['res_model', '=', 'mrp.eco'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']];
            this._rpc({
                    model: 'ir.attachment',
                    method: 'search_read',
                    domain: domain,
        ...
        function open_cover_images_dialog(attachment_ids) {
            ...
            var $content = $(QWeb.render("to_mrp_plm.SetCoverModal", {
                cover_id: coverID,
                attachment_ids: attachment_ids,
                widget: self
            }));
            var $imgs = $content.find('.o_kanban_mrp_plm_cover_image');
            ...
            dialog.opened().then(function () {
                ...
                $content.on('click', '.o_kanban_mrp_plm_cover_image', function (ev) {
                ...
                $content.on('dblclick', '.o_kanban_mrp_plm_cover_image', function (ev) {
