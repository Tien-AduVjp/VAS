...
class TestWallet(TestWalletCommon):
    ...
    def test_create_non_wallet_payment(self):
        ...
        receivable_move_line = payment.move_line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
    ...
    def test_reconcile_non_wallet_invoice_with_non_wallet_payment(self):
        ...
        payment = self.create_payment(amount=100000, wallet=False, invoice_ids=[(6, 0, invoice.ids)])
        ...
        payment_receivable_move_line = payment.move_line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
    ...
    def test_reconcile_wallet_invoice_with_lesser_wallet_payment(self):
        ...
        payment = self.create_payment(amount=100000, wallet=True, invoice_ids=[(6, 0, invoice.ids)])
    ...
    def test_reconcile_wallet_invoice_with_greater_wallet_payment(self):
        ...
        payment = self.create_payment(amount=200000, wallet=True, invoice_ids=[(6, 0, invoice.ids)])
    ...
    def test_reconcile_non_wallet_invoice_with_mix_wallet_payment(self):
        ...
        payment = self.create_payment(amount=100000, wallet=True, wallet_amount=50000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
    ...
    def test_reconcile_mix_wallet_invoice_with_wallet_payment(self):
        ...
        payment = self.create_payment(amount=150000, wallet=True, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 100000,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_1(self):
        ...
        payment = self.create_payment(amount=160000, wallet=True, wallet_amount=70000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_2(self):
        ...
        payment = self.create_payment(amount=160000, wallet=True, wallet_amount=60000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 40000,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_3(self):
        ...
        payment = self.create_payment(amount=160000, wallet=True, wallet_amount=50000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 50000,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_4(self):
        ...
        payment = self.create_payment(amount=200000, wallet=True, wallet_amount=100000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_5(self):
        ...
        payment = self.create_payment(amount=300000, wallet=True, wallet_amount=150000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_6(self):
        ...
        payment = self.create_payment(amount=160000, wallet=True, wallet_amount=80000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 40000,
        ...
        payment2 = self.create_payment(amount=50000, wallet=True, wallet_amount=25000, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
        ...
        payment_receivable_move_line = payment2.move_line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(payment_receivable_move_line, [{
            'amount_residual':-10000,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_7(self):
        ...
        payment_receivable_move_lines = (payment + payment2 + payment3).mapped('move_line_ids').filtered(
        ...
        lines.reconcile()
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        self.assertEqual(invoice2.invoice_payment_state, 'paid')
        self.assertEqual(invoice3.invoice_payment_state, 'paid')
        self.assertEqual(self.partner.commercial_partner_id.wallet_ids.amount, 0)
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_8(self):
        ...
        payment_receivable_move_lines = (payment + payment2 + payment3).move_line_ids.filtered(
            lambda r: r.account_id.internal_type == 'receivable')
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_currency_1(self):
        ...
        payment = self.create_payment(amount=160, wallet=True, wallet_amount=70, currency_id=self.currency_eur.id, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'not_paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 44,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_currency_2(self):
        ...
        payment = self.create_payment(amount=250, wallet=True, wallet_amount=125, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_currency_3(self):
        ...
        payment = self.create_payment(amount=120, wallet=True, wallet_amount=70, currency_id=self.currency_eur.id, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_reconcile_mix_wallet_invoice_with_mix_wallet_payment_currency_4(self):
        ...
        payment = self.create_payment(amount=6000000, wallet=True, wallet_amount=3000000, currency_id=self.currency_vnd.id, invoice_ids=[(6, 0, invoice.ids)])
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_full_reconcile_exchange_rate(self):
        ...
        payment = self.create_payment(amount=100, wallet=True, currency_id=self.currency_eur.id, invoice_ids=[(6, 0, invoice.ids)],
                                      payment_date=datetime.date(self.current_year + 1, 2, 1))
        self.assertEqual(invoice.invoice_payment_state, 'paid')
        invoice_receivable_move_line = invoice.line_ids.filtered(lambda r: r.account_id.internal_type == 'receivable')
        self.assertRecordValues(invoice_receivable_move_line, [{
            'amount_residual': 0,
    ...
    def test_mix_wallet_invoice_not_full_reconcile(self):
        ...
        payment = self.create_payment(amount=300000, wallet=True, invoice_ids=[(6, 0, invoice.ids)])
        # we expect no full reconcile here
        self.assertTrue(all(not line.full_reconcile_id for line in payment.move_line_ids))

        invoice2 = self.create_invoice(custom_line_vals_list=[{
            'price_unit': 200000,
            'wallet': True
        }])
        (invoice2.line_ids + payment.move_line_ids).filtered(lambda l: l.account_id.internal_type == 'receivable').reconcile()
        # we expect the invoice2 is paid but still no full reconcile here
        self.assertTrue(invoice2.invoice_payment_state == 'paid')
        self.assertTrue(all(not line.full_reconcile_id for line in payment.move_line_ids))

        payment2 = self.create_payment(amount=200000, wallet=False, invoice_ids=[(6, 0, invoice.ids)])
        
        receivable_lines = ((payment + payment2).mapped('move_line_ids') + (invoice + invoice2).mapped('line_ids'))\
        ...
        self.assertTrue(invoice.invoice_payment_state == 'paid')
    ...
    def test_reconcile_invoice_with_multiple_payments(self):
        ...
        payment1_receivable_move_lines = payment1.move_line_ids.filtered(lambda line: line.account_id.internal_type == 'receivable')
        payment2 = self.create_payment(amount=1999000, wallet=True)
        payment2_receivable_move_lines = payment2.move_line_ids.filtered(lambda line: line.account_id.internal_type == 'receivable')
        invoice = self.create_invoice(custom_line_vals_list=[{
            'price_unit': 1999000,
            'wallet': True
        }])
    ...
    def test_reconcile_multiple_invoices_with_multiple_payments(self):
        ...
        self.assertRecordValues(invoice1, [{'invoice_payment_state': 'paid'}])
        ...
        self.assertRecordValues(invoice2, [{'invoice_payment_state': 'paid'}])
        self.assertRecordValues(payment1_receivable_move_lines, [{'wallet_amount_residual': 0}])
    ...
    def test_compute_wallet_amount_3(self):
        payment = self.create_payment(wallet=True, currency_id=self.currency_vnd.id)
        payment.write({'state': 'draft', 'amount': 115})
        self.assertEqual(payment.wallet_amount, 115)
        ...
        payment.write({'invoice_ids': [(4, invoice.id)]})
        self.assertEqual(payment.wallet_amount, 111)

        payment.write({'currency_id': self.currency_eur.id})
        self.assertEqual(payment.wallet_amount, self.currency_vnd._convert(
            sum(invoice.mapped('wallet_total')),
            payment.currency_id,
            payment.company_id,
