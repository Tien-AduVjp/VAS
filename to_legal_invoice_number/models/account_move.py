...
class AccountMove(models.Model):
    ...
    def action_synch_legal_number(self):
        ...
        for r in self.filtered(lambda inv: inv.is_invoice(include_receipts=True) and inv.legal_number and inv.legal_number != inv.name):
            r.write({
                'old_number': r.name,
                'name': r.legal_number,
                'payment_reference': r.legal_number,
                })
