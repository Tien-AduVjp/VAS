...
#         return super(UoM, self).write(vals)
    ...
    def calculate_subscription_month_quantity(self, dt_start, dt_end, round=True, rounding_method='UP'):
        ...
        for dt in rrule(freq=MONTHLY, dtstart=dt_start, bymonthday=1, until=dt_end):
            if dt not in dates:
                dates.append(dt)
        ...
        for dt in dates:
            ...
            if diff.days >= uom_subscription_month.factor_inv:
                ...
                months += diff.days / uom_subscription_month.factor_inv
            hr = diff.seconds / 3600
            if hr > 0:
                months += uom_subscription_hour._compute_quantity(hr, uom_subscription_month, round=False)
            last_seen_dt = dt
