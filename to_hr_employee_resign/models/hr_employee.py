...
class HrEmployee(models.Model):
    ...
    def action_view_resignations(self):
        action = self.env.ref('to_hr_employee_resign.hr_employee_resignation_action')
        result = action.read()[0]

        # override the context to get rid of the default filtering
        result['context'] = {'employee_id': self.id, 'default_employee_id': self.id}

        # choose the view_mode accordingly
        if len(self.hr_employee_resignation_ids) != 1:
            result['domain'] = "[('id', 'in', " + str(self.hr_employee_resignation_ids.ids) + ")]"
            ...
            result['res_id'] = self.hr_employee_resignation_ids.id
        return result
