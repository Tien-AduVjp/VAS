...
from odoo.addons.viin_l10n_vn_accounting_sinvoice.models.einvoice_service import \
...
class TestSinvoice(TestBaseEinvoiceCommon):
    ...
    def setUpClass(cls, chart_template_ref='l10n_vn.vn_template'):
        ...
        with open(get_module_resource('viin_l10n_vn_accounting_sinvoice', 'tests', 'download_files',
        ...
        with open(get_module_resource('viin_l10n_vn_accounting_sinvoice', 'tests', 'download_files', 'representation_file_cancelled.pdf'),
                  'rb') as f:
        ...
        invoice.journal_id.account_einvoice_serial_id = cls.serial
        invoice.action_post()
        cls.invoice = invoice
        cls.einvoice_service.sudo().write({
            'company_id': cls.company.id,
            'sandbox_api_url': 'https://demo-sinvoice.viettel.vn:8443',
            'api_url': 'https://api-sinvoice.viettel.vn:443',
            'sandbox_username': '0201994665',
            'sandbox_password': '123456aA@',
    ...
    def run(self, result=None):
        with patch('odoo.addons.viin_l10n_vn_accounting_sinvoice.models.account_move.requests.post', self.mock_request_response):
    ...
    def test_01_check_data_after_setup_sinvoice(self):
        self.assertRecordValues(self.company, [
            {'einvoice_service_id': self.einvoice_service.id}])
        self.assertTrue(self.company_data['default_journal_sale'].einvoice_enabled)

    def test_02_sinvoice_api_url(self):
        self.assertEqual(self.einvoice_service._get_einvoice_api_url(), 'https://demo-sinvoice.viettel.vn:8443')
        self.einvoice_service.write({'mode': 'production'})
        self.assertEqual(self.einvoice_service._get_einvoice_api_url(), 'https://api-sinvoice.viettel.vn:443')
        self.einvoice_service.write({'api_url': 'https://api-sinvoice.viettel.vn:443//'})
        self.assertEqual(self.einvoice_service.get_sinvoice_create_url(),
    ...
    def test_06_sinvoice_synch_payment_status(self):
        ...
        payment = self.create_payment(self.invoice)
        self.assertRecordValues(self.invoice, [{
            'einvoice_state': 'paid',
        }])
    ...
    def test_07_download_file(self):
        ...
        self.assertTrue(self.invoice.einvoice_representation_pdf)
        self.assertTrue(self.invoice.einvoice_official_representation)
        self.assertEqual(self.invoice.einvoice_representation_pdf_filename, 'representation_file.pdf')
        self.assertEqual(self.invoice.einvoice_official_representation_filename, 'representation_file.zip')
        self.assertTrue(self.invoice.einvoice_converted_file)
        self.assertEqual(self.invoice.einvoice_converted_filename, 'converted_file.pdf')
    ...
    def test_09_cancel_sinvoice(self):
        ...
        self.assertEqual(self.invoice.einvoice_representation_pdf.decode(), self.representation_file_cancelled_pdf)
        self.assertEqual(self.invoice.einvoice_official_representation.decode(), self.representation_file_cancelled_zip)
        self.assertEqual(self.invoice.cancellation_record.decode(), self.representation_file_cancelled_pdf)
        self.assertEqual(self.invoice.einvoice_representation_pdf_filename, 'representation_file_cancelled.pdf')
        self.assertEqual(self.invoice.einvoice_official_representation_filename, 'representation_file_cancelled.zip')
    ...
    #     self.assertEqual(self.invoice.einvoice_official_representation, attachments[0].datas)
    ...
    def test_15_prepare_einvoice_lines_data(self):
        ...
        self.assertEqual(result, [
            {'lineNumber': 1, 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
            ...
            {'lineNumber': 3, 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
             'unitName': 'Đơn vị\n(Units)', 'itemCode': 'default_code_product_a', 'unitPrice': 1000.0, 'quantity': 1.0,
        ...
        self.assertEqual(result, [
            {'lineNumber': 1, 'itemName': '[default_code_product_b] product_b (product_b)', 'unitName': 'Tá\n(Dozens)',
             ...
             'itemTotalAmountWithoutTax': 200.0, 'taxPercentage': 10.0, 'taxAmount': 20.0, 'discount': 0,
             'itemDiscount': 0, 'itemTotalAmountWithTax': 220.0},
            {'lineNumber': 2, 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
             'unitName': 'Đơn vị\n(Units)', 'itemCode': 'default_code_product_a', 'unitPrice': 1000.0, 'quantity': 1.0,
             'itemTotalAmountWithoutTax': 1000.0, 'taxPercentage': -2, 'taxAmount': 0.0, 'discount': 0,
             'itemDiscount': 0, 'itemTotalAmountWithTax': 1000.0},
            {'lineNumber': 3, 'selection': 2, 'itemName': 'line note'},
            {'lineNumber': 4, 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
             'unitName': 'Đơn vị\n(Units)', 'itemCode': 'default_code_product_a', 'unitPrice': 1000.0, 'quantity': 1.0,
             'itemTotalAmountWithoutTax': 1000.0, 'taxPercentage': 10.0, 'taxAmount': 100.0, 'discount': 0,
             'itemDiscount': 0, 'itemTotalAmountWithTax': 1100.0},
        ])
    ...
    def test_16_prepare_einvoice_lines_data_2(self):
        ...
        self.assertEqual(result, [
            {'lineNumber': 1, 'itemCode': 'default_code_product_a',
             'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
             'unitName': 'Đơn vị\n(Units)', 'unitPrice': 1000.0, 'quantity': 1.0,
             'itemTotalAmountWithoutTax': 1000.0, 'taxPercentage': 10.0, 'taxAmount': 100.0, 'discount': 0,
             'itemDiscount': 0, 'itemTotalAmountWithTax': 1100.0},
            {'lineNumber': 2, 'itemCode': 'default_code_product_b', 'itemName': '[default_code_product_b] product_b (product_b)',
             'unitName': 'Tá\n(Dozens)',
             'unitPrice': 181.82, 'quantity': 1.0,
    ...
    def test_16_onchange_label_on_invoice_lines(self):
        self.maxDiff = None
        self.assertEqual(
            self.invoice.invoice_line_ids[0]._prepare_einvoice_line_name('invoice_line_name'),
            '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
        )
        self.assertEqual(
            self.invoice.invoice_line_ids[0]._prepare_einvoice_line_name('invoice_line_product'),
    ...
    def test_17_prepare_sinvoice_data_foreign_partner(self):
        ...
        self.assertEqual(result, {
            ...
            'sellerInfo':
                {
                    ...
                    'sellerAddressLine': 'Kỳ Sơn, Sơn, Kỳ',
            ...
            'itemInfo': [
                ...
                {'lineNumber': 2, 'itemCode': 'default_code_product_b',
                 'itemName': '[default_code_product_b] product_b (product_b)', 'unitName': 'Tá\n(Dozens)',
                 'unitPrice': 200.0, 'quantity': 1.0,
                 'itemTotalAmountWithoutTax': 200.0, 'taxPercentage': 10.0, 'taxAmount': 20.0, 'discount': 0,
                 'itemDiscount': 0, 'itemTotalAmountWithTax': 220.0},
                {'lineNumber': 3, 'itemCode': 'default_code_product_a',
                 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
                 'unitName': 'Đơn vị\n(Units)', 'unitPrice': 1000.0,
        ...
        self.assertDictEqual(result, {
            ...
            'itemInfo': [
                {'lineNumber': 1,
                 ...
                 'taxPercentage': 10.0,
                 'taxAmount': 100.0,
                 'adjustmentTaxAmount': 100.0,
                 'discount': 0,
                 'isIncreaseItem': False,
                 'itemTotalAmountAfterDiscount': 1100.0,
                 'itemDiscount': 0,
                ...
                {'lineNumber': 2,
                 ...
                 'taxPercentage': 10.0,
                 'taxAmount': 20.0,
                 'adjustmentTaxAmount': 20.0,
                 'discount': 0,
                 'isIncreaseItem': False,
                 'itemTotalAmountAfterDiscount': 220.0,
                 'itemDiscount': 0,
                 'itemTotalAmountWithTax': 220.0},
                {'lineNumber': 3,
                 'itemName': '[default_code_product_a] product_a_vi_VN; description; sale vi_VN (product_a; description; sale)',
                 'unitName': 'Đơn vị\n(Units)',
                 'itemCode': 'default_code_product_a',
