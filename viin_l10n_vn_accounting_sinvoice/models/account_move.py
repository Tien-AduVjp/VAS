...
from odoo.tools import format_date, float_round, float_repr
...
SINVOICE_TIMEOUT_V1 = 20000
SINVOICE_TIMEOUT_V2 = 60000
...
class AccountMove(models.Model):
    ...
    def _prepare_sinvoice_seller_bank(self):
        ...
        if self.einvoice_service_id.sinvoice_max_len_bank_account < len(bank_info['bank_account']) or self.einvoice_service_id.sinvoice_max_len_bank_name < len(bank_info['bank_name']):
    ...
    def _prepare_sinvoice_strIssueDate(self):
        ...
        return int(self.einvoice_invoice_date.timestamp()) * 1000
    ...
    def _get_sinvoice_presentation_data(self, file_type):
        ...
        request_data = {
            ...
            'templateCode': self._get_einvoice_template_name(),
            'fileType': file_type
        }
        if self.einvoice_api_version == 'v1':
            auth = self.einvoice_service_id.get_einvoice_auth_str()
            req = requests.post(
                self.einvoice_service_id.get_sinvoice_presentation_file_url(),
                data=json.dumps(request_data),
                headers={"Content-type": "application/json; charset=utf-8"},
                timeout=SINVOICE_TIMEOUT_V1,
                auth=auth
                )
        else:
            req = requests.post(
                ...
                data=json.dumps(request_data),
                headers={"Content-type": "application/json; charset=utf-8",
                         "Cookie": "access_token=%s"%self.einvoice_service_id._get_sinvoice_token()},
                timeout=SINVOICE_TIMEOUT_V2,
                )
    ...
    def get_sinvoice_representation_files(self):
        ...
        update_vals = {}
        attachments = []
        generate_attachment = self.company_id.einvoice_representation_file_as_attachment or self._context.get('force_einvoice_representation_file_as_attachment', False)
        try:
            for file_type in SINVOICE_REPRESENTATION_FILETYPE:
                ...
                if data['errorCode'] not in ('', None, 200):
    ...
    def get_einvoice_representation_files(self):
        if (
            self.einvoice_provider == 'sinvoice'
            and (not self.einvoice_representation_pdf
                 or not self.einvoice_official_representation
    ...
    def _ensure_sinvoice_representation_files(self, retries=0, sleep=3):
        ...
        """
        self.ensure_one()
        if not self.einvoice_representation_pdf or not self.einvoice_official_representation or self._context.get('refresh_einvoice_representation_file', False):
            self.get_sinvoice_representation_files()
        data = {
            'pdf': self.einvoice_representation_pdf,
            'zip': self.einvoice_official_representation
            }
    ...
    def _get_exchange_user(self):
        return self.einvoice_issue_user_id or self.env.user
    ...
    def _get_sinvoice_exchange_data(self):
        ...
        req_path = self.einvoice_service_id.get_sinvoice_exchange_file_url()
        request_data = {
            'supplierTaxCode' : self.company_id.vat,
            'templateCode': self._get_einvoice_template_name(),
            'invoiceNo' : self.legal_number,
            'strIssueDate' : self._prepare_sinvoice_strIssueDate(),
            'exchangeUser' : self._get_exchange_user().name, #todo consider who is exchangeUser?
            }
        if self.einvoice_api_version == 'v1':
            req = requests.post(
                req_path,
    ...
    def get_sinvoice_converted_files(self):
        self.ensure_one()
        update_vals = {}
        attachments = []
        try:
            ...
            if data['errorCode']:
                ...
                # if the company wants attachment
                if self.company_id.einvoice_exchange_file_as_attachment or self._context.get('force_einvoice_exchange_file_as_attachment', False):
                    req_content = base64.decodebytes(file_content.encode())
                    attachment = (filename, req_content)
                    attachments.append(attachment)
                else:
    ...
    def _ensure_sinvoice_converted_file(self, retries=0, sleep=3):
        ...
        """
        self.ensure_one()
        if not self.einvoice_converted_file or self._context.get('refresh_einvoice_converted_file', False):
            self.get_sinvoice_converted_files()

        if not self.einvoice_converted_file:
            if retries > 0:
                retries -= 1
                time.sleep(sleep)
    ...
    def _prepare_sinvoice_tax_breakdowns(self):
        ...
        for tax_line in invoice_tax:
            if tax_line['tax_group_id'] == exemption_group:
                tax_groups.append({
                    'taxPercentage': -2,
                    'taxableAmount': float(float_repr(self.currency_id.round(tax_line['amount']), precision_digits=self.currency_id.decimal_places)),
                    'taxAmount': 0
                    })
            else:
                tax_groups.append({
                        ...
                        'taxableAmount': float(float_repr(self.currency_id.round(tax_line['amount']), precision_digits=self.currency_id.decimal_places)),
                        'taxAmount': float(float_repr(self.currency_id.round(tax_line['amount_tax']), precision_digits=self.currency_id.decimal_places)),
                    })
        return tax_groups
    ...
    def _prepare_sinvoice_data(self):
        ...
        general_invoice_info = {
            ...
            'exchangeRate': float_round((1 / self.currency_id.with_context(date=date).rate),
                                        precision_digits=7),
            'invoiceSeries': self._get_account_einvoice_serial_name(),
            'templateCode': self._get_einvoice_template_name(),
            'invoiceType': self._get_account_einvoice_type_name(),
            ...
            'adjustmentType': '1',
            # Trạng thái thanh toán củadjustmentTypea hóa đơn
            # True: Đã thanh toán
            # False: Chưa thanh toán
            'paymentStatus': True if self.payment_state == 'paid' else False,
            'paymentType': 'TM/CK',
        ...
        if self.einvoice_api_version == 'v2':
            general_invoice_info.update({
                'invoiceIssuedDate': int(fields.Datetime.now().timestamp()) * 1000,
            })
        ...
        if self.move_type == 'out_refund':
            ...
            if self.einvoice_api_version == 'v2':
                general_invoice_info.update({
                    'originalInvoiceIssueDate': int(self.reversed_entry_id.einvoice_invoice_date.timestamp()) * 1000,
                    'additionalReferenceDate': int(date.timestamp()) * 1000
                    })
        ...
        return {
            ...
            'sellerInfo': {
                ...
                'sellerPhoneNumber': self.company_id.phone and self._phone_format(self.company_id.phone) or '',
    ...
    def _prepare_update_vals_after_issuing(self, returned_vals):
        ...
        if self.einvoice_service_id.provider == 'sinvoice':
            res.update({
                'einvoice_transactionid': returned_vals['result']['transactionID'] or self.access_token,
    ...
    def _issue_sinvoice(self, raise_error=True):
        ...
        error = False
        data = self._prepare_sinvoice_data()
        try:
            if self.einvoice_api_version == 'v1':
                req = requests.post(
                    self.einvoice_service_id.get_sinvoice_create_url(),
            ...
            if returned_vals['errorCode']:
                ...
                issue_date = data['generalInvoiceInfo'].get('invoiceIssuedDate', False)
                returned_vals['issue_date'] = datetime.datetime.fromtimestamp(issue_date/1000) if issue_date else False
    ...
    def _sinvoice_unpaid(self, raise_error=True):
        """
        This method will connect the S-Invoice system and set the status of the corresponding S-Invoice to Unpaid
        """
        self.ensure_one()
        req_data = {
            'supplierTaxCode': self.company_id.vat,
        ...
        try:
            if self.einvoice_api_version == 'v1':
                req = requests.post(
                    self.einvoice_service_id.get_sinvoice_cancel_payment_status_url(),
                    data=req_data,
    ...
    def _write(self, vals):
        ...
        if payment_state and payment_state != 'paid':
            for r in self.filtered(lambda inv: inv.einvoice_state == 'paid' and inv.einvoice_service_id.sinvoice_synch_payment_status and inv.einvoice_provider == 'sinvoice'):
                r._sinvoice_unpaid()
        return res
    ...
    def _sinvoice_paid(self, raise_error=True):
        ...
        try:
            if self.einvoice_api_version == 'v1':
                req = requests.post(
                    self.einvoice_service_id.get_sinvoice_update_payment_status_url(),
                    data=req_data,
                    headers={"Content-type": "application/x-www-form-urlencoded; charset=utf-8"},
                    timeout=SINVOICE_TIMEOUT_V1,
                    auth=self.einvoice_service_id.get_einvoice_auth_str()
    ...
    def action_invoice_paid(self):
        ...
        for r in self.filtered(lambda inv: inv.payment_state == 'paid' and inv.einvoice_state == 'issued' and inv.einvoice_service_id.sinvoice_synch_payment_status and inv.einvoice_provider == 'sinvoice'):
            r._sinvoice_paid()
    ...
    def _cancel_sinvoice(self, raise_error=True):
        ...
        if self.einvoice_api_version == 'v1':
            additional_ref_date_format = self.env['to.base'].convert_utc_time_to_tz(additional_ref_date, tz_name='Asia/Ho_Chi_Minh').strftime('%Y%m%d%H%M%S')
        else:
            additional_ref_date_format = int(additional_ref_date.timestamp()) * 1000
        req_data = {
            'supplierTaxCode': self.company_id.vat,
            'templateCode': self._get_einvoice_template_name(),
            'invoiceNo': self.legal_number,
            ...
            'additionalReferenceDate': additional_ref_date_format,
            }
        error = False
        ...
        try:
            if self.einvoice_api_version == 'v1':
                req = requests.post(
                    self.einvoice_service_id.get_sinvoice_cancel_url(),
                    data=req_data,
                    headers={"Content-type": "application/x-www-form-urlencoded; charset=utf-8"},
                    timeout=SINVOICE_TIMEOUT_V1,
                    auth=self.einvoice_service_id.get_einvoice_auth_str()
            ...
            error = True
            message = _("Could not cancel the S-Invoice %s %s.") % (self.legal_number, str(e.response.text))
            if e.response.status_code == 500:
                message += _(" You might have not configured allowed IPs in your S-Invoice portal to grant access from your Odoo instance's IP."
                             " Or there could be something wrong with your S-Invoice's username and/or password.")
        if error and raise_error:
    ...
    def action_sync_sinvoice_status(self):
        for r in self:
            if (
                ...
                or r.einvoice_provider != 'sinvoice'
                or not r.access_token
            ):
                continue
            api_url = r.einvoice_service_id.get_sinvoice_search_by_transaction_uuid_url()
                ...
                try:
                    if self.einvoice_api_version == 'v1':
                        req = requests.post(
                            ...
                            headers={"Content-type": "application/x-www-form-urlencoded; charset=utf-8"},
                            timeout=SINVOICE_TIMEOUT_V1,
                            auth=r.einvoice_service_id.get_einvoice_auth_str(),
                            )
                    else:
                        req = requests.post(
                            api_url,
    ...
    @api.model
    def _cron_sinvoice_invoice_status_sync(self):
        invoices = self.env['account.move'].search([
            ('einvoice_state', 'in', ('issued', 'paid', 'adjusted')),
            ('einvoice_provider', '=', 'sinvoice'),
            ('access_token', '!=', False),
        ])
        invoices._sinvoice_invoice_status_sync()
