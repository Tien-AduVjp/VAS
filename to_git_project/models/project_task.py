...
class ProjectTask(models.Model):
    ...
    def _onchange_git_repository_id(self):
        ...
        if self.git_repository_id:
            default = self.git_repository_id.get_default_branch()
            if default:
                self.git_branch_id = default
                res['domain'] = {'git_branch_id': [('repository_id', '=', self.git_repository_id.id)]}
            else:
                res['domain'] = {'git_branch_id': []}
        else:
