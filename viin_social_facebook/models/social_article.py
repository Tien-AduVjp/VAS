...
class SocialArticle(models.Model):
    ...
    schedule_later = fields.Boolean(string='Schedule later', default=False, help='Only for Facebook')
    schedule_date = fields.Datetime(string='Scheduled Post Date',
                                    help='You need to schedule the post within the next 1 hour to 70 days, since the post is confirmed by the user with approval permission.')
        ...
        for r in self:
            if r.display_facebook_preview:
                ...
                if r.attachment_type == 'none' and r.attachment_link:
                    ...
                    if data:
                        data_dict.update({'title': data.xpath("//meta[@property='og:title']")[0].attrib.get('content', False) if data.xpath("//meta[@property='og:title']") else r.attachment_link_title,
                    ...
                    data_dict.update({'images': [
                                            ...
                                            else base64.b64encode(open(image._full_path(image.store_fname), 'rb').read()) for image in r.attachment_ids]
                                    })
                r.facebook_preview = self.env.ref('viin_social_facebook.facebook_preview').render(data_dict)
