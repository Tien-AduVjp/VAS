...
class WarrantyClaim(models.Model):
    ...
    warranty_start_date = fields.Date(string='Warranty Start Date', tracking=True,
                                      compute='_compute_warranty_start_date', store=True,
                                      readonly=False, states={'disclaimed': [('readonly', True)],
                                                              'confirmed': [('readonly', True)],
                                                              'done': [('readonly', True)],
                                                              'cancelled': [('readonly', True)]})
    warranty_period = fields.Integer('Warranty period (months)', compute='_compute_warranty_period',
    ...
    def _compute_warranty_period(self):
        for r in self:
            if r.product_id.warranty_period:
                r.warranty_period = r.product_id.warranty_period
            else:
                r.warranty_period = False
    ...
    def _check_constrains_milestone(self):
        for r in self:
            policy_for_sales = r.mapped('warranty_claim_policy_ids').filtered(lambda p: p.apply_to == 'sale')
            policy_for_purchases = r.warranty_claim_policy_ids - policy_for_sales
    ...
    def _compute_warranty_claim_policy_ids(self):
        for r in self:
            warranty_claim_policies = r.env['warranty.claim.policy']
            if r.type == 'vendor':
                warranty_policies = r.product_id.warranty_policy_ids.filtered(lambda l: l.apply_to == 'purchase')
            elif r.type == 'customer':
                warranty_policies = r.product_id.warranty_policy_ids.filtered(lambda l: l.apply_to == 'sale')
            else:
                warranty_policies = False
            if warranty_policies:
                for warranty_policy_id in warranty_policies:
                    warranty_claim_policies += warranty_claim_policies.new(warranty_policy_id._prepare_warranty_claim_policy_data())
