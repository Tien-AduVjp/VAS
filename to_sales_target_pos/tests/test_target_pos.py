...
class TestTargetPos(TestTargetSaleCommon):
    ...
    def test_compute_sales_kpi_on_target(self):
        ...
        # Stage3
        self.pos_config.open_session_cb()
        order_salesman.refund()
        make_payment = Form(self.env['pos.make.payment'].with_context({'active_id': order_salesman.refund_order_ids.id}))
        payment = make_payment.save()
        payment.check()

        # Team target
        self.assertEqual(self.sale_target.non_invoiced_pos_sales_total, 0)
        self.assertEqual(self.sale_target.invoiced_pos_sales_total, leader_value_invoiced)
        self.assertEqual(self.sale_target.pos_sales_total, leader_value_invoiced)
        self.assertEqual(self.sale_target.pos_sales_target_reached, leader_value_invoiced * 100 / team_target)
        self.assertEqual(self.sale_target.sales_total, leader_value_invoiced + invoice_value)
        self.assertEqual(self.sale_target.target_reached, (leader_value_invoiced + invoice_value) * 100 / team_target)
        # Salesman target
        self.assertEqual(self.sale_target_saleman.non_invoiced_pos_sales_total, 0)
        self.assertEqual(self.sale_target_saleman.invoiced_pos_sales_total, 0)
        self.assertEqual(self.sale_target_saleman.pos_sales_total, 0)
        self.assertEqual(self.sale_target_saleman.pos_sales_target_reached, 0)
        self.assertEqual(self.sale_target_saleman.sales_total, invoice_value)
        self.assertEqual(self.sale_target_saleman.target_reached, invoice_value * 100 / saleman_target)
        # Leader target
        self.assertEqual(self.sale_target_leader.non_invoiced_pos_sales_total, 0)
        self.assertEqual(self.sale_target_leader.invoiced_pos_sales_total, leader_value_invoiced)
        self.assertEqual(self.sale_target_leader.pos_sales_total, leader_value_invoiced)
