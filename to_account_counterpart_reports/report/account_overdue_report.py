import time
from odoo import fields, models, api


class ReportOverdue(models.AbstractModel):
    _inherit = 'report.account.report_overdue'

    def _sub_ctp_select(self):
        return """
        SELECT
            rel.aml1_id AS org_aml_id,
            CASE WHEN l1.name != '/'
                THEN l1.name
                ELSE l1.ref
            END AS communication
        """

    def _get_ctp_move_line_communication_query(self):
        return """
        %s
        FROM account_move_line AS l1
        JOIN aml_counterpart_rel AS rel ON rel.aml2_id = l1.id
        """ % (self._sub_ctp_select(),)

    def _get_aml_query_select(self, date_str=None):
        date_str = date_str or fields.Date.today()

        select_str = """
        WITH ctp_comm AS (%s)
        SELECT m.name AS move_id, l.date, l.name, l.ref, l.date_maturity, l.partner_id, l.blocked, l.amount_currency, l.currency_id,
            CASE WHEN at.type = 'receivable'
                THEN SUM(l.debit)
                ELSE SUM(l.credit * -1)
            END AS debit,
            CASE WHEN at.type = 'receivable'
                THEN SUM(l.credit)
                ELSE SUM(l.debit * -1)
            END AS credit,
            CASE WHEN l.date_maturity < '%s'
                THEN SUM(l.debit - l.credit)
                ELSE 0
            END AS mat,
            string_agg(ctp_comm.communication,', ') AS communication
        """ % (self._get_ctp_move_line_communication_query(), date_str,)
        return select_str

    def _get_aml_query_from(self):
        return """
        FROM account_move_line l
        JOIN account_account_type at ON (l.user_type_id = at.id)
        JOIN account_move m ON (l.move_id = m.id)
        LEFT JOIN ctp_comm ON ctp_comm.org_aml_id = l.id
        """

    def _get_aml_query_where(self, partner_ids):
        return """
        WHERE l.partner_id IN (%s) AND at.type IN ('receivable', 'payable') AND l.full_reconcile_id IS NULL
        """ % (",".join([str(partner_id) for partner_id in partner_ids]),)

    def _get_aml_query_group_by(self):
        return """
        GROUP BY
            l.date,
            l.name,
            l.ref,
            l.date_maturity,
            l.partner_id,
            at.type,
            l.blocked,
            l.amount_currency,
            l.currency_id,
            l.move_id,
            m.name
        """

    def _get_account_move_lines(self, partner_ids):
        res = dict(map(lambda x:(x, []), partner_ids))
        query = """
        %s
        %s
        %s
        %s
        """ % (self._get_aml_query_select(), self._get_aml_query_from(), self._get_aml_query_where(partner_ids), self._get_aml_query_group_by())
        self.env.cr.execute(query)
        for row in self.env.cr.dictfetchall():
            res[row.pop('partner_id')].append(row)
        return res

    @api.model
    def _prepare_fleet_docargs(self, docids, data=None):
        totals = {}
        lines = self._get_account_move_lines(docids)
        lines_to_display = {}
        company_currency = self.env.company.currency_id
        for partner_id in docids:
            lines_to_display[partner_id] = {}
            totals[partner_id] = {}
            for line_tmp in lines[partner_id]:
                line = line_tmp.copy()
                currency = line['currency_id'] and self.env['res.currency'].browse(line['currency_id']) or company_currency
                if currency not in lines_to_display[partner_id]:
                    lines_to_display[partner_id][currency] = []
                    totals[partner_id][currency] = dict((fn, 0.0) for fn in ['due', 'paid', 'mat', 'total'])

                if line['debit'] and line['currency_id']:
                    line['debit'] = line['amount_currency']
                if line['credit'] and line['currency_id']:
                    line['credit'] = line['amount_currency']
                if line['mat'] and line['currency_id']:
                    line['mat'] = line['amount_currency']

                lines_to_display[partner_id][currency].append(line)
                if not line['blocked']:
                    totals[partner_id][currency]['due'] += line['debit']
                    totals[partner_id][currency]['paid'] += line['credit']
                    totals[partner_id][currency]['mat'] += line['mat']
                    totals[partner_id][currency]['total'] += line['debit'] - line['credit']

        return {
            'doc_ids': docids,
            'doc_model': 'res.partner',
            'docs': self.env['res.partner'].browse(docids),
            'time': time,
            'Lines': lines_to_display,
            'Totals': totals,
            'Date': fields.Date.today(),
        }

    @api.model
    def render_html(self, docids, data=None):
        docargs = self._prepare_fleet_docargs(docids, data)
        return self.env['report'].render('account.report_overdue', values=docargs)
