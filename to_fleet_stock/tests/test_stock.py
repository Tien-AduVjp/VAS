from odoo.exceptions import QWebException, ValidationError
...
class TestStock(Common):
    ...
    def test_generate_cost_by_done_picking(self):
        picking = self.create_fleet_consumption_picking(self.type_service, self.product, 2, self.vehicle)
        cost_generate = self.env['fleet.vehicle.cost'].search([('created_from_stock_stock_picking_id', '=', picking.id)])
        self.assertEqual(len(cost_generate), 1)
        self.assertEqual(cost_generate.cost_subtype_id, self.type_service)
        self.assertEqual(cost_generate.amount, self.product.standard_price * 2)
