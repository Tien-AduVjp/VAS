...
class AbstractLoanDisbursement(models.AbstractModel):
    ...
    state = fields.Selection(selection_add=[('refunded', 'Refunded'), ('cancelled', 'Cancelled')],
                             ondelete={'refunded': 'set default', 'cancelled': 'set default'},
    ...
    journal_id = fields.Many2one('account.journal', string='Journal', readonly=False, states={'confirmed': [('readonly', True)],
                                                                                              'paid': [('readonly', True)],
    ...
    last_interest_date_to = fields.Date(string="Last Interest Date", compute='_compute_last_interest_date_to', store=True,
    ...
    @api.depends('refund_line_ids.to_pay_amount')
    def _compute_to_refund_amount(self):
        for r in self:
            if r.refund_line_ids:
    ...
    def _check_total_disbursement(self):
        for r in self:
            disbursement_line_ids = r.order_id.disbursement_line_ids.filtered(lambda l: l.state in ('draft', 'cancelled'))
            total_disbursement = sum(disbursement_line_ids.mapped('amount'))
            if float_compare(total_disbursement, r.order_id.loan_amount, precision_rounding=r.currency_id.rounding) == 1:
                raise ValidationError(_("You must not have total disbursement amount that is greater that the contract's loan amount (%s)."
                                  " You were trying to add %s to make the total disbursement amount %s.")
                                  % (
                                      format_amount(r.env, r.order_id.loan_amount, r.currency_id),
                                      format_amount(r.env, r.amount, r.currency_id),
                                      format_amount(r.env, total_disbursement, r.currency_id)
                                      )
                                  )
    ...
    def _get_refund_line_model(self):
        raise ValidationError(_("The method `_get_refund_line_model()` has not been implemented for the model '%s' yet") % (self._name,))

    def _get_loan_interest_line_model(self):
        raise ValidationError(_("The method `_get_loan_interest_line_model()` has not been implemented for the model '%s' yet") % (self._name,))
        ...
        if not self.refund_line_ids:
            ...
            for refund_line_id in self.refund_line_ids:
                if refund_line_id.payment_match_ids:
                    for payment_match in self.mapped('refund_line_ids.payment_match_ids').filtered(lambda pm: pm.payment_id.payment_date <= date):
...
#                 amount -= refunded_amount
    ...
    def _generate_refund_lines(self):
        ...
        for r in self.filtered(lambda l: not float_is_zero(l.to_refund_amount, precision_rounding=l.currency_id.rounding)):
...
#                         continue
    ...
    def action_generate_interest(self):
        ...
        for r in self:
            ignored_time_frame = r.interest_line_ids.filtered(lambda l: l.state in ('confirmed', 'paid')).generate_timeframe()
            date_from = False
            for date in r._get_dates_for_interests():
                ...
                if not date_from:
                    ...
                    if date_from == r.order_id.find_last_date_of_period(date_from):
    ...
    def action_confirm(self):
        for r in self:
            if r.order_id.state != 'confirmed':
                raise UserError(_("You may not be able to confirm the disbursement '%s' while its contract (Ref.: %s) is not in"
    ...
    def action_paid(self):
        for r in self:
            if not float_compare(r.paid_amount, r.amount, r.currency_id.rounding) == 0:
                raise UserError(_("You may not mark a disbursement as Paid while its paid amount is not equal to its loan amount!"))
            if r.order_id.state != 'confirmed':
                raise UserError(_("You may not be able to set the disbursement '%s' Paid while its contract (Ref.: %s) is not in"
                                  " Confirmed state.") % (r.name, r.order_id.name))
...
#         self.action_generate_interest()
    ...
    def action_cancel(self):
        for r in self:
            ...
            if any(payment.state not in ('draft', 'cancelled') for payment in r.payment_ids):
                raise UserError(_("You may not be able to cancel the disbursement %s while its payments status is neither Draft nor Cancelled."
    ...
    def action_draft(self):
        for r in self:
            if r.order_id.state == 'cancelled':
                raise UserError(
                    _("Cannot set this disbursement plan line to draft because it linked to the contract %s in the 'Cancelled' state") % r.order_id.display_name)
        super(AbstractLoanDisbursement, self).action_draft()
