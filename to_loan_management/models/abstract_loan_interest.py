...
class AbstractLoanInterest(models.AbstractModel):
    ...
    state = fields.Selection(selection_add=[('cancelled', 'Canceled')], ondelete={'cancelled': 'set default'})
    ...
    loan_amount = fields.Monetary(string='Loan Amount', group_operator=False)
    ...
    daily_rate = fields.Float(string='Daily Rate(%)', digits=(26, 20), default=0.0, compute='_compute_daily_rate', store=True, group_operator=False)
    ...
    def _compute_fully_invoiced(self):
        for r in self:
            r.fully_invoiced = True if float_is_zero(r.to_invoice_amount, precision_rounding=(r.currency_id or self.env.company.currency_id).rounding) and r.state not in ('draft', 'cancel') else False
    ...
    def _prepare_inv_line_data(self, qty=1, raise_if_fully_invoiced=False):
        ...
        res = {
            ...
            'analytic_account_id': self.order_id.account_analytic_id.id,
    ...
    def _compute_nbr_of_days(self):
        ...
        for r in self:
            if not r.date_from or not r.date_to or not r.rate_type_id:
                ...
                elif r.rate_type_id.interest_rate_period == 'month' and r.order_id.interest_cycle == 'monthly' and nbr_of_days > days_of_month or is_full_month(nbr_of_days, date_from):
    ...
    def _compute_daily_rate(self):
        for r in self:
            ...
            # get rate
            rate = 0.0
            if r.rate_type_id.type == 'fixed':
                rate = r.rate_type_id.fixed_rate
