...
class LoanMixin(models.AbstractModel):
    ...
    name = fields.Char(string='Ref.', required=True, default='/', readonly=True)
    currency_id = fields.Many2one('res.currency', string='Currency', default=lambda self: self.env.company.currency_id)
    company_id = fields.Many2one('res.company', string='Company', readonly=False, states={'confirmed': [('readonly', False)],
                                                                                          'paid': [('readonly', False)]})
    date_maturity = fields.Date(string='Due Date', readonly=False, states={'confirmed': [('readonly', False)],
                                                                           'paid': [('readonly', False)]})
    date_confirmed = fields.Date(string='Date Confirmed', tracking=True, readonly=False, states={'confirmed': [('readonly', False)],
                                                                                                 'paid': [('readonly', False)]})
    ...
    def action_draft(self):
        for r in self:
            if r.state != 'cancelled':
                raise UserError(_("You may not  be able to set to draft the document '%s' which is not in cancelled state. Please cancel it first.") % r.display_name)
        self.write({
            'state':'draft',
