...
class LoanBorrowingOrder(models.Model):
    ...
    def get_default_journal(self):
        return self.env.company.loan_borrowing_journal_id
    ...
    def _check_loan_amount(self):
        for r in self:
            if sum(r.disbursement_line_ids.mapped('amount')) > r.loan_amount:
                raise ValidationError(_("You must not have the loan amount of the contract less than the total disbursement amount(%s).")
                                  % (
                                      format_amount(r.env, sum(r.disbursement_line_ids.mapped('amount')), r.currency_id),
                                      )
                                  )
