...
class AbstractLoanOrder(models.AbstractModel):
    ...
    def _default_currency_id(self):
        return self.get_default_journal().currency_id or self.env.company.currency_id
    ...
    name = fields.Char(string='Name',
                       ...
                       index=True, default='/')
    partner_id = fields.Many2one('res.partner', string='Loan Partner', required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                                              'done': [('readonly', True)],
                                                                                                              'cancelled': [('readonly', True)]})

    currency_id = fields.Many2one('res.currency', readonly=False, states={'confirmed': [('readonly', True)], 'done': [('readonly', True)], 'cancelled': [('readonly', True)]},
                                compute='_compute_currency', store=True, required=True, default=_default_currency_id)
    loan_amount = fields.Monetary(string='Loan Amount', store=True,
                                  required=True, readonly=False, states={'confirmed': [('readonly', True)], 'done': [('readonly', True)], 'cancelled': [('readonly', True)]})
    date_confirmed = fields.Date(string='Confirmation Date', default=fields.Date.today(), required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                                                                 'done': [('readonly', True)],
                                                                                                                                 'cancelled': [('readonly', True)]},
                                 copy=False,
                                 help='The date on which this order to be confirmed')
    date_end = fields.Date(string='Contract End', tracking=True, required=True,compute='_compute_date_end', store=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                                                                                'done': [('readonly', True)],
                                                                                                                                                'cancelled': [('readonly', True)]},
                           help="The date on which this contract will become expired",
                           default=lambda self: fields.Date.today() + relativedelta(months=12, days=-1), copy=False)
    disbursement_method = fields.Selection([
        ('number', 'By Number'),
        ('end', 'By End Date'),
        ('upon_request', 'Upon Request')], copy=False,
        string='Disbursement Method', default='number', required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                               'done': [('readonly', True)],
                                                                                               'cancelled': [('readonly', True)]},
    ...
    disbursement_start_date = fields.Date(string='Start Date', compute='_get_disbursement_start_date', store=True, copy=False,
    ...
    disbursement_method_number = fields.Integer(string='Disbursements Number', default=1,
                                                required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                       'done': [('readonly', True)],
                                                                                       'cancelled': [('readonly', True)]},
                                                help='Disburses times in number')
    disbursement_method_period = fields.Integer(string='Disbursement Period Months', default=1,
                                                required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                       'done': [('readonly', True)],
                                                                                       'cancelled': [('readonly', True)]},
                                                help='Number of months between two disbursements')
    disbursement_end_date = fields.Date(string='End Date', readonly=False, states={'confirmed': [('readonly', True)],
                                                                                   'done': [('readonly', True)],
                                                                                   'cancelled': [('readonly', True)]},
    ...
    principal_refund_method_number = fields.Integer(string='Refund Numbers', default=1,
                                                    required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                           ...
                                                                                           'cancelled': [('readonly', True)]},
                                                    help='')
    principal_refund_method_period = fields.Integer(string='Refund Period Months', default=1, required=True, readonly=False, states={'confirmed': [('readonly', True)],
                                                                                                                              'done': [('readonly', True)],
                                                                                                                              'cancelled': [('readonly', True)]})
    ...
    loan_duration = fields.Integer(string='Loan Duration', default=12, required=True,
                                       readonly=False, states={'confirmed': [('readonly', True)],
                                                               'done': [('readonly', True)],
                                                               'cancelled': [('readonly', True)]},
                                       help="The duration (in months) of the loan contract.", copy=False)
    ...
    account_analytic_id = fields.Many2one('account.analytic.account', string='Analytic Account', help="If specified, when invoicing interests of this contract,"
    ...
    _sql_constraints = [
        ('loan_amount_positive_check',
         'CHECK(loan_amount > 0.0)',
         "Loan amount less than or equal to 0 makes no sense. It must be greater than zero!"),
...
#             r.loan_amount = sum(r.disbursement_line_ids.mapped('amount'))
    ...
    def _compute_received_amount(self):
        for r in self:
            r.received_amount = sum(r.disbursement_line_ids.mapped('paid_amount'))
    ...
    def _check_disbursement_method_period_vs_disbursement_method_number(self):
        for r in self:
            if r.disbursement_method != 'upon_request' and r.disbursement_method_number > 1 and not r.disbursement_method_period > 0:
    ...
    def get_first_disbursement_date(self):
        if self.disbursement_line_ids and self.disbursement_line_ids[0].date:
            return self.disbursement_line_ids[0].date
        else:
            ...
            if r.disbursement_method == 'number':
                ...
                if r.disbursement_method_number == 1:
                    ...
                    if disbursement_end_dt:
                        disbursement_start_dt = disbursement_end_dt - relativedelta(months=(self.disbursement_method_number - 1) * self.disbursement_method_period)
    ...
    def _compute_date_end(self):
        for r in self:
            if r.disbursement_start_date and r.loan_duration:
                r.date_end = r.disbursement_start_date + relativedelta(months=+r.loan_duration, days=-1)
        ...
        for r in self:
            if r.principal_refund_method == 'end_contract':
                ...
                if r.principal_refund_method_number == 1:
                    ...
                    principal_refund_start_date = r.date_end - relativedelta(months= (r.principal_refund_method_number - 1) * r.principal_refund_method_period)
                    first_disbursement_date = r.get_first_disbursement_date()
    ...
    def action_compute_data_line(self):
        ...
        for r in self:
            if not regenerate and r.refund_line_ids and r.disbursement_line_ids:
                continue
            if r.disbursement_method != 'upon_request':
    ...
    def action_invoice(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_invoicing_wizard_action_xml_id())
    ...
    def _prepare_disbursement_line_data(self, amount, date_start, date_maturity):
        return {
            'order_id': self.id,
            'amount': amount,
            'date': date_start,
            ...
            if r.disbursement_method in ('number', 'end'):
                if r.disbursement_method_number > 0:
                    ...
                    for i in range(r.disbursement_method_number):
                        ...
                        vals_list.append(r._prepare_disbursement_line_data(amount, date_start, date_start))
                        new_date_start = date_start + relativedelta(months=r.disbursement_method_period)
                        date_start = new_date_start
            disbursement_line_ids = self.env[self._get_disbursement_model()].create(vals_list) | self.disbursement_line_ids
    ...
    def action_view_invoice(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_invoice_action_xml_id())
        if len(invoices) > 1:
    ...
    def action_view_refund_lines(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_refund_line_action_xml_id())
    ...
    def action_view_interest_lines(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_interest_line_action_xml_id())
    ...
    def action_disbursement_register_wizard(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_disbursement_payment_action_xml_id())
        ctx = ast.literal_eval(action['context'])
        ctx.update({'default_disbursement_ids': disbursement_ids.ids})
        action['context'] = ctx
        return action
    ...
    def copy(self, default=None):
        ...
        default['currency_id'] = self.journal_id.currency_id.id or self.env.company.currency_id.id
