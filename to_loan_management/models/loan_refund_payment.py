...
class LoanRefundPayment(models.Model):
    ...
    def reconcile_disbusement(self):
        ...
        for r in self:
            ...
            account = refund_ids.disbursement_id.account_id[:1]

            move_line_ids = r.move_id.line_ids.filtered(lambda line: line.account_id.reconcile \
                                                        and not line.full_reconcile_id \
                                                        and line.account_id == account)
            if not move_line_ids:
                continue
            if refund_ids:
                move_line_ids += refund_ids.mapped('disbursement_id.move_ids.line_ids').filtered(lambda line: line.account_id.reconcile \
                                                                                                 and not line.full_reconcile_id \
                                                                                                 and line.account_id == account)
