...
class LoanInterestRateType(models.Model):
    ...
    fixed_rate = fields.Float(string='Fixed Rate (%)', help='Fixed Rate for this interest type (%)')
    ...
    @api.constrains('fixed_days_of_month', 'fixed_days_of_year', 'days_of_month', 'days_of_year')
    def _check_fixed_days(self):
        for r in self:
            if r.fixed_days_of_year and r.days_of_year <= 0:
                raise UserError(_("You must set Days of Year being positive"))
            if r.fixed_days_of_month and r.days_of_month <= 0:
                raise UserError(_("You must set Days of Year being positive"))

    @api.constrains('product_id')
    def _check_product_id(self):
        for r in self:
            if r.product_id and not r.product_id.is_loan:
                raise ValidationError(_("Product '%s' is not marked as 'Can be Loan'. "
                                        "You can only select the product marked 'Can be Loan'. "
                                        "Please select another product or mark it as 'Can be Loan'.")
                                        % (r.product_id.name))
    ...
    def _get_display_name(self):
        ...
        if self.type == 'fixed':
            name = '%s%s %s %s' % (name, _("Fixed Rate"), str(self.fixed_rate) + '%', interest_rate_period_label)
        else:
