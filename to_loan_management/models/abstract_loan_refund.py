...
class AbstractLoanRefund(models.AbstractModel):
    ...
    state = fields.Selection(selection_add=[('cancelled', 'Cancelled')], ondelete={'cancelled': 'set default'},
    ...
    def _check_amount_vs_disbursement(self):
        for r in self:
            ...
            if float_compare(total_refund, r.disbursement_id.amount, precision_rounding=r.currency_id.rounding) == 1:
                raise ValidationError(_("You must not have total refund amount that is greater that the associated disbursement amount (%s)."
    ...
    def action_confirm(self):
        for r in self:
            if r.order_id.state != 'confirmed':
                raise UserError(_("You may not be able to confirm the refund '%s' while its contract (Ref.: %s) is not in"
                                  " Confirmed state.") % (r.name, r.order_id.name))
            if r.disbursement_id.state != 'paid':
                raise UserError(_("You may not be able to confirm the refund '%s' while the original disbursement (Ref.: %s) is not in"
                                  " Paid state.") % (r.name, r.disbursement_id.name))
            if r.date <= r.disbursement_id.date:
                raise UserError(_("You may not be able to confirm the refund %s while the date of which is earlier than its associated disbursement's date") % (r.name,))
    ...
    def action_paid(self):
        ...
        disbursement_ids.filtered(lambda d: float_is_zero(d.to_refund_amount, precision_rounding=d.currency_id.rounding)).action_refund()
    ...
    def action_cancel(self):
        for r in self:
            if any(payment.state not in ('draft', 'cancelled') for payment in r.payment_ids):
                raise UserError(_("You may not be able to cancel the refund %s while its payments status is neither Draft nor Cancelled."
    ...
    def _get_refund_payment_action_xml_id(self):
        raise ValidationError(_("The method `_get_refund_payment_action_xml_id()` has not been implemented for the model '%s' yet") % (self._name,))

    def action_refund_register_wizard(self):
        refund_ids = self.filtered(lambda l:l.state == 'confirmed')
        action = self.env['ir.actions.act_window']._for_xml_id(self._get_refund_payment_action_xml_id())
