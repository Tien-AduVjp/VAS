...
class HrSalaryRule(models.Model):
    ...
    generate_account_move_line = fields.Boolean(string='Generate Journal Item', tracking=True,
                                               ...
                                               " account of the rule.")
    employee_journal_item = fields.Boolean(string='Force Encoding with Employee', tracking=True, default=False,
                                            help="If enabled, accounting journal items generated by this rule will always be encoded"
                                            " with the private address of the employee as accounting partner, no matter the"
                                            " company's Payslip Batch Journal Entry mode is enabled or disabled.\n"
                                            "In other words, all the payslips of a batch would be"
                                            " consolidated in a shared single account journal entry whose journal items had no"
                                            " employee information except the ones generated by this rule and the others that"
                                            " also have Force Encoding with Employee enabled.")
    ...
    @api.constrains('employee_journal_item', 'register_id')
    def _check_register_employee_journal_item(self):
        for r in self:
            if r.employee_journal_item and r.register_id.partner_id:
                raise ValidationError(_("The rule `%s` must not have both Force Encoding with Employee and Contribution Register `%s` with partner set."
                                        " Please disable either.") % (r.display_name, r.register_id.display_name))
