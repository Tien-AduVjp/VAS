...
class HrMealOrder(models.Model):
    ...
    def _get_default_kitchen(self):
        default_kitchen = self.env.ref('to_hr_meal.main_kitchen')
    ...
    order_lines_count = fields.Integer('Number of Lines', tracking=True, compute='_count_order_lines', store=True)
    total_qty = fields.Integer('Total Quantity', tracking=True, compute='_compute_total', store=True)
    total_price = fields.Float('Total Price', tracking=True, compute='_compute_total', store=True)
    ...
    def _prepare_new_lines(self, employees):
        ...
        for employee in employees:
            if employee in self.order_line_ids.mapped('employee_id') or not employee.order_meal:
                continue
            emp_absent = self.env['hr.leave'].sudo().search([('employee_id', '=', employee.id),
    ...
    def _onchange_department_id(self):
        if self.department_id:
            ...
            self.order_line_ids += self._prepare_new_lines(self.department_id.member_ids)
            self.department_id = False
            self.load_employee = False

    @api.onchange('load_employee', 'company_id')
    ...
    def is_manager(self):
        return self.env.user.has_group('to_hr_meal.group_hr_meal_manager') and True or False
    ...
    def action_view_partners(self):
        ...
        action = self.env.ref('to_hr_meal.action_partners')
