...
class TestWebsiteAffiliate(HttpCase):
    ...
    def test_website_affiliate_flow(self):
        ...
        self.assertEqual(re.sub('(\t)|(\n)', '', content[0].text), 'There are currently no affiliate invoices for your account.')
    ...
    def test_01_link_tracker_same_url_campaign_id_medium_source_without_affiliate_id(self):
        vals = {
                'title': 'Test link tracker',
                'url': 'http://localhost:8014/web#id=&action=438&model=link.tracker&view_type=form&cids=&menu_id=101',
                'campaign_id': self.env.ref('utm.utm_campaign_fall_drive').id,
                'medium_id': self.env.ref('utm.utm_medium_website').id,
                'source_id': self.env.ref('utm.utm_source_facebook').id,
        }
        link1 = self.env['link.tracker'].create(vals)
        # Don't create a new link tracker if same (url, campaign, medium and source)
        link2 = self.env['link.tracker'].create(vals)
        self.assertEqual(link1.id, link2.id)

        vals['source_id'] = self.env.ref('utm.utm_source_search_engine').id
        link3 = self.env['link.tracker'].create(vals)
        # Error when same (url, campaign, medium and source)
        with self.assertRaises(UserError):
            link3.write({'source_id': self.env.ref('utm.utm_source_facebook').id})

    def test_02_link_tracker_same_url_campaign_id_medium_source_with_affiliate_id(self):
        aff_code = self.env['affiliate.code'].create({
            'partner_id': self.user_portal.partner_id.id,
            'company_id': self.user_portal.company_id.id,
        })
        vals = {
            'title': 'Test link tracker',
            'url': 'http://localhost:8014/web#id=&action=438&model=link.tracker&view_type=form&cids=&menu_id=101',
    ...
    def test_03_link_tracker_diff_url_campaign_id_medium_source(self):
        vals = {
                'title': 'Test link tracker',
                'url': 'http://localhost:8014/web#id=&action=438&model=link.tracker&view_type=form&cids=&menu_id=101',
                'campaign_id': self.env.ref('utm.utm_campaign_fall_drive').id,
    ...
    def test_04_link_tracker_diff_url_campaign_id_medium_source_with_affiliate_id(self):
        aff_code = self.env['affiliate.code'].create({
            'partner_id': self.user_portal.partner_id.id,
            'company_id': self.user_portal.company_id.id,
        })
        ...
        vals['affiliate_code_id'] = aff_code.id
        link2 = self.env['link.tracker'].create(vals)
        self.assertNotEqual(link1.id, link2.id)

        aff_code = self.env['affiliate.code'].create({
            'partner_id': self.env.ref('base.partner_demo').id,
            'company_id': self.env.ref('base.partner_demo').company_id.id,
        })
        vals['affiliate_code_id'] = aff_code.id
        link3 = self.env['link.tracker'].create(vals)
        self.assertNotEqual(link2.id, link3.id)
