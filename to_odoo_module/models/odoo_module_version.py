...
class OdooModuleVersion(models.Model):
    ...
    zipped_source_code = fields.Binary(string='Zipped Source Code', attachment=True, odlname='file',
    ...
    index_html = fields.Html(string='HTML Description', translate=html_translate, sanitize_attributes=False,
    ...
    def unlink(self):
        ...
        ProductTemplateAttributeLine = self.env['product.template.attribute.line']  # this was self.env['product.attribute.line'] in Odoo 11
        for product_tmpl in self.mapped('product_tmpl_id'):
            # search for attribute line that is Odoo Version line
            odoo_version_attribute_line = ProductTemplateAttributeLine.search([
                ('product_tmpl_id', '=', product_tmpl.id),
                ('attribute_id', '=', self.env.ref('to_product_odoo_version.product_attribute_odoo_version').id)], limit=1)
            ...
            # note that, if the variant is being referred by another record, Odoo will ignore without any error raised
                ...
                for product_attribute_value_id in product_attribute_values:
                    try:
                        with self._cr.savepoint(), tools.mute_logger('odoo.sql_db'):
                            product_tmpl.write({
                                'attribute_line_ids':[(1, odoo_version_attribute_line.id, {
                                    'value_ids':[(3, product_attribute_value_id.id)]
                                    })]
                                })
                    except Exception as e:
                        # set inactive
                        module_versions = to_process_odoo_module_versions.filtered(lambda mv: mv.odoo_version_id == product_attribute_value_id.odoo_version_id)
                        module_versions.filtered(lambda mv: mv.active).toggle_active()
