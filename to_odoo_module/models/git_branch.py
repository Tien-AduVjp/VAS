...
class GitBranch(models.Model):
    ...
    def _parse_manifest(self, manifest_file_name):
        ...
        if 'license' in vals:
            LicenseVersion = self.env['product.license.version']
            license_version_id = LicenseVersion.search([('short_name', '=', vals['license'])], limit=1)
            if not license_version_id:
                license_version_id = LicenseVersion.search([('name', '=', vals['license'])], limit=1)
            if not license_version_id:
                license_version_id = LicenseVersion.search([('name', 'ilike', vals['license'])], limit=1)
            if not license_version_id:
                license_version_id = LicenseVersion.search([('name', 'ilike', '%' + vals['license'] + '%')], limit=1)
            if license_version_id:
                vals['license_version_id'] = license_version_id.id
            else:
                vals['license_version_id'] = self.env.ref('to_product_license.license_version_lgpl_v3').id
        else:
            branch_version_id = self.get_license_version()
            vals['license_version_id'] = branch_version_id and branch_version_id.id or self.env.ref('to_product_license.license_version_opl_v1').id
    ...
    def _prepare_odoo_module_data(self, manifest_file_name):
        """
        remove unsupported keys which cannot be mapped with the fields of the odoo.module.version
        """
        vals = self._parse_manifest(manifest_file_name)
    ...
    def _discover_odoo_modules(self):
        ...
        for r in self.with_context(lang='en_US', no_generate_dependencies_on_create_write=True):
            ...
            for manifest_name in MANIFEST_NAMES:
                ...
                for manifest_file_name in glob.iglob(r.working_tree + '/**/' + manifest_name, recursive=True):
                    if not r._verify_module_manifest_location(manifest_file_name):
                        continue
                    # prepare module data
                    odoo_module_data = r._prepare_odoo_module_data(manifest_file_name)
    ...
    def action_discover_odoo_modules(self):
        if not self.env.su:
            if self.env.user.has_group('to_odoo_module.odoo_module_user'):
                self = self.sudo()
            else:
                raise UserError(_("Sorry, you don't have the permission for this operation. This operation is allowed for the group: \n\t - Odoo Apps/User"))

        # ensure branches are sorted by their Odoo versions ascendingly
        if len(self) > 1:
            self = self.sorted_by_odoo_version()
        for r in self:
        ...
        odoo_module_versions = self._discover_odoo_modules()
        return odoo_module_versions
