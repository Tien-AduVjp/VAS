...
class TestGitBranchDiscovery(common.TestGitCommon):
    ...
    def test_action_discover_odoo_module(self):
        ...
        git_branch = self.env['git.branch'].search([('name', '=', '14.0'), ('repository_id', '=', self.public_repo.id)])
        git_branch.write({
                'generate_app_products': True,
            })
        git_branch.checkout()
        git_branch.action_discover_odoo_modules()
        self.assertEqual(first=git_branch.odoo_module_versions_count,
                         second=2, msg='Public branch 14.0 of Viindoo test public should have 2 modules')
