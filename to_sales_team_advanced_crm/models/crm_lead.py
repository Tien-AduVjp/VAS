...
class Lead(models.Model):
    ...
    team_leader_id = fields.Many2one('res.users', string='Team Leader', compute='_compute_team_leader_id', store=True)
    ...
    def _compute_team_leader_id(self):
        for r in self:
            r.team_leader_id = r.team_id.user_id

    @api.depends('team_id', 'user_id')
    def _compute_crm_team_region_id(self):
        for r in self:
            if r.team_id:
                r.crm_team_region_id = r.team_id.crm_team_region_id
    ...
    def _convert_opportunity_data(self, customer, team_id=False):
        # This make outer function when calling write method not recompute team_id
        res = super(Lead, self)._convert_opportunity_data(customer=customer, team_id=team_id)
        team_id = team_id if team_id else self.team_id.id
        res['team_id'] = team_id
        return res
