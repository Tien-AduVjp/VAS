...
class TestPageAccessRights(SavepointCase):
    ...
    def test_restricted_editor_with_own_page(self):
        ...
        try:
            ...
            self.fail("Marc Demo should be able to edit his own page while it is not published")

        with self.assertRaises(AccessError):
            page.write({'is_published': True})

        try:
            page.with_user(self.user_admin).write({'is_published': True})
        except AccessError:
            self.fail("Admin should be able to publish Marc Demo's page")
