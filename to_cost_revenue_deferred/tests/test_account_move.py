...
class TestAccountMove(Common):
    ...
    def test_aml_onchange_product_revenue_01(self):
        f = Form(self.env['account.move'].with_context(default_move_type='out_invoice'), view='account.view_move_form')
    ...
    def test_aml_onchange_product_cost_01(self):
        f = Form(self.env['account.move'].with_context(default_move_type='in_invoice'), view='account.view_move_form')
        with f.invoice_line_ids.new() as line:
            line.product_id = self.product_A
        invoice = f.save()
        move_line = invoice.invoice_line_ids[0]
