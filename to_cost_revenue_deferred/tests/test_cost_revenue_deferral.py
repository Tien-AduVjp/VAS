...
class TestCostRevenueDeferral(Common):
    ...
    def test_deferral_category_onchange_journal_01(self):
        ...
        """
        f = Form(self.env['cost.revenue.deferral.category'].with_context(default_type='revenue'), view="to_cost_revenue_deferred.revenue_deferral_category_form")
        f.journal_id = self.journal_revenue_deferral
        self.assertEqual(f.deferred_account_id, self.journal_revenue_deferral.default_debit_account_id)
    ...
    def test_deferral_category_onchange_journal_02(self):
        ...
        """
        f = Form(self.env['cost.revenue.deferral.category'].with_context(default_type='cost'), view="to_cost_revenue_deferred.cost_deferral_category_form")
        f.journal_id = self.journal_cost_deferral
        self.assertEqual(f.deferred_account_id, self.journal_cost_deferral.default_credit_account_id)
    ...
    def test_deferral_onchange_deferral_category_id(self):
        ...
        """
        f = Form(self.env['cost.revenue.deferral'].with_context(default_type='revenue'), view="to_cost_revenue_deferred.cost_revenue_deferral_form")
