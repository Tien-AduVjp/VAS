...
class OkrNode(models.Model):
    ...
    year = fields.Char(default=_default_year, required=True, compute='_compute_year', store=True,
    ...
    result = fields.Selection([('successful', 'Successful'),
                              ('failed', 'Failed')], string="Result", tracking=True, compute='_compute_result', store=True,
                               help="Based on point, if point < 0.7 then Result will be set Failed else of point >= 0.7 Result will be set Successful")
    company_id = fields.Many2one('res.company', string="Company", compute='_compute_company', store=True, tracking=True,
    ...
    recursive_child_ids = fields.Many2many('okr.node', 'recursive_objective', 'parent_id', 'child_id', string='Recursive Children',
                                           compute='_compute_recursive_child_ids', store=True)
    points = fields.Float(string="Points", digits=(2, 2), compute='_compute_points', readonly=False, store=True, tracking=True, group_operator='avg')
    ...
    @api.depends('points', 'type')
    def _compute_result(self):
        for r in self:
            if (r.type == 'aspirational' and r.points > 0.7) or r.points >= 1:
                r.result = 'successful'
            else:
                r.result = 'failed'
    ...
    def unlink(self):
        ...
        return super(OkrNode, self).unlink()
