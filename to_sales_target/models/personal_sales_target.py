...
class PersonalSalesTarget(models.Model):
    ...
    sale_person_id = fields.Many2one('res.users', string='Sale Person', required=True, index=True,
                                     readonly=False, states={'confirmed': [('readonly', True)],
                                                             'refused': [('readonly', True)],
                                                             'approved': [('readonly', True)],
                                                             'cancelled': [('readonly', True)]}, ondelete='cascade')
    ...
    def subscribe_approvers(self):
        for r in self:
            ...
            if r.sale_person_id.sale_team_id:
                if r.sale_person_id.sale_team_id.user_id and r.sale_person_id.sale_team_id.user_id.partner_id not in r.message_follower_ids.partner_id:
                    subscribers += [r.sale_person_id.sale_team_id.user_id.partner_id.id]
                if r.sale_person_id.sale_team_id.regional_manager_id and r.sale_person_id.sale_team_id.regional_manager_id.partner_id not in r.message_follower_ids.partner_id:
