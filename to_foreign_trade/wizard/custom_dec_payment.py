...
#     export_tax_ids = fields.Many2many('custom.declaration.tax.export', string='Export Tax')
    ...
    def _onchange_custom_declaration(self):
        ...
        res['domain'] = {'partner_id': [('id', 'in', partner_ids.ids)]}
    ...
    def action_account_move_create(self):
        ...
        if account_move_line_ids:
            ...
            lines_to_reconcile.auto_reconcile_lines()
            self.try_set_done()

    @api.model
    def try_set_done(self):
        if self.custom_dec_import_id.is_paid:
            self.custom_dec_import_id.action_done()
        elif self.custom_dec_export_id.is_paid:
            self.custom_dec_export_id.action_done()
