...
class AbstractCustomDeclaration(models.AbstractModel):
    ...
    partner_id = fields.Many2one('res.partner', string="Custom Authority", required=True, readonly=False, states={'open': [('readonly', True)],
    ...
    stock_picking_id = fields.Many2one('stock.picking', string="Transfer", required=True, index=True, readonly=False,
    ...
    picking_type_id = fields.Many2one('stock.picking.type', string='Transfer Type', related='stock_picking_id.picking_type_id')

    company_id = fields.Many2one('res.company', string='Company', related='stock_picking_id.company_id', readonly=True, store=True)
    ...
    def create(self, vals_list):
        ...
        for r in recs:
            for tax_line_id in r.tax_line_ids:
                custom_declaration_line_ids = r.custom_declaration_line_ids.filtered(
                    lambda line: tax_line_id.product_id == line.product_id and tax_line_id.tax_id.id in line.tax_ids.ids)
                tax_line_id.custom_declaration_line_id = custom_declaration_line_ids and custom_declaration_line_ids[0].id or False 
        return recs
    ...
    def _check_stock_picking(self):
        for r in self:
            for move in r.stock_picking_id.move_lines:
                if move.product_id.valuation != 'real_time' or move.product_id.cost_method not in ['fifo', 'average']:
                    raise ValidationError(_("The product '%s' of the transfer '%s' must be configured in automated valuation with"
                                            ...
                                            % (move.product_id.display_name, r.stock_picking_id.name))
    ...
    def _onchange_stock_picking_id(self):
        ...
        if not 'warning' in res:
            self.custom_declaration_line_ids = self._prepare_custom_declaration_lines_data()
            return {}
        else:
            return res
    ...
    def action_open(self):
        for r in self:
            exists_request = r.search([
                ('stock_picking_id', '=', r.stock_picking_id.id),
                ('state', '!=', 'cancel'),
                ('id', '!=', r.id)
            ])
            if exists_request:
                raise UserError(_('Custom clearance document for transfer "%s" exists.\n You should either get back to work on that'
                                  ' document or cancel / delete it.')
                                % (self.stock_picking_id.name))

            if not r.custom_declaration_line_ids:
                raise UserError(_('Please create some operations.'))

            if r.stock_picking_id.state != 'done':
                raise UserError(_('Please validate the transfer \'%s\' before you can open this request!') % (r.stock_picking_id.name,))
            
            if not r._check_picking_and_declaration_lines_consistency():
                raise UserError(_('The transfer \'%s\' is not consistent with content of declaration. Please select transfer again!') % (r.stock_picking_id.name,))
    ...
    def _check_picking_and_declaration_lines_consistency(self):
        ...
        The check based on product_id, qty and product_uom
        """
        return False
