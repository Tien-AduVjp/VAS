...
class StockWarehouse(models.Model):
    ...
    def _create_foreign_trade_seq_pktype(self):
        ...
        for r in self:
            ...
            foreign_customer_loc = self.env.ref('to_foreign_trade.to_stock_location_customers_export')
    ...
    def _get_buy_import_pull_rule(self):
        buy_import_route_id = self.env.ref('to_foreign_trade.route_warehouse0_import')
        return {
...
#            ('location_id', '=', input_loc.id),
    ...
    def get_all_routes_for_wh(self, warehouse):
        ...
        if warehouse.buy_import_pull_id and warehouse.buy_import_pull_id.route_id:
            all_routes += [warehouse.buy_import_pull_id.route_id.id]
        return all_routes
        ...
        if warehouse.buy_import_pull_id and warehouse.buy_import_pull_id.route_id:
            for product_id in res:
                for route in self.env['product.product'].browse(product_id).route_ids:
                    if route.id == warehouse.buy_import_pull_id.route_id.id:
