...
class TestToHrSkillsRecruitment(Common):
    ...
    def test_create_employee_from_applicant(self):
        act_window = self.hr_applicant_1.create_employee_from_applicant()
        self.env['hr.employee'].with_context(act_window.get('context')).create({'name': 'Mitchell Demo 1'})
        employee = self.env['hr.employee'].search([('name', '=', 'Mitchell Demo 1')], limit=1)
        resume_line = employee.resume_line_ids[:1]
        employee_skill = employee.employee_skill_ids[:1]
