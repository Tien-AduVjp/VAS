...
class HrOvertimePlan(models.Model):
    ...
    state = fields.Selection([
        ...
        ('refuse', 'Refused'),
        ('cancel', 'Cancelled')
        ], default=False, compute='_compute_state', string='Status', store=True)
    ...
    def _get_overlap_domain(self):
        ...
        domain = expression.AND([
            domain,
            ['|', ('state', '=', False), ('state', 'in', ('confirm', 'validate1', 'validate', 'done'))]
            ])
        return domain
    ...
    def _compute_read_only(self):
        ...
        for r in self.filtered(lambda rec: rec.state in ('confirm', 'validate1', 'validate', 'done', 'refuse', 'cancel')):
            r.readonly = True

    @api.constrains('state', 'employee_id', 'date_start', 'date_end')
    def _check_overlap(self):
        for r in self:
            if r.state == False or r.state in ('confirm', 'validate1', 'validate', 'done'):
