...
class FleetVehicle(models.Model):
    ...
    def action_view_insurances(self):
        action = self.env.ref('to_fleet_insurance_basic.action_fleet_vehicle_insurance')
        result = action.read()[0]

        # reset context
        result['context'] = {'search_default_confirmed':1}
        # choose the view_mode accordingly
        if self.current_insurances_count != 1:
            result['domain'] = "[('id', 'in', " + str(self.insurance_ids.ids) + ")]"
            ...
            result['res_id'] = self.current_insurance_ids.id
        return result
