...
class TestFleetInsurance(Common):
    ...
    def test_02_insurance_type(self):
        ...
        with self.assertRaises(IntegrityError):
            ...
            self.insurance_type_public_liability_insurance.flush()

    #Case 10
    def test_expired_fleet_vehicle_insurance(self):
        """
        INPUT: set state expired and date_end = today + 5days for vehicle A
        OUTPUT: end_date of vehicle A = today
        """
        self.vehicle_insurance.write({'date_end': fields.Date.today() + relativedelta(days=5)})
        self.vehicle_insurance.action_set_expire()
        self.assertEqual(self.vehicle_insurance.date_end, fields.Date.today())
