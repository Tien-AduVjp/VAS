...
class MaintenanceEquipment(models.Model):
    ...
    # recursive children and parents
    ...
    # synch with parents
    assign_date = fields.Date(compute='_compute_assign_date', readonly=False, store=True)
    owner_user_id = fields.Many2one(compute='_compute_owner_user_id', readonly=False, store=True)
    ...
    def _check_location_synch(self):
        for r in self:
            if r.location and r.parent_id and r.location != r.parent_id.location:
                raise UserError(_("It is impossible for the part '%s' to be put in a location other than the location of its parent ('%s')'s location")
                                %(r.display_name, r.parent_id.display_name))
    ...
    def action_view_recursive_maintenances(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('maintenance.hr_equipment_request_action_from_equipment')
