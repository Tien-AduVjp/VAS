...
class WizardL10n_vn119_02GTGT (models.TransientModel):
    ...
    def _add_content_body(self, moves, content_body):
        ...
        tags |= self.env.ref('l10n_vn_common.account_analytic_tag_vat_taxable', raise_if_not_found=False) or tags
        tags |= self.env.ref('l10n_vn_common.account_analytic_tag_vat_exemption', raise_if_not_found=False) or tags
        tags |= self.env.ref('l10n_vn_common.account_analytic_tag_vat_vat_taxable_and_exemption', raise_if_not_found=False) or tags
        tags |= self.env.ref('l10n_vn_common.account_analytic_tag_goods_and_services_investment_projects', raise_if_not_found=False) or tags
        ...
        for tag in tags:
            ...
            for move in moves.line_ids.filtered(lambda line: tag in line.analytic_tag_ids and line.tax_ids).move_id:
                for tax in move.line_ids.tax_ids:
                    ...
                    if line_total:
                        vals = (move.legal_number or '', move.invoice_date, move.partner_id.commercial_partner_id.name, move.partner_id.commercial_partner_id.vat or '', sum(line_total) * refund, sum(line_taxes) * refund)
    ...
    def export_excel_purchase(self):
        ...
        for key, values in content_body.items():
            ...
            if key == self.env.ref('l10n_vn_common.account_analytic_tag_vat_taxable'):
            ...
            elif key == self.env.ref('l10n_vn_common.account_analytic_tag_vat_exemption'):
                worksheet.merge_range(
                    'A{0}:H{0}'.format(sequence_vat + count + 8),
                    _('%s. VAT Exemption Goods & Services for Production and Business') % (sequence_vat,),
                    content_cell_format_title
                    )
            elif key == self.env.ref('l10n_vn_common.account_analytic_tag_vat_vat_taxable_and_exemption'):
                worksheet.merge_range(
                    'A{0}:H{0}'.format(sequence_vat + count + 8),
                    _('%s. VAT Taxable and Exemption Goods & Services for Production and Business be eligible for VAT deduction') % (sequence_vat,),
                    content_cell_format_title
                    )
            elif key == self.env.ref('l10n_vn_common.account_analytic_tag_goods_and_services_investment_projects'):
                worksheet.merge_range(
                    'A{0}:H{0}'.format(sequence_vat + count + 8),
            ...
            if key == self.env.ref('l10n_vn_common.account_analytic_tag_vat_exemption'):
