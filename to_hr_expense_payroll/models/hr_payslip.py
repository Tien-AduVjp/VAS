...
class HrPayslip(models.Model):
    ...
    hr_expense_ids = fields.One2many('hr.expense', 'hr_payslip_id', string='HR Expenses')
    ...
    def action_payslip_done(self):
        ...
        for sheet in hr_expenses.sheet_id:
            if all(expense.state == 'done' for expense in sheet.expense_line_ids):
                sheet.set_to_paid()
        ...
        for r in self:
            if r.hr_expense_ids:
                ...
                for sheet in expenses.sheet_id:
                    if sheet.state == 'done' and sheet.account_move_id:
                        sheet.state = 'post'
                    else:
                        sheet.state = 'approve'    
                expenses.write({
