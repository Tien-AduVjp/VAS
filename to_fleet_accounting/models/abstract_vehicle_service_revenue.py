...
class FleetVehicleServiceRevenue(models.AbstractModel):
    _name = 'abstract.vehicle.service.revenue'
    _description = "Share business logics between vehicle service and revenue models"

    product_id = fields.Many2one('product.product', string='Invoiceable Product', index=True,
                                 help="The product to be used when invoicing the vehicle service")
    ...
    invoice_id = fields.Many2one('account.move', string='Invoice Ref.', related='invoice_line_id.move_id')
    ...
    def _guess_product(self):
        ...
        return self.product_id or product_obj.search([('name', '=', self.service_type_id.name)]) or product_obj.search([('name', 'ilike', '%' + self.service_type_id.name + '%')])
        ...
        for r in self:
            if r.vehicle_id.company_id and r.company_id:
                if r.company_id.id != r.vehicle_id.company_id.id:
                    raise ValidationError(_("You are about create the service of the vehicle '%s' for the company '%s'"
