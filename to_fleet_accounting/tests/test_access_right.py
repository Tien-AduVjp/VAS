...
class TestAccessRight(Common):
    ...
    def test_access_invoicing_user(self):
        ...
        cost_himself = self.generate_service_vehicle(self.vehicle_1, self.service_1, self.vendor_1, user_created=self.user_account_invoice)
        cost_himself.read()
        cost_himself.write({'description': 'clean'})
        self.assertRaises(AccessError, cost_himself.unlink)
        # Cost created by other user
        cost_other_user = self.generate_service_vehicle(self.vehicle_1, self.service_1, self.vendor_1).with_user(self.user_account_invoice)
        self.assertRaises(AccessError, cost_other_user.read)
        self.assertRaises(AccessError, cost_other_user.write, {'description': 'clean'})
