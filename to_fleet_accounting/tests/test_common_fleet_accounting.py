...
class TestCommonFleetAccouting(Common):
    ...
    def setUpClass(cls):
        ...
        cls.new_company = cls.env['res.company'].create({'name': 'New Company'})
    ...
    def test_autofill_cost_invoices_partner_change(self):
        # Create extra cost
        self.generate_cost_vehicle(self.vehicle_1, self.service_2, self.vendor_1)
    ...
    def test_compute_invoiceable_cost(self):
        ...
        cost_distribution_wz = Form(self.env['vehicle.cost.distribution'].with_context(active_ids=[cost_invoice_line.id],
        ...
        cost_just_generated = self.env['fleet.vehicle.cost'].search([]) - all_costs_current
        self.assertFalse(cost_just_generated.invoiceable)
        
        # Test cron clean garbage cost
        self.assertFalse(cost_just_generated.exists())
        cost_just_generated.invoice_line_id = False
        self.env.ref('to_fleet_accounting.ir_cron_fleet_vehicle_cost_garbage').method_direct_trigger()
        self.assertFalse(cost_just_generated.exists())
        
        cost_no_vendor = self.generate_cost_vehicle(self.vehicle_1, self.service_1, False)
    ...
    def test_contrain_create_invoice_from_cost(self):
        cost = self.generate_cost_vehicle(self.vehicle_1, self.service_1, False)
        with self.assertRaises(UserError):
            self.create_invoice_from_costs(cost)
    ...
    def test_constraint_save_company_on_vehicle_and_cost(self):
        with self.assertRaises(ValidationError):
            self.cost_1.company_id = self.env['res.company'].search([('id', '!=', self.env.company.id)], limit=1)
