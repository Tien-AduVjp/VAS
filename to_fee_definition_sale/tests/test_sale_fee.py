...
class TestSaleFee(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.fee_apply_to_virtual_service = cls.env['fee.definition'].create({
            'product_tmpl_id': cls.virtual_service.product_tmpl_id.id,
            'product_id': cls.service_01.id,
            'quantity': 1,
        })
    ...
    def test_create_nested_fee(self):
        self.assertEqual(self.product_1.nested_fee_definition_ids,
                         ...
                         self.fee_apply_to_virtual_service +
    ...
    def check_order_values(self, order):
        ...
        self.assertRecordValues(order.order_line,
                                [{'product_id': self.product_1.id, }, {'product_id': self.service_01.id},
                                 ...
                                 {'product_id': self.service_03_a.id}])
        # check order line fee after compute supplementary fees
        self.assertRecordValues(order.order_line_fee_ids, [{
            'fee_line_id': order.order_line.filtered(lambda l: l.product_id == self.service_01).id,
            'line_id': order.order_line.filtered(lambda l: l.product_id == self.product_1).id,
            'quantity': 1,
            'fee_definition_id': self.fee_apply_to_product_1.id,
            'lst_price': self.fee_apply_to_product_1.sale_price
        }, {
            'fee_line_id': order.order_line.filtered(lambda l: l.product_id == self.service_02).id,
            'line_id': order.order_line.filtered(lambda l: l.product_id == self.service_01).id,
            'quantity': 1,
            'fee_definition_id': self.fee_apply_to_service_01.id,
            'lst_price': self.fee_apply_to_service_01.sale_price
        }, {
            'fee_line_id': order.order_line.filtered(lambda l: l.product_id == self.virtual_service).id,
            'line_id': order.order_line.filtered(lambda l: l.product_id == self.service_01).id,
            'quantity': 1,
            'fee_definition_id': self.fee_apply_to_service_01__02.id,
            'lst_price': self.fee_apply_to_service_01__02.sale_price
        }, {
            'fee_line_id': order.order_line.filtered(lambda l: l.product_id == self.service_03_a).id,
            'line_id': order.order_line.filtered(lambda l: l.product_id == self.service_02).id,
            'quantity': 1,
