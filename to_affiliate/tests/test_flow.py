...
class TestFlow(TestAffiliateCommon):
    ...
    def test_affcode_unique_partner_company(self):
        with mute_logger('odoo.sql_db'):
            with self.assertRaises(IntegrityError):
                ...
                self.env['affiliate.code'].create({
    ...
    def test_create_affcode(self):
        ...
        affcode_partner_1 = self.env['affiliate.code'].create({
            'partner_id': partner_1.id
        })
        # then create user later
        user_with_partner_1 = self.env['res.users'].create({
            'login': 'pim',
            'partner_id': partner_1.id,
            'groups_id': [(6, 0, [self.group_portal.id])]
        })
    ...
    def test_02_remove_partner_has_been_linked_to_affiliate_code(self):
        ...
        commission = self.env['affiliate.commission'].create(vals)
    ...
    def test_default_commission_percentage(self):
        commission_rule = self.env['affiliate.commission.rule'].create({
            'name': 'Rule Test',
            'type': 'all'
        })
    ...
    def test_cancel_commission__invoice_paid(self):
        ...
        self.payment_request.user_id = self.user_aff.partner_id.id
