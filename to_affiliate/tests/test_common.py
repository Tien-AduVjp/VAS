...
class TestAffiliateCommon(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_internal_2 = cls.env['res.users'].create({
            'name': 'Yasuo',
            'login': 'affinternal2',
        ...
        cls.user_aff = cls.env['res.users'].create({
            'name': 'Affiliate User',
            'login': 'affuser',
            'email': 'Jax@abc.com',
        ...
        cls.manager_aff = cls.env['res.users'].create({
            'name': 'Rich Kid',
            'login': 'affmanager',
        ...
        cls.commission_rule_all = cls.env['affiliate.commission.rule'].create({
            'name': '11%',
            'type': 'all',
            'affiliate_comm_percentage': 11.00,
            'company_id': cls.company_1.id,
        })
        cls.commission_rule_category = cls.env['affiliate.commission.rule'].create({
            'name': '5%',
            'type': 'all',
        ...
        cls.commission_rule_product = cls.env['affiliate.commission.rule'].create({
            'name': '3%',
            'type': 'product_tmpl',
            'company_id': cls.company_1.id,
            'line_ids': [(0, 0, {
        ...
        cls.commission = cls.env['affiliate.commission'].create(cls.prepare_commission_data())

        cls.payment_request = cls.env['affiliate.payment'].create({
            'user_id': cls.user_internal.partner_id.id,
            'company_id': cls.company_1.id
        })

        cls.payment_request_2 = cls.env['affiliate.payment'].create({
            'user_id': cls.user_internal_2.partner_id.id,
    ...
    def check_commission(self, user):
        # own commission
        commission = self.env['affiliate.commission'].with_user(user).create(self.prepare_commission_data())
    ...
    def check_payment_request(self, user):
        # own payment request
        payment_request = self.env['affiliate.payment'].with_user(user).create({
            'user_id': self.user_internal.partner_id.id,
    ...
    def check_commission_rule(self, user):
        ...
        commission_rule_product.create({
            'name': '100%',
            'type': 'all'
        })
