...
class TestInvoiceLineSummaryBase(AccountTestInvoicingCommon):
    ...
    @classmethod
    def create_invoice(cls, invoice_line_summary_mode='product_unit_price_tax_discount', products=[], discount=0):
        move_form = Form(cls.env['account.move'].with_context(default_type='out_invoice'))
        move_form.invoice_date = fields.Date.to_date('2021-01-01')
        move_form.partner_id = cls.partner_a
        move_form.invoice_line_summary_mode = invoice_line_summary_mode
