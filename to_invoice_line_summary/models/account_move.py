...
class AccountMove(models.Model):
    ...
    invoice_line_summary_ids = fields.One2many('invoice.line.summary', 'invoice_id', string='Invoice Lines Summary', store=True,
                                                compute='_compute_invoice_line_summary_ids', groups='account.group_account_invoice')
    # TODO: rename `invoice_line_summary_mode` into `invoice_line_group_mode`
    invoice_line_summary_mode = fields.Selection([('product_unit_price_tax_discount', 'Product, Unit price, Tax, Discount'),
    ...
    def _compute_default_summary_mode(self):
        for r in self:
            r.invoice_line_summary_mode = r.journal_id.invoice_line_summary_mode
    ...
    def create(self, vals_list):
        ...
        for val in vals_list:
            ...
            if not journal.exists():
                move_type = 'entry'
                if val.get('type', False):
                    move_type = val['type']
                elif self._context.get('default_type', False):
                    move_type = self._context['default_type']
                journal = self.env['account.move'].with_context(default_type=move_type)._get_default_journal()
                val.setdefault('journal_id', journal.id)
            val.setdefault('invoice_line_summary_mode', journal.invoice_line_summary_mode)
            val.setdefault('invoice_display_mode', journal.invoice_display_mode)
        res = super(AccountMove, self).create(vals_list)
        return res

    @api.depends('invoice_line_summary_mode', 'invoice_line_ids.product_id',
    ...
    def _compute_invoice_line_summary_ids(self):
        ...
        for r in self:
            vals_list = [(5, 0, 0)]
            if r.type in self.get_invoice_types():
                invoice_lines_with_product = r.invoice_line_ids.filtered(lambda line: line.product_id)
                ...
                if r.invoice_line_summary_mode in ('product_unit_price_tax_discount', 'product_tax_discount'):
                ...
                for keys, group_invoice_lines in groupby(
                    ...
                    if r.invoice_line_summary_mode in ('product_unit_price_tax_discount', 'product_template_unit_price_tax_discount'):
