        ...
        if command[0].strip().lower() == '/ticket':
            if len(command) == 1:
                ...
                ticket_description = ''.join(
                    '<b>%s</b>: %s<br/>' % (message.author_id.name or self.anonymous_name, message.body)
                ...
                message = _(
                    'New helpdesk ticket has created: <a href="#" data-oe-id="%s" data-oe-model="helpdesk.ticket">%s</a>') \
                          % (helpdesk_ticket.id, html_escape(helpdesk_ticket.name))
        self._send_transient_message(partner, message)
