...
class ApprovalRequest(models.Model):
    ...
    def action_refuse(self):
        ...
        for approval in check_approval_request:
            if approval.overtime_plan_ids.line_ids.payslip_ids.filtered(lambda p: p.state != 'draft'):
                raise UserError(_("You cannot refuse an approval request '%s' while it still has a reference to"
                                ...
                                %(approval.display_name, 
                                  approval.overtime_plan_ids.line_ids.payslip_ids.filtered(lambda p: p.state != 'draft')[:1].display_name))
