    ...
    def action_view_customer_invoices(self):
        ...
        if not self.customer_invoice_ids:
            ...
            if default_journal_id:
                action['context']['default_journal_id'] = default_journal_id.id
        else:
            # Use the same account journal than a previous invoice
            action['context']['default_journal_id'] = self.customer_invoice_ids[0].journal_id.id
        ...
        if len(self.customer_invoice_ids) != 1:
            ...
            action['res_id'] = self.customer_invoice_ids.id
        return action
