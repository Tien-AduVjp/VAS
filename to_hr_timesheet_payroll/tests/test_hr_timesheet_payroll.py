...
class TestHrTimesheetPayroll(Common):
    ...
    def test_01_hr_timesheet_payroll(self):
        ...
        timesheet_cost = self.payslip_employee_1.timesheet_lines_count * \
                        self.payslip_employee_1.company_cost / self.payslip_employee_1.duty_working_hours * self.env.company.general_overhead
        self.assertRecordValues(
            self.payslip_employee_1,
            [{
                'timesheet_lines_count': 1,
    ...
    def test_05_hr_timesheet_payroll(self):
        ...
        Input: Nhân viên có một phiếu lương đang ở trang thái xác nhận/ hoàn thành
            Truy cập vào thông tin nhân viên, tab "Cài đặt nhân sự"
    ...
    def test_06_hr_timesheet_payroll(self):
        ...
        cost_employee = payslip_oct.currency_id.round(payslip_oct.company_cost / payslip_oct.duty_working_hours)
    ...
    def test_07_hr_timesheet_payroll(self):
        ...
        amount_timesheet_3 = payslip_oct.currency_id.round(self.timesheet_3.unit_amount * self.employee_2.timesheet_cost)
    ...
    def test_08_hr_timesheet_payroll(self):
        ...
        self.payslip_employee_1.action_payslip_verify()
        self.assertEqual(self.payslip_employee_1.state, 'verify')
        cost_employee_1 = self.payslip_employee_1.currency_id.round(self.payslip_employee_1.company_cost / self.payslip_employee_1.duty_working_hours * self.env.company.general_overhead)
        ...
        cost_employee_2 = self.payslip_employee_1.currency_id.round(self.payslip_employee_1.company_cost / self.payslip_employee_1.duty_working_hours * self.env.company.general_overhead)
    ...
    def test_11_hr_timesheet_payroll(self):
        ...
        - Output:
            + Timesheet 4,5 không hợp lệ không được tính vào phiếu lương
    ...
    def test_13_02_hr_timesheet_payroll(self):
        ...
        with self.assertRaises(UserError):
            self.timesheet_1.write({'amount': 100})
        ...
        with self.assertRaises(UserError):
            self.timesheet_1.write({'task_id': new_task.id})

        with self.assertRaises(UserError):
            self.timesheet_1.write({'project_id': new_project.id})
    ...
    def test_14_02_hr_timesheet_payroll(self):
        """
        Case 14: Kiểm tra Xóa thông tin chấm công khi đã liên kết với một phiếu lương
        TH2:
            - Input:
                + Chấm công nhân viên 1 liên kết với phiếu lương ở trạng thái Xác nhận/Hoàn thành
                + Thực hiện xóa thông tin chấm công: employee_id, amount, task_id, project_id
            - Output: Xóa thất bại -> Xảy ra ngoại lệ
        """
        self.assertIn(self.timesheet_1, self.payslip_employee_1.timesheet_line_ids)
        self.payslip_employee_1.action_payslip_verify()
        with self.assertRaises(UserError):
    ...
    def test_17_hr_timesheet_payroll(self):
        ...
        currency = self.payslip_employee_1.currency_id
        timesheet_cost_payslip = currency.round(self.payslip_employee_1.company_cost / self.payslip_employee_1.duty_working_hours * 2 \
        ...
        amount_timesheet_1 = -1 * currency.round(self.payslip_employee_1.company_cost / self.payslip_employee_1.duty_working_hours * 2 \
