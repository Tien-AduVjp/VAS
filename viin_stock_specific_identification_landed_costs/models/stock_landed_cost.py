...
class StockLandedCost(models.Model):
    ...
    def button_validate(self):
        ...
        specs_costs = self.filtered(lambda c: c.valuation_adjustment_lines\
                                              and all([l.spec_move_id for l in c.valuation_adjustment_lines]))
        res = super(StockLandedCost, self - specs_costs).button_validate()
        account_moves = self.env['account.move'].search([('ref', 'in', self.mapped('name'))])
        for r in self:
            valuation_layer_ids = []
            move_lines = []
            ...
            for line in r.valuation_adjustment_lines.filtered(lambda line: line.spec_move_id):
                line.write({'move_id': line.spec_move_id.id})
                ...
                move_lines.extend(line._create_accounting_entries(move, qty_out))
            ...
            for product in products:  # iterate on recordset to prefetch efficiently quantity_svl
                if not float_is_zero(product.quantity_svl, precision_rounding=product.uom_id.rounding):
                    product.with_context(force_company=r.company_id.id).sudo().standard_price += cost_to_add_byproduct[product] / product.quantity_svl
    ...
    def get_valuation_lines(self):
        ...
        try:
            ...
            error = _("You cannot apply landed costs on the chosen transfer(s). Landed costs can only be applied for products with automated inventory valuation and FIFO or average costing method.")
            if error == e.name:
                if lines:
                    return lines
                raise UserError(_("You cannot apply landed costs on the chosen transfer(s). Landed costs can only be applied for products with automated inventory valuation and FIFO or average or specific identification costing method."))
            else:
                raise
        return lines
