...
class StockValuationAdjustmentLines(models.Model):
    ...
    def _create_account_move_line(self, move, credit_account_id, debit_account_id, qty_out, already_out_account_id):
        ...
        AccountMoveLine = []
        landed_cost_lot_name = ''
        if self.env.context.get('landed_cost_lot_name', False):
            landed_cost_lot_name = ' (%s)' % self.env.context['landed_cost_lot_name']
        base_line = {
            'name': self.name + landed_cost_lot_name,
        ...
        if qty_out > 0:
            debit_line = dict(base_line,
                              name=(self.name + landed_cost_lot_name + ": " + str(qty_out) + _(' already out')),
                              quantity=0,
