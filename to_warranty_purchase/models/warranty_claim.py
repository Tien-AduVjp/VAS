...
class WarrantyClaim(models.Model):
    ...
    available_po_ids = fields.Many2many('purchase.order', compute='_compute_available_po_ids', string='Available PO',
                                        help="Technical field to filter purchase order based on selected product", groups='to_warranty_purchase.group_warranty_user_read_purchase')
    purchase_order_id = fields.Many2one('purchase.order', string='Purchase Order', readonly=False,
                                        ...
                                        domain="[('id', 'in', available_po_ids)]" , groups='to_warranty_purchase.group_warranty_user_read_purchase')
    ...
    def _compute_warranty_claim_policy_ids(self):
        ...
        for r in self:
            ...
            if r.type == 'vendor' and r.purchase_order_id:
                warranty_policies = r.purchase_order_id.order_line.filtered(lambda line: line.product_id == self.product_id).mapped('warranty_policy_ids')
                if warranty_policies:
                    for warranty_policy in warranty_policies:
                        warranty_claim_policies += warranty_claim_policies.new(warranty_policy._prepare_warranty_claim_policy_data())
    ...
    def action_investigation(self):
        for r in self:
            if r.type == 'vendor' and r.product_id and r.purchase_order_id:
