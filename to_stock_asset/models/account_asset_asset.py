...
class AccountAssetAsset(models.Model):
    ...
    def _check_same_production_lot_id(self):
    ...
    def _check_first_depreciation_date(self):
        res = super(AccountAssetAsset, self)._check_first_depreciation_date()
        ...
        for r in self:
            ...
            if dates:
                date = max(dates).date()
                first_depreciation_date = r.first_depreciation_date if r.date_first_depreciation == 'manual' else r.date
    ...
    def create_stock_move(self, name, location_id, location_dest_id, date):
        ...
        for r in self:
            ...
            move_val = {
                'name': _("%(name)s%(asset_name)s", name=name, asset_name=r.name.upper().replace(' ', '')),
                'date': date,
