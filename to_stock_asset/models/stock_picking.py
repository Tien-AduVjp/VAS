...
class StockPicking(models.Model):
    ...
    is_asset_allocation = fields.Boolean(compute='_compute_is_asset_allocation', store=True, help='A technical field to show/hide Assets button box.')
    ...
    def action_view_account_assets(self):
        self.ensure_one()
        action = self.env.ref('to_account_asset.action_account_asset_asset').read()[0]
        action['domain'] = [('id', 'in', self.move_line_ids.mapped('account_asset_asset_id').ids)]
        return action
    
    def button_validate(self):
        self.ensure_one()
        if self.picking_type_id.code == 'asset_allocation':
            if self.location_id.usage != 'asset_allocation' and self.location_dest_id.usage != 'asset_allocation':
            ...
            for product in self.mapped('move_lines.product_id') | self.mapped('move_line_ids.product_id'):
                if product.tracking == 'none':
                    raise UserError(_('Please enable Product Traceability on %s product!') % product.name)
                elif product.valuation != 'real_time':
                    raise UserError(_("The product '%s' of the transfer '%s' must be configured in "\
                                      "automated valuation method!") % (product.name, self.name))
