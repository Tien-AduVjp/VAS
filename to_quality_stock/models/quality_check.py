...
class QualityCheck(models.Model):
    ...
    picking_id = fields.Many2one('stock.picking', 'Operation', tracking=True, index=True)
    ...
    def _redirect_after_pass_fail(self):
        check = self.mapped('picking_id.check_ids').filtered(lambda x: x.quality_state == 'none')[:1]
        if check:
