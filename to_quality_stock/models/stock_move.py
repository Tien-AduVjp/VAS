...
class StockMove(models.Model):
    ...
    def _prepare_quality_check_values(self, picking, point, product_id):
        return {
            'type_id': point.type_id.id,
            'company_id': picking.company_id.id,
            'point_id': point.id,
            'picking_id': picking.id,
            'team_id': point.team_id.id,
            'product_id': product_id,
            'quantity_to_check': point.quantity_to_check,
            'checked_quantity': point.quantity_to_check,
    ...
    def _create_quality_checks(self):
        ...
        for picking, moves in pick_moves.items():
            domain_point = [('picking_type_id', '=', picking.picking_type_id.id),
                            ...
                            '&', ('product_id', '=', False), ('product_tmpl_id', 'in', moves.product_id.product_tmpl_id.ids)]
            ...
            for point in quality_points:
                if point.check_execute_now():
                    if point.product_id:
                        ...
                        products = picking.move_lines.filtered(lambda m: m.product_id.product_tmpl_id == point.product_tmpl_id).product_id
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        if not self.env.context.get('skip_check', False):
            stock_moves.sudo()._create_quality_checks()
        return stock_moves
