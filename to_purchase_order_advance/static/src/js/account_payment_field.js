odoo.define('to_purchase_order_advance.payment', function (require) {
"use strict";
...
payment.ShowPaymentLineWidget.include({
    _onOutstandingCreditAssign: function (event) {
        ...
        var payment_with_po = false;
        if (id && outstanding_value.content) {
            for (let i=0; i < outstanding_value.content.length; i++) {
                if (outstanding_value.content[i].id == id && outstanding_value.content[i].payment_with_po) {
                    payment_with_po = true;
                }
            }
        }
