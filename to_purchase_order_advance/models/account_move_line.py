...
class AccountMoveLine(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        advance_payment_purchase_ids = self._context.get('advance_payment_purchase_ids', False)
        if isinstance(advance_payment_purchase_ids, list):
            domain = [
                '|', '|',
                ('payment_id.purchase_order_ids', 'in', advance_payment_purchase_ids),
                ('payment_id.purchase_order_ids', '=', False),
                ('payment_id', '=', False),
            ]
    ...
    def _check_reconcile_with_po(self):
        inv_ids = self.move_id.filtered(lambda l: l.move_type == 'in_invoice')
        if not inv_ids:
            return
        ...
        for aml in self:
            ...
            purchase_orders = payment.purchase_order_ids
            if payment and purchase_orders:
                po_on_invoice = inv_ids.invoice_line_ids.purchase_line_id.order_id
                for po in po_on_invoice:
                    if po not in purchase_orders:
                        raise UserError(_("You can not reconcile entries those belong to different purchase orders."
                                          " Please add the purchase order '%s' to the payment '%s' to reconcile.")
                                          % (po.name, payment.name))
