...
class AccountPayment(models.Model):
    ...
    @api.onchange('purchase_order_ids', 'currency_id', 'payment_date')
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id not in self.purchase_order_ids.partner_id:
        ...
        for po in self.purchase_order_ids:
            ...
            if po.currency_id != self.currency_id:
                amount = po.currency_id._convert(amount,
                                                 self.currency_id,
                                                 po.company_id,
                                                 self.payment_date or po.date_order,
                                                 round=False)
            amount_residual += amount
        return amount_residual

    def _calculate_advance_payment_amount_with_po(self, currency):
        advance_payment_amount = 0.0
        for r in self.filtered(lambda r: r.state not in ['draft', 'cancelled']):
            if r.currency_id != currency:
                payment_amount = r.currency_id._convert(r.amount,
                                                        currency,
                                                        r.company_id,
                                                        r.payment_date,
                                                        round=True)
