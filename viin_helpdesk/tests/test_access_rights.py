...
class TestAccessRightsBase(TestHelpdeskCommon):
    ...
    def test_employee_user_ticket_access_rights(self):
        ...
        self.assertRaises(AccessError, ticket_public.with_user(self.user_employee_user).unlink)

        ticket_employee.with_user(SUPERUSER_ID).write({'stage_id': self.assigned_stage.id})
        ticket_employee.flush(['stage_id'])
        ticket_employee.with_user(self.user_employee_user).write({'name': 'Ticket 1'})
    ...
    def test_user_group_ticket_access_rights(self):
        ...
        ticket_portal.with_user(user_member_user).write({'name': 'ticket public'})
        ticket_portal.with_user(user_member_user).write({'name': 'ticket public'})
        ticket_employee.with_user(user_member_user).write({'name': 'ticket employee'})
