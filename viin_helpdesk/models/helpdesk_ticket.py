...
class HelpdeskTicket(models.Model):
    ...
    description = fields.Text(string='Description', help="Description detail of the request support by partner")
    ...
    def create(self, vals_list):
        ...
        # TODOS: remove this line on Odoo 14.0
            ...
            if stage_id and len(vals) == 1:
                ...
                if user == self.team_id.team_leader_id \
                   ...
                   or (user.has_group('base.group_user') and user == self.create_uid):
                    self = self.sudo()
            if not self.env.su:
                self._check_acess_right_to_write_or_unlink()

        res = super(HelpdeskTicket, self).write(vals)

        # TODOS: remove this line on Odoo 14.0
        self._reattaching_attachments_to_ticket()
    ...
    # TODOS: remove this method on Odoo 14.0
    def _reattaching_attachments_to_ticket(self):
        ...
        Re-attaching them to the ticket at the end of the create() / write()
        call fixes the ownership.
        """
        for r in self:
            # fix attachment ownership
            if r.attachment_ids:
                r.attachment_ids.write({'res_model': r._name, 'res_id': r.id})
    ...
    def action_view_all_rating(self):
        ...
        action = self.env['ir.actions.act_window'].for_xml_id('viin_helpdesk', 'rating_rating_action_view_ticket')
    ...
    def _check_acess_right_to_write_or_unlink(self):
        # Employee user is only allowed to write or unlink if ticket is in first stage,
        # but we can't make it here because it is a technical limitation, we can make it in python code
        user = self.env.user
        if user.has_group('base.group_user') and not user.has_group('viin_helpdesk.group_helpdesk_user'):
            for r in self:
                if not (r.create_uid == user and r.stage_id == r.team_id.stage_ids[:1]):
                    raise AccessDenied(_("You only write / delete the ticket '%s' which is in '%s' stage.") % (r.name, r.team_id.stage_ids[:1].name))
        elif not user.has_group('viin_helpdesk.group_helpdesk_manager'):
            for r in self:
                if not (r.create_uid == user or r.team_id.team_leader_id == user or r.user_id == user):
                    raise AccessDenied(_("You only write the ticket '%s' which is assigned to you.") % r.name)

    # Open new Wizard to Confirm and Update `team_id` and `user_id` to ticket record
    def assign_ticket_to(self):
        for r in self:
            if not self._context.get('call_from_wizard', False):
                view = self.env.ref('viin_helpdesk.action_confirm_wizard_form_view')
                ctx = dict(self._context or {})
                ctx.update({'default_ticket_id': r.id})
                return {
    ...
    def prepare_partner_to_mail_tempalte_data(self):
        self.ensure_one()
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype=None):
        return super(HelpdeskTicket, self).rating_apply(rate, token=token, feedback=feedback, subtype='viin_helpdesk.mt_ticket_rating')
