...
class HrOvertimeRequestMass(models.TransientModel):
    ...
    company_ids = fields.Many2many('res.company', string='Companies', default=lambda self: self.env.companies,
                                   domain=lambda self: [('id', 'in', self.env.companies.ids)], required=True)
    department_ids = fields.Many2many('hr.department', string='Departments', compute='_compute_department_ids', store=True,
                                      domain="[('company_id', 'in', company_ids)]", readonly=False)
    employee_ids = fields.Many2many('hr.employee', string='Employees', compute='_compute_employee_ids', store=True,
                                    domain="[('company_id', 'in', company_ids), ('department_id', 'in', department_ids)]", readonly=False)
    ...
    def action_schedule(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('viin_hr_overtime.action_hr_overtime_plan')
        result['context'] = {}
