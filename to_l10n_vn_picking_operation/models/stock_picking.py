...
class Picking(models.Model):
    ...
    def _compute_footer_date(self):
        for r in self:
            ...
            r.footer_date = _("Day %s Month %s Year %s") % (day, month, year)
    ...
    def _compute_reason(self):
        for r in self:
            if r.purchase_id:
                ...
                reason = _("According to the purchase order %s, placed on Day %s Month %s Year %s") % (r.purchase_id.name, day, month, year)
            elif r.sale_id:
                year, month, day = self.env['to.base'].split_date(r.sale_id.date_order)
                reason = _("According to the sales order %s, confirmed on Day %s Month %s Year %s") % (r.sale_id.name, day, month, year)
            elif r.group_id:
                reason = _("According to the %s") % r.group_id.name
            elif r.origin:
                reason = _("According to the %s") % r.origin
            else:
                reason = False
            r.reason = reason
