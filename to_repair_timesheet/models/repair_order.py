...
class Repair(models.Model):
    ...
    def _get_invoiced(self):
        for r in self:
            invoice_ids = r.fees_lines.invoice_lines.move_id.filtered(lambda r: r.type in ['out_invoice', 'out_refund'])
            invoice_ids |= r.operations.invoice_line_id.move_id.filtered(lambda r: r.type in ['out_invoice', 'out_refund'])
            refunds = invoice_ids.search([('invoice_origin', 'like', r.name), ('company_id', '=', r.company_id.id)]).filtered(lambda r: r.type in ['out_invoice', 'out_refund'])
            ...
            if invoice_ids:
                for inv in invoice_ids:
                    refund_ids += refund_ids.search([('type', '=', 'out_refund'), ('invoice_origin', '=', inv.name), ('invoice_origin', '!=', False), ('journal_id', '=', inv.journal_id.id)])
    ...
    def _prepare_timesheet(self, repair_fee, invoice_line):
        ...
        invoice_line_condition = (
            invoice_line.move_id.type == 'out_invoice' and
    ...
    def _prepare_invoice(self):
        ...
        journal = self.env['account.move'].with_context(force_company=company.id, default_type='out_invoice')._get_default_journal()
        if not journal:
    ...
    def action_invoice_create_same_partner(self, group=False, final=False):
        ...
        for r in self:
            ...
            for uninvoiced_fee_line in r.fees_lines.sorted(key=lambda l: l.qty_to_invoice < 0):
                ...
                if not invoice:
                    ...
                    invoice = self.env['account.move'].with_context(default_type='out_invoice').create(inv_data)
        ...
        for repair_fee in self.fees_lines.filtered(lambda feeline: not feeline.invoice_line_id):
