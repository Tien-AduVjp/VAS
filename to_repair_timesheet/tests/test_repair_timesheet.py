...
class TestRepairTimesheet(TestRepairTimesheetCommon):
    ...
    def test_16_00_create_invoice(self):
        """
            Input:
                Create repair order that has:
                    1 Repair Line
            ...
            Expect: Can't create invoice for this repair order
        """
        self.service_product.write({
            'service_tracking': 'task_global_project',
            'project_id': self.env.ref('sale_timesheet.project_support').id,
        ...
        repair_order.write({'partner_id': False})
        with self.assertRaises(UserError):
            self._create_invoice(repair_order, 'delivered')
    ...
    def test_36_check_timesheet_link_to_created_invoice(self):
        """
            Product:
                'service_policy': 'delivered_timesheet'
                'service_tracking': 'task_global_project'
            ...
            Input:
                Create a repair order that has 1 repair fee
                Add log timesheet
                Create invoice
                Add 2 log timesheet
                Create invoice
            ...
            Expect:
                Created 2 invoices in total
                The first timesheet record is linked to the first invoice
                The second, and the third timesheet record is linked to the second invoice
        """
        ...
        self.assertEqual(repair_order.invoice_ids[0].timesheet_count, 1)
        self.assertEqual(repair_order.invoice_ids[1].timesheet_count, 2)
