...
class TestAccessRights(TestBase):
    ...
    def test_access_user_repair_manager(self):
        ...
        - Expected Result: repair manager group has full permissions on stock production lot model
        """
        lot = self.env['stock.production.lot'].with_user(self.user_manager).create({
            'name': 'lot1',
        ...
        lot.with_user(self.user_manager).write({'name': 'lot 1 updated'})
        lot.with_user(self.user_manager).unlink()
