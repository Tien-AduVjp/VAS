...
class VNPayTest(VNPayCommon):
    ...
    def setUp(self):
        super(VNPayTest, self).setUp()
        # set default ip as 127.0.0.1 if request type as werkzeug.local.LocalProxy type
        if type(request) == werkzeug.local.LocalProxy:
            self.patch(type(self.env['payment.acquirer']), '_get_real_ip', lambda self: '127.0.0.1')
    ...
    def test_50_vnpay_form_render(self):
        ...
        ipaddr = self.vnpay._get_real_ip()
